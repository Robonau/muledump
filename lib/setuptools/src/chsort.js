...
setuptools.app.muledump.charSeen = function (guid, charid) {
  ...
  if (typeof setuptools.data.muledump.charsSeen[guid] !== "object")
    setuptools.data.muledump.charsSeen[guid] = {};
  ...
  if (
    ...
    for (var i in d.Char) {
      if (d.Char.hasOwnProperty(i)) {
        ...
        if (index > -1)
          CharList[index] = clist === true ? d.Char[i] : d.Char[i].id;
      }
    }
...
setuptools.app.muledump.charsort.create = function (guid, charlist) {
  if (
    ...
    setuptools.data.muledump.chsortcustom.accounts[guid] = $.extend(
      true,
      {},
      setuptools.objects.charlist
    );
  if (
    typeof setuptools.data.muledump.chsortcustom.accounts[guid].data[
      charlist
    ] === "undefined"
  ) {
...
setuptools.app.muledump.charsort.delete = function (guid, charlist) {
  if (
    typeof setuptools.data.muledump.chsortcustom.accounts[guid] ===
      "undefined" ||
    typeof setuptools.data.muledump.chsortcustom.accounts[guid].data[
      charlist
    ] === "undefined"
  ...
  if (setuptools.data.muledump.chsortcustom.accounts[guid].active === charlist)
 ...
 */
setuptools.app.muledump.charsort.add = function (
  guid,
  charlist,
  charid,
  position
...
setuptools.app.muledump.charsort.clean = function (guids) {
  if (typeof guids === "undefined")
    guids = Object.keys(setuptools.data.muledump.chsortcustom.accounts);
  ...
  for (var id = 0; id < guids.length; id++) {
    ...
    var guid = guids[id];
    var CustomList = setuptools.data.muledump.chsortcustom.accounts[guid];
    if (typeof CustomList === "object" && typeof CustomList.data === "object") {
      var saveReq = [];
      var d = mules[guid].data.query.results.Chars;
      ...
      for (var list in CustomList.data) {
        if (CustomList.data.hasOwnProperty(list)) {
          ...
          CustomList.data[list] = CustomList.data[list].filter(function (
            charid
          ) {
            return (
              d.Char.filter(function (char) {
                return char.id === charid;
              }).length > 0
            );
          });

          if (CustomList.data[list].length < length)
            saveReq.push([list, length - CustomList.data[list].length]);
        }
      }
      ...
      if (saveReq.length > 0)
        setuptools.app.config.save(
          "CharSort/Cleanup updated these lists: " + saveReq
        );
    }
  }
};
...
setuptools.app.muledump.charsort.sort = function (a, b) {
  ...
  //  by total fame
  if (setuptools.data.muledump.chsortcustom.sort === 2)
...
setuptools.app.muledump.charsort.menu = function (guid, charlist) {
  ...
  if (
    ...
    if (typeof charlist === "undefined") {
      ...
      function toggleButtonDraw() {
        ...
        var toggleButtonClasses =
          "muledump link toggleEnabled menuStyle textCenter mr0".split(" ");
        ...
        return (
          '<div class="' +
          toggleButtonClasses.join(" ") +
          '" style="padding: 5px 7px; margin-bottom: 14px; width: 225px;">' +
          toggleButtonText +
          "</div>"
        );
      }
      ...
      lists =
        typeof setuptools.data.muledump.chsortcustom.accounts[guid] === "object"
          ? Object.keys(
              setuptools.data.muledump.chsortcustom.accounts[guid].data
            )
          : [];
      if (lists.length > 0) {
        for (var i in lists) {
          if (lists.hasOwnProperty(i)) {
            setuptools.lightbox.build(
              ...
              ' \
                ...
                lists[i] +
                '" ' +
                (setuptools.app.muledump.charsort.active(guid) === lists[i]
                  ? "selected"
                  : "") +
                ">" +
                lists[i] +
    ...
    //  character sorting gui
      ...
      chars.sort(function (a, b) {
        if (list.indexOf(a.id) > -1 && list.indexOf(b.id) > -1)
      ...
      for (var cid in chars) {
        if (chars.hasOwnProperty(cid)) {
          portraits[chars[cid].id] = setuptools.app.muledump.drawPortrait(
            chars[cid]
          );
          portraitHtml +=
            '<div class="cell charsort ' +
            (list.indexOf(chars[cid].id) > -1 ? "enabled" : "") +
            '" data-charid="' +
            chars[cid].id +
            '">' +
      ...
      setuptools.lightbox.build(
        "muledump-charsort",
        ' \
                <div id="charField" class="charField flex-container noFlexAutoWidth scrollbar">' +
          portraitHtml +
          '</div> \
                ...
                <div class="flex-container" style="justify-content: space-between;">\
                    ...
                    <div style="display: flex; flex-flow: column;">\
                        ...
                        <select class="setuptools" name="chsortmode">\
                            <option value="0" ' +
          (setuptools.data.muledump.chsortcustom.sort === 0 ? "selected" : "") +
          '>Sort by ID</option> \
                            <option value="1" ' +
          (setuptools.data.muledump.chsortcustom.sort === 1 ? "selected" : "") +
          '>Sort by Fame</option> \
                            <option value="2" ' +
          (setuptools.data.muledump.chsortcustom.sort === 2 ? "selected" : "") +
          '>Sort by Total Fame</option> \
                            <option value="3" ' +
          (setuptools.data.muledump.chsortcustom.sort === 3 ? "selected" : "") +
          '>Sort by Class, then Fame</option> \
                        </select>\
                        <div>&nbsp;<br><strong>Disabled Characters Display Mode</strong></div>\
                        <select class="setuptools" name="chsortdisabledmode">\
                            <option value="0" ' +
          (setuptools.data.muledump.chsortcustom.disabledmode === false
            ? "selected"
            : "") +
          '>Off</option> \
                            <option value="1" ' +
          (setuptools.data.muledump.chsortcustom.disabledmode === true
            ? "selected"
            : "") +
          ">On</option> \
                        </select>\
  ...
  setuptools.lightbox.drawhelp(
    "muledump-charsort",
    "docs/setuptools/help/charsort.md",
    "Character Sorting Help"
  );
  if (goback === true)
    setuptools.lightbox.goback("muledump-charsort", function () {
  ...
  //  create new list and open it in the editor
  $("div.muledump.link.deleteList").on(
    "click.charsort.deleteList",
    function () {
  ...
  $('select[name="chsortmode"]').on("change.chsortmode", function () {
    ...
    if (value > -1 && value <= 3)
      setuptools.data.muledump.chsortcustom.sort = value;
    setuptools.app.muledump.charsort.unsaved = true;
    if (setuptools.data.muledump.chsortcustom.disabledmode === true)
  ...
  $('select[name="chsortdisabledmode"]').on(
    "change.chsortdisabledmode",
    function () {
      setuptools.data.muledump.chsortcustom.disabledmode =
        $(this).val() === "1";
      setuptools.app.muledump.charsort.unsaved = true;
      setuptools.app.muledump.charsort.menu(guid, charlist);
    }
  ...
  setuptools.tmp.itemSortDragging = new Muledump_Dragging({
    ...
    callbacks: {
      during_drag: function (parent, self) {
        ...
        if (!parent.CharList)
          parent.CharList = setuptools.app.muledump.charsort.read(
            guid,
            charlist
          );
        ...
        if (
          parent.srcIndex === -1 ||
          parent.destIndex === -1 ||
          self.attr("data-charid") === parent.target
        )
          return;
        return true;
      },
      after_drag: function (parent, self) {
        ...
        parent.CharList.splice(
          parent.CharList.indexOf(parent.target) + parent.indexModifier,
          0,
          parent.srcId
        );
  ...
  $("div.muledump.link.toggleEnabled").on("click.toggle.state", function () {
    ...
    if (state === true) {
      $(this)
        ...
        .addClass("negative")
        .text("Turn off Custom Character Sorting");

      if (setuptools.app.muledump.charsort.active(guid) === list)
        $("div.muledump.link.enableList")
          .removeClass("positive")
          .text("Disable");
      ...
      $(this)
        ...
        .addClass("positive")
        .text("Turn on Custom Character Sorting");
 ...
 */
setuptools.app.muledump.chsortcustomDedupAndValidate = function (
  CustomList,
  mule
) {
  ...
  for (var i = 0; i < mule.data.query.results.Chars.Char.length; i++)
    CharIdList.push(mule.data.query.results.Chars.Char[i].id);
    ...
    if (CustomList.hasOwnProperty(i)) {
      ...
      if (CharIdList.indexOf(CustomList[i]) === -1) {
        if (typeof CustomList[i] === "string")
          RemovedList.push(JSON.parse(JSON.stringify(CustomList[i])));
        delete CustomList[i];
      }
      ...
      if (DuplicateList.indexOf(CustomList[i]) > -1) {
        ...
        if (typeof CustomList[i] === "string")
          DuplicateList.push(JSON.parse(JSON.stringify(CustomList[i])));
      }
    }
  }
  ...
  for (var i in CustomList)
    if (CustomList.hasOwnProperty(i))
      if (typeof CustomList[i] === "string") FinalList.push(CustomList[i]);
...
setuptools.app.muledump.chsortcustom = function (mule) {
  if (!mule)
    setuptools.lightbox.error("No mule provided to custom sort utility.");

  //  prepare the object is none exists
  if (
    typeof setuptools.data.muledump.chsortcustom.accounts[mule.guid] ===
    "undefined"
  )
    setuptools.data.muledump.chsortcustom.accounts[mule.guid] = {
      active: "",
      data: {},
    };

  setuptools.lightbox.build(
    "muledump-chsortcustom-index",
    ' \
        Saving a list will set that list as active and overwrite any stored data for that list or create a new list item. \
        <br><br><strong>For account :</strong><span style="font-weight: bold;">:</span> ' +
      mule.guid +
      " \
        " +
      ...
      setuptools.data.accounts.accounts[mule.guid].ign.length > 0
        ? '<br><strong>IGN :</strong><span style="font-weight: bold;">:</span> ' +
          setuptools.data.accounts.accounts[mule.guid].ign +
          "</span>"
        : "") +
      '\
  ...
  if (
    Object.keys(setuptools.data.muledump.chsortcustom.accounts[mule.guid].data)
      .length > 0
  ) {
    setuptools.lightbox.build(
      "muledump-chsortcustom-index",
    ...
    //  loop thru data
    for (var i in setuptools.data.muledump.chsortcustom.accounts[mule.guid]
      .data)
      if (
        setuptools.data.muledump.chsortcustom.accounts[
          mule.guid
        ...
        setuptools.lightbox.build(
          ...
          ' \
            ...
            i +
            '" ' +
            (setuptools.app.muledump.charsort.active(this.guid) === i
              ? "selected"
              : "") +
            ">" +
            i +
  ...
  setuptools.lightbox.build(
    ...
    ' \
            ...
            <br><br><input name="chsortListName" class="setuptools app nomargin" value="' +
      setuptools.data.muledump.chsortcustom.accounts[mule.guid].active +
      '"> \
        </div>\
        <br><strong>Prepare List</strong> \
        <br><br>Enter a list of IDs separated by commas. \
        <br><br><input type="text" name="chsortcustom" class="setuptools app wideinput" value="' +
      (setuptools.data.muledump.chsortcustom.accounts[mule.guid].active.length >
      0
        ? setuptools.data.muledump.chsortcustom.accounts[mule.guid].data[
            setuptools.data.muledump.chsortcustom.accounts[mule.guid].active
          ].join(", ")
        : "") +
      '"> \
  ...
  setuptools.lightbox.settitle(
    "muledump-chsortcustom-index",
    "Character Sorting Lists"
  );
  setuptools.lightbox.drawhelp(
    "muledump-chsortcustom-index",
    "docs/features/character-sorting",
    "Character Sorting Lists Help"
  );
  ...
  $(".setuptools.link.muledump.save.delete").click(function () {
    ...
    if (
      typeof setuptools.data.muledump.chsortcustom.accounts[mule.guid].data[
        chsortId
      ] === "undefined"
    )
      return;
    ...
    //  delete the list
    delete setuptools
      .data.muledump.chsortcustom.accounts[mule.guid].data[chsortId];

    //  set default value to top key
    var newChsortId = Object.keys(
      setuptools.data.muledump.chsortcustom.accounts[mule.guid].data
    )[0];
    if (
      setuptools.data.muledump.chsortcustom.accounts[mule.guid].active ===
      ...
      setuptools.data.muledump.chsortcustom.accounts[mule.guid].active =
        typeof newChsortId === "undefined" ? "" : newChsortId;
    ...
    //  update the form
    $(
      'select[name="chsortExisting"] option[value="' + chsortId + '"]'
    ).remove();
    if (typeof newChsortId === "string") {
      $(
        'select[name="chsortExisting"] option[value="' + newChsortId + '"]'
  ...
  $(".setuptools.link.muledump.save.list").click(async function () {
    var UserListName = $('input[name="chsortListName"]').val();
    var UserInput = $('input[name="chsortcustom"]').val();
    var Lists = false;
    if (UserInput.length > 0)
      Lists = setuptools.app.muledump.chsortcustomDedupAndValidate(
        UserInput,
        mule
      );
    var SaveState = false;
    ...
    if (typeof Lists === "object" && Lists.FinalList.length > 0) {
      ...
      setuptools.data.muledump.chsortcustom.accounts[mule.guid].data[
        UserListName
      ] = $.extend(true, [], Lists.FinalList);
      SaveState = await setuptools.app.config.save("CharacterSort/Save");
      if (
        setuptools.config.devForcePoint !== "chsortcustom-save" &&
        SaveState === true
      ) {
        if (Lists.RemovedList.length > 0)
          setuptools.lightbox.build(
            "muledump-chsortcustom-save",
            "The follow IDs were invalid or duplicates: <br><br>" +
      ...
      if (UserInput.length === 0) {
        setuptools.lightbox.build(
          "muledump-chsortcustom-save",
          "Choose 'Delete List' to remove a list."
        );
      } else {
        setuptools.lightbox.build(
          "muledump-chsortcustom-save",
    ...
    if (SaveState === false)
      setuptools.lightbox.build(
        "muledump-chsortcustom-save",
        "Hmm, there was a problem saving. This setting will reset on page reload."
      );
