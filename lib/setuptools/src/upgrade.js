...
setuptools.app.upgrade.version = function () {
  ...
  function alertNewVersion() {
    ...
    if (setuptools.state.loaded === true && setuptools.state.hosted === true) {
      //  display the message
      var type =
        setuptools.data.config.alertNewVersion === 2 ? "version" : "release";
      setuptools.state.notifier = true;
      setuptools.lightbox.build(
        "muledump-alertNewVersion",
        " \
                Muledump Online has been updated to a new " +
          type +
          '. \
                <br><br>The changelog can be viewed here: \
                <br><a href="' +
          setuptools.config.url +
          '/CHANGELOG" class="drawhelp docs nostyle" target="_blank">' +
          setuptools.config.url +
          '/CHANGELOG</a> \
      ...
      $(".drawhelp.docs").click(function (e) {
        setuptools.lightbox.drawHelp(
          e,
          { title: "Muledump Changelog", url: $(this).attr("href") },
          this
        );
      });
      $(".setuptools.app.link.settingsmanager").on(
        "click.setuptools.newVersion.settingsmanager",
        function () {
          setuptools.app.config.settings("alertNewVersion");
        }
      );
    }
    ...
    if (setuptools.state.loaded === true) {
      setuptools.app.config.save(
        "SetupTools/ClientConfig version updated to " +
          JSON.stringify(setuptools.version)
      );
    } else {
      window.techlog(
        "SetupTools/ClientConfig version updated to " +
  ...
  if (setuptools.data.config.alertNewVersion === 2) {
    if (
      setuptools.version.major > setuptools.data.version.major ||
      setuptools.version.minor > setuptools.data.version.minor ||
      setuptools.version.patch > setuptools.data.version.patch
    )
      alertNewVersion();
...
setuptools.app.upgrade.seek = async function () {
  var newObject;
  var saveCount = 0;
  var autoBackup = {};
  ...
  /*
    ...
    */
  if (
    typeof setuptools.data.muledump.chsortcustom === "object" &&
    !setuptools.data.muledump.chsortcustom.format
  ) {
    ...
    window.techlog(
      "SetupTools/ClientConfig upgraded muledump.chsortcustom 0:1",
      "force"
    );
    saveCount++;
  }
  ...
  /*
    ...
    */
  if (
    typeof setuptools.data.groups !== "object" ||
    (typeof setuptools.data.groups === "object" &&
      Object.keys(setuptools.data.groups).length === 0)
  ) {
    setuptools.data.groups = $.extend(true, {}, setuptools.objects.dataGroup);
    window.techlog(
      "SetupTools/ClientConfig upgraded groups to new object format",
      "force"
    );
    saveCount++;
  }
  ...
  var checkObject =
    setuptools.data.accounts.accounts[
      Object.keys(setuptools.data.accounts.accounts)[0]
    ];
  ...
  if (accountsUpgrade === true) {
    ...
    window.techlog(
      "SetupTools/ClientConfig upgraded accounts with new keys",
      "force"
    );
    saveCount++;
  }
  ...
  if (
    ...
    Object.filter(
      setuptools.data.muledump.whitebagTracker.accounts,
      function (guid, data) {
    ...
    window.techlog(
      "SetupTools/ClientConfig upgraded whitebags to format 1",
      "force"
    );
    saveCount++;
  }
  ...
  Object.keys(setuptools.data.accounts.accounts).filter(function (guid) {
    ...
    if (
      typeof setuptools.data.muledump.wardrobe.accounts[guid] === "undefined"
    ) {
      ...
      if (
        ...
        setuptools.data.muledump.wardrobe.accounts[guid].format <
          setuptools.objects.wardrobeConfig.format
      ) {
        setuptools.data.muledump.wardrobe.accounts[guid] = $.extend(
          true,
          setuptools.objects.wardrobeConfig,
        ...
        window.techlog(
          "SetupTools/ClientConfig upgraded wardrobe to format " +
            setuptools.objects.wardrobeConfig.format,
          "force"
        );
        saveCount++;
  ...
  /*
    ...
    */
  if (typeof setuptools.data.accounts.meta.encryption === "undefined")
    setuptools.data.accounts.meta.encryption = false;
  if (typeof setuptools.data.accounts.meta.encryptionTestKey === "undefined")
  ...
  if (
    ...
    saveCount++;
    window.techlog(
      "SetupTools/ClientConfig enabled UA for 9.6 local first run",
      "force"
    );
  }
  ...
  /*
    ...
    */
  if (saveCount > 0) {
    autoBackup = await setuptools.app.config.backup(true, true);
    if (autoBackup.status === true) {
      setuptools.app.config.save(
        "SetupTools/ClientConfig updated " + saveCount + " keys"
      );
    } else
      window.techlog(
        "SetupTools/ClientConfig not saving accounts upgrade due to auto backup error",
        "force"
      );
  }
