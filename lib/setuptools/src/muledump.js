...
setuptools.app.muledump.warnData = function (Mule, name, data) {
  ...
  if (!setuptools.tmp.warnData[Mule.guid]) {
    ...
    setuptools.tmp.warnData[Mule.guid].dom = $(
      '<div class="button" title="Error in Account Data">'
    ).text("?");
    setuptools.tmp.warnData[Mule.guid].dom.prependTo(Mule.dom);

    //  capture clicks on the icon
    setuptools.tmp.warnData[Mule.guid].dom.click(async function () {
      setuptools.lightbox.build(
        "muledump-warnData",
        "Unusual account data was detected. The following items failed to process:"
      );
      ...
      for (var i in setuptools.tmp.warnData[Mule.guid].problems)
        if (setuptools.tmp.warnData[Mule.guid].problems.hasOwnProperty(i))
          setuptools.lightbox.build(
            "muledump-warnData",
            ' \
                        <br><br><strong class="problem">' +
              i +
              "</strong> \
                        <br>" +
              setuptools.tmp.warnData[Mule.guid].problems[i].join(", ") +
              " \
                    "
          );
      ...
      setuptools.lightbox.settitle(
        "muledump-warnData",
        "Account Data Problems"
      );
      setuptools.lightbox.display("muledump-warnData", {
        variant: "setuptools-medium",
      });

      $("strong.problem").css("text-transform", "capitalize");

      $(".setuptools.link.accountsManager").click(
        setuptools.app.accounts.manager
      );
    });
  }

  if (typeof setuptools.tmp.warnData[Mule.guid].problems[name] === "undefined")
    setuptools.tmp.warnData[Mule.guid].problems[name] = [];
...
setuptools.app.muledump.tooltip = function ($i, classes, content, ttl) {
  ...
  var contentCallback = function (self) {
    setuptools.lightbox.tooltip(self, content, {
      heightFrom: "tooltip",
      classes: classes,
    });
  };
  ...
  var defaultCallback = function (self) {
    ...
    html +=
      ' \
                ...
                <div class="ignore" style="display: flex; flex-flow: column; justify-content: center; align-items: center;"> \
                    <div class="bagType" style="background-position: ' +
      bagPosition +
      ';">&nbsp;</div> \
                </div>\
            ';
    ...
    html +=
      " \
            <div> \
                " +
      ItemData[0] +
      (ItemData[8] === true || $.isNumeric(oid)
        ? '<span class="tooltip generic text" style="margin-left: 2px;"> ' +
          ($.isNumeric(oid) ? "ID: " + oid : "(SB)") +
          "</span>"
        : "") +
      " \
                " +
      (ItemData[6]
        ? '<br><span class="tooltip feed">Feed Power: ' +
          ItemData[6] +
          "</span>"
        : "") +
      "\
                " +
      (count > 0
        ? '<br><span class="tooltip count">Count: <span class="tooltip generic text">' +
          count +
          "</span>"
        : "") +
      "\
    ...
    //  column three
    var tier = "";
    if (ItemData[2] > -1 && ItemData[1] !== 10)
      tier += '<span class="tooltip tiered">T' + ItemData[2] + "</span>";
    if (ItemData[9] === 1 && ItemData[1] !== 10)
      tier += '<span class="tooltip ut">UT</span>';
    ...
    html +=
      ...
      " \
                " +
      (ItemData[5]
        ? '<br><span class="tooltip generic text">XP Bonus:</span> <span class="tooltip famebonus value">' +
          ItemData[5] +
          "%</span>"
        : "") +
      " \
  ...
  $i.off("mouseenter.muledump.tooltip").on(
    "mouseenter.muledump.tooltip",
    function (e) {
      ...
      setuptools.tmp.tstateOpen = setTimeout(
        typeof content === "string" ? contentCallback : defaultCallback,
        ttl,
        self
      );
...
setuptools.app.muledump.notices.monitor = function () {
  if (typeof setuptools.tmp.noticesMonitorMax !== "number")
    setuptools.tmp.noticesMonitorMax = 0;
  if (setuptools.tmp.noticesMonitorInterval)
    clearInterval(setuptools.tmp.noticesMonitorInterval);
  ...
  setuptools.tmp.noticesMonitorInterval = setInterval(function () {
    ...
    if (
      setuptools.tmp.noticesMonitorAge > setuptools.config.noticesMonitorMaxAge
    )
      clearInterval(setuptools.tmp.noticesMonitorInterval);
  }, 1000);
};
...
setuptools.app.muledump.notices.remove = function (queuePos) {
  ...
  if (setuptools.app.muledump.notices.queue.length === 0)
    noticeDom.fadeOut(1000, function () {
      $(this).remove();
    });
};
...
setuptools.app.muledump.notices.display = function () {
  ...
  for (var i = 0; i < queue.length; i++)
    if (typeof queue[i].modifiers === "object") {
      //  determine if this queue object has priority modifiers
      if (
        typeof queue[i].modifiers.notifyClass === "string" &&
        classOrder.indexOf(queue[i].modifiers.notifyClass) >
          classOrder.indexOf(notifyClass)
      )
      ...
      if (
        classOrder.indexOf(notifyClass) === 0 ||
        notifyClass === queue[i].modifiers.notifyClass
      )
        modifiers = queue[i].modifiers;
    }

  //  build the context menu
  var noticeDom = $("#notice");
  noticeDom.unbind("click").click(async function () {
    ...
    if (typeof modifiers.menuClass === "string")
      options.push({
        option: "class",
    ...
    setuptools.lightbox.menu.context.create(
      "notices",
      false,
      noticeDom,
      options
...
setuptools.app.muledump.pagesearch.mode.account.init = function () {
  ...
  setuptools.app.muledump.pagesearch.paginate =
    new setuptools.lightbox.menu.paginate.create(
      ...
      undefined,
      {
        search: {
          ...
          context: function (self, e) {
            e.preventDefault();
            setuptools.app.muledump.mulemenu(
              e,
              $(self).text(),
              undefined,
              $("div#pagesearch"),
 ...
 */
setuptools.app.muledump.pagesearch.mode.account.execute = function (
  state,
  searchTerm
) {
  //  find our result object
  var object = setuptools.app.muledump.pagesearch.paginate.PageList.find(
    function (element) {
  ...
  if (setuptools.data.config.pagesearch === 1)
  ...
  options.push(
      ...
      name:
        ' \
                <div class="flex-container" style="justify-content: flex-start;"> \
                    <div class="pagesearch account button mr5 ' +
        (setuptools.data.config.pagesearchMode === 0 ? "active" : "") +
        '">&nbsp;</div>\
    ...
    },
    {
      ...
      name:
        ' \
                <div class="flex-container" style="justify-content: flex-start;"> \
                    <div class="pagesearch items button mr5 ' +
        (setuptools.data.config.pagesearchMode === 1 ? "active" : "") +
        '">&nbsp;</div>\
                    <div>Item Search</div> \
                </div>\
            ',
...
setuptools.app.muledump.keys = function (keys, e, raw) {
  ...
  var keymap = $.extend(
    true,
    setuptools.data.muledump.keys[0],
    setuptools.data.muledump.keys[setuptools.data.config.keyBindings]
  );
  var result = {};
...
setuptools.app.muledump.toggleoption = function (option, value, skip) {
  ...
  for (var i = 0; i < option.length; i++)
    setuptools.data.options[option[i]] =
...
setuptools.app.muledump.bodymenu = function (e) {
  ...
  if (
    MuleQueueButton.hasClass("running") === true ||
    MuleQueueButton.hasClass("paused") === true
  ) {
    options.push({
      option: "header",
      value: "MuleQueue",
 ...
 */
setuptools.app.muledump.mulemenu = function (
  e,
  guid,
  context,
  track,
  ...
  //  convert ign to guid
  if (
    typeof accounts[guid] === "undefined" &&
    setuptools.app.config.determineFormat(setuptools.data.accounts) === 1
  )
    Object.keys(setuptools.data.accounts.accounts).filter(function (element) {
      if (setuptools.data.accounts.accounts[element].ign === guid)
        guid = element;
    });
  ...
  if (typeof extraopts.top === "object")
    options = options.concat(extraopts.top);
  ...
  if (setuptools.data.options.totals === true)
    options.push({
      ...
      name:
        (Array.isArray(
          setuptools.app.muledump.totals.config.getKey("accountFilter")
        ) === false ||
        (Array.isArray(
          setuptools.app.muledump.totals.config.getKey("accountFilter")
        ) === true &&
          setuptools.app.muledump.totals.config
            .getKey("accountFilter")
            .indexOf(guid) === -1)
          ? "Add to"
          : "Remove from") + " Account Filter",
      callback: function (guid) {
        if (
          setuptools.app.muledump.totals.config
            .getKey("accountFilter")
  ...
  if (
    ...
    (setuptools.state.loaded === true &&
      setuptools.data.accounts.accounts[guid].loginOnly === false)
  )
    options.push({
      class: "chsort",
  ...
  if (mules[guid].loginOnly === false && mules[guid].disabled === false)
    options.push({
  ...
  if (typeof extraopts.bottom === "object")
    options = options.concat(extraopts.bottom);
...
setuptools.app.muledump.screenshotmenu = function (guid) {
  ...
  if (mules[guid].opt("accountinfo"))
    options.push({
      class: "ssAccountInfo",
      name: "Account Info",
      callback: function (guid) {
        setuptools.app.muledump.exporter.canvas(
          $(mules[guid].dom.find("div.stats"))[0]
        );
      },
      callbackArg: guid,
    });

  if ($(mules[guid].dom.find("table.chars"))[0] !== undefined)
    options.push({
      class: "ssCharTable",
      name: "Character Table",
      callback: function (guid) {
        setuptools.app.muledump.exporter.canvas(
          $(mules[guid].dom.find("table.chars"))[0]
        );
      },
      callbackArg: guid,
    });

  if (mules[guid].opt("vaults"))
    options.push({
      class: "ssVaults",
      name: "Vault Table",
      callback: function (guid) {
        setuptools.app.muledump.exporter.canvas(
          $(mules[guid].dom.find("table.vaults"))[0]
        );
      },
      callbackArg: guid,
    });

  if (mules[guid].opt("potions"))
    options.push({
      class: "ssPotions",
      name: "Potion Storage",
      callback: function (guid) {
        setuptools.app.muledump.exporter.canvas(
          $(mules[guid].dom.find("table.potions"))[0]
        );
      },
      callbackArg: guid,
    });

  if (mules[guid].opt("gifts"))
    options.push({
      class: "ssGifts",
      name: "Gift Table",
      callback: function (guid) {
        setuptools.app.muledump.exporter.canvas(
          $(mules[guid].dom.find("table.gifts"))[0]
        );
      },
      callbackArg: guid,
    });
 ...
 */
setuptools.app.muledump.copymenu = async function (guid) {
  ...
  options.push({
    ...
    callback: function (guid) {
      new ClipboardJS("div.setuptools.menu > div.link", {
        text: function () {
          return setuptools.app.config.determineFormat(
  ...
  //  offer deep copy
  const ttttmmmmpppp = await setuptools.app.accounts.ExportDeepCopy(guid, true);
  options.push({
...
setuptools.app.muledump.totalsWidth = function (totals) {
  ...
  if (setuptools.data.config.totalswidth === 0) {
    var maxItems = Math.floor(
      $(window).innerWidth() / setuptools.config.totalsItemWidth
    );
...
setuptools.app.muledump.realmeye.link = function (itemNumber, link, extra) {
  ...
  //  determine itemName
  if (typeof itemNumber === "number" || typeof Number(itemNumber) === "number")
    itemName = items[itemNumber][0];
  ...
  if (
    ...
    (typeof extra === "number" || typeof Number(extra) === "number")
  )
    extra = "/" + extra;
...
setuptools.app.muledump.realmeye.itemCtrlUp = function (e) {
  ...
  if (selected.length === 1) {
    ...
    var options = [
      ...
      },
      {
        ...
        callback: function () {
          for (var i in setuptools.app.muledump.realmeye.state.selected)
            if (
              setuptools.app.muledump.realmeye.state.selected.hasOwnProperty(i)
            )
              $(setuptools.app.muledump.realmeye.state.selected[i]).removeClass(
                "realmeyeBorder"
              );
      ...
      },
      {
        option: "link",
        name: "View in Wiki",
        href: setuptools.app.muledump.realmeye.link(
          $(selected[0]).attr("data-itemid"),
          "wiki"
        ),
        target: "_blank",
      },
    ...
    if (
      setuptools.config.realmeyeOfferIds.indexOf(
        Number($(selected[0]).attr("data-itemid"))
      ) > -1
    )
      options.push(
        {
  ...
  //  trading menu
    ...
    for (var x = 0; x < selected.length - 2; x++)
      $(selected[x]).removeClass("realmeyeBorder");
    ...
    if (
      ...
      setuptools.config.realmeyeOfferIds.indexOf(
        Number($(selected[1]).attr("data-itemid"))
      ) > -1
    ) {
      options.push(
        {
...
setuptools.app.muledump.cleanupSecondaryCache = function (string) {
  ...
  var keyCheck = new RegExp(
    "^" + setuptools.config.keyPrefix + "cache:.+$",
    "i"
  );
...
setuptools.app.muledump.cleanupDataForGuid = function (guid) {
  ...
  for (var key in localStorage)
    if (localStorage.hasOwnProperty(key))
      if (key.indexOf(guid) > -1) localStorage.removeItem(key);
};
...
setuptools.app.muledump.about = function () {
  if (setuptools.state.hosted === false) {
    ...
    setuptools.lightbox.build(
      "muledump-checkupdates",
      ' \
            You are on the latest version. \
            <br><br><a href="#" class="muledump link renders nostyle" target="_blank">Renders</a> | \
            <a href="' +
        setuptools.config.url +
        '/CHANGELOG" class="drawhelp docs nostyle" target="_blank">v' +
        VERSION +
        ' Changelog</a> | \
            <a href="' +
        setuptools.config.url +
        '/" target="_blank">Muledump Homepage</a> | \
    ...
    $("a.muledump.link.renders").on("click.muledump.renders", function (e) {
      e.preventDefault();
      setuptools.lightbox.override(
        "rendersupdate-assistant",
        "goback",
        setuptools.app.muledump.about
      );
...
setuptools.app.muledump.checkupdates = function (force, renders) {
  ...
  function BaseMessage() {
    setuptools.lightbox.build(
      "muledump-about",
      "You are on the latest release.<br>"
    );
  }
  ...
  function DisplayMessage() {
    ...
    setuptools.lightbox.build(
      ...
      ' \
        ...
        '/" target="_blank">Muledump Homepage</a> | \
                    ...
                    <br><br>Check out <a href="' +
        setuptools.config.url +
        '/muledump.html" target="_blank">Muledump Online</a> to see it in action. \
                '
    );
    if (
      setuptools.state.loaded === true &&
    ...
    setuptools.lightbox.settitle(
      "muledump-about",
      "<strong>Muledump Local v" + VERSION + "</strong>"
    );
  ...
  async function ProcessResponse(data) {
    ...
    if (typeof setuptools.tmp.updatecheck === "undefined")
      setuptools.tmp.updatecheck = {
        expires: Date.now() + setuptools.config.updatecheckTTL,
        data: data,
      };

    //  head of renders check
    var currentRendersData = rendersVersion.match(
      setuptools.config.regex.renderscheck
    );
    var currentRenders = new Date(
      currentRendersData[1],
      currentRendersData[2] - 1,
      currentRendersData[3],
      currentRendersData[4],
      currentRendersData[5],
      currentRendersData[6]
    );
    var latestRenders = new Date(
      currentRendersData[1],
      currentRendersData[2] - 1,
      currentRendersData[3],
      currentRendersData[4],
    ...
    for (var i = 0; i < d.length; i++) {
      //  version check
      if (
        d[i].name.indexOf("renders-") === -1 &&
        cmpver(d[i].name, topver) > 0
      ) {
        topver = d[i].name;
    ...
    if (latestRenders > currentRenders)
    ...
    if (force === true && renders === true) {
      setuptools.app.assistants.rendersupdate(
        $.extend(true, [], latestRendersData, rendersData, {
    ...
    var notifiedver = await setuptools.storage.read("updatecheck-notifier");
    if (url)
      setuptools.app.muledump.notices.add(
    ...
    if (
      url &&
      ((force === true && renders !== true) ||
        (!force &&
          setuptools.data.options.updatecheck === true &&
          (typeof notifiedver === "undefined" ||
            (typeof notifiedver === "string" &&
              cmpver(notifiedver, topver) > 0))))
    ) {
      ...
      await setuptools.storage.write("updatecheck-notifier", topver);
    }
  ...
  if (
    ...
    if (typeof setuptools.tmp.updatecheck === "object")
      delete setuptools.tmp.updatecheck;
...
setuptools.app.muledump.whitebag.items = function (full) {
  ...
  setuptools.tmp.whitebagsFull =
    typeof setuptools.tmp.whitebagsFull === "object"
  ...
  if (typeof setuptools.tmp.whitebagsCompact === "object")
...
setuptools.app.muledump.whitebag.init = function (guid, counters, owned) {
  ...
  if (typeof config.accounts[guid] === "undefined")
    config.accounts[guid] = $.extend(
      true,
      {},
      setuptools.objects.whitebagTrackerAccount
    );
...
setuptools.app.muledump.whitebag.tracker = function (guid) {
  ...
  var config = $.extend(
    true,
    {},
    setuptools.data.muledump.whitebagTracker.accounts[guid]
  );
        ...
        <div class="wbmenu flex-container">\
            <div class="flex-container mr5">\
                <select class="setuptools menuTiny" name="wbaction">\
                    <option value="totals">' +
      (setuptools.data.config.wbTotals === true ? "Disable" : "Enable") +
      ' Totals</option>\
                    <option>Save</option>\
                    <option>Clear</option>\
        ...
        <div class="wbmenu flex-container">\
            <div class="flex-container mr5">\
                <select class="setuptools menuTiny textCenter" name="wbexport">\
                    <option value="txt" ' +
      (setuptools.data.config.exportDefault === 1 ? "selected" : "") +
      ">Text</option>\
                    <option " +
      (setuptools.data.config.exportDefault === 2 ? "selected" : "") +
      ">CSV</option>\
                    <option " +
      (setuptools.data.config.exportDefault === 3 ? "selected" : "") +
      ">JSON</option>\
                    <option " +
      (setuptools.data.config.exportDefault === 4 ? "selected" : "") +
      ">Image</option>\
                    <option " +
      (setuptools.data.config.exportDefault === 5 ? "selected" : "") +
      '>Imgur</option>\
                </select>\
            </div>\
  ...
  setuptools.lightbox.drawhelp(
    "whitebag-totals",
    "docs/muledump/whitebags",
    "White Bag Tracker Help"
  );
  ...
  var whitebags = {};
  var wbitems = setuptools.app.muledump.whitebag
    .items()
  ...
  wbitems.filter(function (key) {
    var selected = false;
    if (config.owned.indexOf(key) === -1 && config.items[key] > 0)
      config.owned.push(key);
    if (config.owned.indexOf(key) > -1) selected = true;
    var itemDom = setuptools.app.muledump.item(
      key,
      "wb noselect" + (selected === true ? " selected" : ""),
  ...
  //  bind tooltips
  if (setuptools.data.config.tooltipItems > -1)
  ...
  wbitemsdom.on("click.wb.increment", function (e) {
    ...
    if (setuptools.app.muledump.keys("shift", e) === true) {
      if (config.owned.indexOf(itemid) === -1) {
        ...
        config.owned.splice(config.owned.indexOf(itemid), 1);
        config.items[itemid] = 0;
      }
    ...
    value =
      setuptools.app.muledump.keys("ctrl", e) === true
        ? value > 0
          ? value - 1
          : value
        : value + 1;
    ...
    if (value === 0 && config.owned.indexOf(itemid) === -1)
      $(this).removeClass("selected");
  ...
  $("div.muledump.link.go").on("click.wbmenu.go", function () {
    ...
    if (action === "reset") {
      ...
      if (
        setuptools.tmp.lightboxStatus &&
        setuptools.tmp.lightboxStatus.indexOf("wb-save") > -1
      )
        return;
      setuptools.tmp.wbSaveOnClose = false;
      setuptools.app.muledump.whitebag.save(guid, config);
      ...
      setuptools.app.config.save(
        "WhiteBag/Tracker switching wbTotals option",
        true
      );
      setuptools.app.muledump.whitebag.tracker(guid);
      ...
      setuptools.lightbox.drawhelp(
        "whitebags-import",
        "docs/features/whitebags",
        "White Bag Tracker Help"
      );
      setuptools.lightbox.display("whitebags-import", {
        variant: "nobackground",
      });

      //  bind cancel button
      $("div.muledump.link.import.cancel").on(
        "click.import.cancel",
        function () {
      ...
      //  bind confirm button
      $("div.muledump.link.import:not(.cancel)").on(
        ...
        function (e) {
          ...
          wbitems.filter(function (itemid) {
            ...
            if (mules[guid].totals instanceof Muledump_TotalsCounter)
              newvalue += mules[guid].totals.data.totals[itemid] || 0;
            setuptools.data.muledump.whitebagTracker.accounts[guid].items[
              itemid
            ] =
              (setuptools.data.muledump.whitebagTracker.accounts[guid].items[
                itemid
  ...
  $("div.muledump.link.export").on("click.wb.export", function (e) {
    ...
    setuptools.lightbox.insert("exporter-main", "goback", [
      "exporter-main",
      function () {
        setuptools.app.muledump.whitebag.tracker(guid);
      },
...
setuptools.app.muledump.exporter.canvas = function (dom, quiet, callback) {
  ...
  if (quiet !== true) {
    setuptools.lightbox.build(
      "canvas-loading",
      "This may take awhile for larger images. Please wait..."
    );
...
setuptools.app.muledump.exporter.imgur = function (canvas, callback) {
  if (typeof callback !== "function")
    callback = function (data, err, errDesc) {
      setuptools.app.techlog(
        "Muledump/Exporter imgur received response: " +
          (err || errDesc ? errDesc : JSON.stringify(data))
      );
    };
...
setuptools.app.muledump.exporter.paste = function (type, data, callback) {
  if (typeof callback !== "function")
    callback = function (data, err, errDesc) {
      setuptools.app.techlog(
        "Muledump/Exporter Paste API response: " + JSON.stringify(data)
      );
    };
  ...
  if (typeof setuptools.data.exporter.remote.paste === "string")
    pasteOptions.apiKey = setuptools.data.exporter.remote.paste;
...
setuptools.app.muledump.exporter.saveLink = function (string, type) {
  ...
  if (
    setuptools.data.exporter.storedLinks.length >
    setuptools.config.exporterStoredLinks
  )
    setuptools.data.exporter.storedLinks.splice(
      0,
      setuptools.data.exporter.storedLinks.length -
        setuptools.config.exporterStoredLinks
    );
...
setuptools.app.muledump.exporter.ui.main = function (canvas, type, override) {
  ...
  function render(canvas) {
    ...
    $("div.exporter.status").html(
      "<div> \
        " +
        (typeof canvas === "string" && canvas.match(/^https?:\/\/.*$/)
          ? '<br>Your Imgur Link: <a href="' +
            canvas +
            '" target="_blank">' +
            canvas +
            "</a><br>&nbsp;"
  ...
  setuptools.lightbox.build(
    ...
    '\
      ...
      '>JSON</option>\
            ...
            <div class="setuptools link canvas totals export remote imgur menuStyle menuSmall notice textCenter noclose mr0">Upload to  ' +
      (type === "image" || type === "imgur" || typeof type === "undefined"
        ? "Imgur"
        : "Paste") +
      '</div>\
        ...
        <div class="flex-container exporter canvas">' +
      (typeof canvas === "string" ? "Loading image..." : "") +
      '</div>\
  ...
  setuptools.lightbox.drawhelp(
    "exporter-main",
    "docs/muledump/exporter",
    "Muledump Exporter Help"
  );
  ...
  function procLocal(type) {
    if (type === "image") {
      ...
      $("div.exporter.status").html("");
      $("div.exporter.canvas").html(
        '<textarea class="setuptools text exporter w100 scrollbar">' +
          text +
          "</textarea>"
      );
    }
  ...
  function procRemote(type) {
    ...
    if (type === "image") {
      function imgur(data, err, errDesc) {
        ...
        //  no we did not
          ...
          statusDom.prepend(
            '<br><strong class="negative">Error</strong>: Failed to upload to Imgur<br>' +
              errDesc
          );
        }
      }
      ...
      if (
        image.length === 0 ||
        (image.length === 1 &&
          image.attr("src").match(/^https?:\/\/.*/) !== null)
      ) {
        if (image.length === 0)
        ...
        render_totals(function (canvas) {
          if (canvas.match(/^data:image\/.*$/)) {
            ...
            $("div.exporter.status").html(
              "<br>No totals information available"
            );
            $("div.canvas.exporter").html("");
          }
        }, override);
        ...
        $("div.exporter.canvas").html(
          "<br>Error processing request. Reload and try again."
        );
    }
    //  process paste uploads
      ...
      setuptools.app.muledump.exporter.paste(
        type,
        text,
        function (data, err, errDesc) {
          if (err !== "success" || data.data.error === true) {
            $("div.exporter.status").html(
              '<br><strong class="negative">Error</strong>: Failed to upload to Paste<br>&nbsp;'
            );
            return;
          }
          ...
          if (setuptools.data.exporter.remote.paste === false) {
            ...
            setuptools.data.exporter.remote.pastesecret =
              data.data.result.secretKey;
            setuptools.app.config.save("Storing Paste API Key");
          }

          setuptools.app.muledump.exporter.saveLink(
            data.data.result.link,
            type
          );
          ...
          status.html(
            '<br>Your Paste Link: <a href="' +
              data.data.result.link +
              '" target="_blank" class="paste link">' +
              data.data.result.link +
              "</a><br>&nbsp;"
          );
        }
      ...
      $("div.exporter.canvas").html(
        '<textarea class="setuptools text exporter w100 scrollbar m0">' +
          text +
          "</textarea>"
      );
    }
  ...
  if (canvas instanceof jQuery) {
    ...
    if (
      typeof type === "string" &&
      ["txt", "csv", "json", "image"].indexOf(type) > -1
    ) {
      procLocal(type);
    } else if (type === "imgur") {
      procRemote("image");
  ...
  //  screenshot binding
  $("div.setuptools.link.canvas.mules").on(
    "click.screenshot.mules",
    function () {
      $("div.exporter.canvas").html("&nbsp;<br>Loading image...");
...
setuptools.app.muledump.exporter.ui.menu = function (track) {
  ...
  var options = [
    ...
    },
    {
      ...
      name: "Text",
      callback: function () {
        setuptools.app.muledump.exporter.ui.main(undefined, "txt");
      },
    },
    {
      class: "exportCSV",
      name: "CSV",
      callback: function () {
        setuptools.app.muledump.exporter.ui.main(undefined, "csv");
      },
    },
    {
      class: "exportJSON",
      name: "JSON",
      callback: function () {
        setuptools.app.muledump.exporter.ui.main(undefined, "json");
      },
    },
    {
      class: "exportImage",
    ...
    },
    {
      ...
      name: "Imgur",
      callback: function () {
        setuptools.app.muledump.exporter.ui.main(undefined, "imgur");
      },
    },
  ];
...
setuptools.app.muledump.drawPortrait = function (c, $c) {
    ...
    for (var x = 0; x < boosts.length; x++) {
      if (boosts[x] !== "0") {
        ...
        if (c.XpBoosted !== "0") {
          ...
          boostHtml +=
            '<div class="cfleft ignore">XpBooster</div><div class="cfright ignore">' +
            length +
            "</div>";
        }
        ...
        if (c.LDTimer !== "0") {
          ...
          boostHtml +=
            '<div class="cfleft ignore">Loot Drop</div><div class="cfright ignore">' +
            length +
            " minutes</div>";
        }
        ...
        if (c.LTTimer !== "0") {
          ...
          boostHtml +=
            '<div class="cfleft ignore">Loot Tier</div><div class="cfright ignore">' +
            length +
            " minutes</div>";
        }
  ...
  chtext
    .append(
      $('<div style="padding-top: 3px;">').html(
        '<span class="' +
          seasonal +
          '">' +
          cl[0] +
  ...
  var chdesc = $('<div class="chdesc">').append(portimg).append(chtext);
  ...
  if (
    ...
    boost &&
    boost.hasClass("hidden") === false
  ) {
    boost.on(
      "mouseenter.muledump.boostTooltip",
      [boost, boostHtml],
      function (e) {
        setuptools.lightbox.tooltip(e.data[0].parent().parent(), e.data[1], {
...
setuptools.app.muledump.skinWardrobe = function (guid, tex1, tex2) {
  ...
  if (typeof tex1 !== "number" || !textures[tex1])
    tex1 =
      typeof setuptools.data.muledump.wardrobe.accounts[guid] === "object" &&
      typeof setuptools.data.muledump.wardrobe.accounts[guid].clothes.large ===
        "number" &&
      textures[setuptools.data.muledump.wardrobe.accounts[guid].clothes.large]
        ? setuptools.data.muledump.wardrobe.accounts[guid].clothes.large
        : undefined;
  if (typeof tex2 !== "number" || !textures[tex2])
    tex2 =
      typeof setuptools.data.muledump.wardrobe.accounts[guid] === "object" &&
      typeof setuptools.data.muledump.wardrobe.accounts[guid].clothes.small ===
        "number" &&
      textures[setuptools.data.muledump.wardrobe.accounts[guid].clothes.small]
        ? setuptools.data.muledump.wardrobe.accounts[guid].clothes.small
        : undefined;
  var large;
  var small;
  ...
  var ownedSkins = $.extend(
    true,
    [],
    mules[guid].data.query.results.Chars.OwnedSkins
  );
  ...
  if (typeof setuptools.tmp.skins[guid] !== "object") {
    ...
    Object.filter(classes, function (classid, data) {
      if (!Array.isArray(setuptools.tmp.skins[guid][classid]))
        setuptools.tmp.skins[guid][classid] = {
          total: 0,
          owned: 0,
          skins: [],
        };
    ...
    Object.filter(skins, function (skinid, data) {
      ...
      if (
        ...
        data[0].match(
          new RegExp(
            "^(?:.*? Set(?: Skin)?|Christmas (?:" + classlist.join("|") + "))$"
          )
        ) !== null
      )
        return;
  ...
  Object.filter(setuptools.tmp.skins[guid], function (classid, data) {
    ...
    var html =
      ' \
            <div class="flex-container ownedSkins ' +
      classData[0] +
      '"> \
    ...
    data.skins.filter(function (skinid) {
      ...
      var $s = $(
        '<img class="ownedSkins" style="padding: 5px; margin: 2px 3px 3px 2px; border: 1px solid #222; overflow: hidden;" data-skinid="' +
          skinid +
          '">'
      );
    ...
    html = html.replace(
      "[[" + classid + "_count]]",
      setuptools.tmp.skins[guid][classid].owned +
        " of " +
        setuptools.tmp.skins[guid][classid].total +
        " (" +
        (
          (setuptools.tmp.skins[guid][classid].owned /
            setuptools.tmp.skins[guid][classid].total) *
  ...
  setuptools.lightbox.build(
    ...
    ' \
      ...
      count.total +
      " skins (" +
      ((count.owned / count.total) * 100).toFixed(0) +
      '%)</li>\
            </ul>\
            <div class="flex-container noFlexAutoWidth" style="padding-right: 10px;">\
                <div class="cloth large item transbackground noselect mr5" style="background-position: -' +
      large[3] +
      "px -" +
      large[4] +
      'px;"></div>\
                <div class="cloth small item transbackground noselect mr5" style="background-position: -' +
      small[3] +
      "px -" +
      small[4] +
      'px;"></div>\
  ...
  Object.keys(setuptools.tmp.skins[guid])
    .sort(function (a, b) {
    ...
    .filter(function (classid) {
      ...
      setuptools.lightbox.build(
        ...
        ' \
          ...
          data[0] +
          '">\
                <img src="#" class="ownedSkins skinmenu" style="margin: 5px 7px; padding: 5px; overflow: hidden;" data-classid="' +
          classid +
          '">\
            </a>\
        '
  ...
  Object.keys(classes).filter(function (classid) {
    ...
    setuptools.app.muledump.tooltip(
      $s,
      "ownedSkinsTooltip skinmenu",
      "<div>" + classes[classid][0] + "</div>",
      100
    );
  });
  ...
  $("div.cloth.item").on("click.wardrobe.clothes", function (e) {
    ...
    Object.keys(textures).filter(function (tex) {
      ...
      if (
        typeof setuptools.data.muledump.wardrobe.accounts[guid] === "object" &&
        setuptools.data.muledump.wardrobe.accounts[guid].clothes[clothkey] ===
          tex
      )
        enabled = "enabled";
      ...
      setuptools.lightbox.build(
        ...
        ' \
                <div data-tex="' +
          tex +
          '" class="cloth selector item transbackground noselect mr5 ' +
          enabled +
    ...
    $("div.cloth.selector.item")
      ...
      .on("click.wardrobe.clothselector", function (e) {
        ...
        if (
          typeof setuptools.data.muledump.wardrobe.accounts[guid] ===
          "undefined"
        )
          setuptools.data.muledump.wardrobe.accounts[guid] = $.extend(
            true,
            {},
            setuptools.objects.wardrobeConfig
          );
        ...
        setuptools.data.muledump.wardrobe.accounts[guid] = $.extend(
          true,
          setuptools.objects.wardrobeConfig,
          setuptools.data.muledump.wardrobe.accounts[guid]
        );
        ...
        if ($(this).hasClass("enabled")) {
          ...
          delete setuptools.data.muledump.wardrobe.accounts[guid].clothes[
            clothkey
          ];
...
setuptools.app.muledump.ocl.manager = function (profile, advanced, hardreset) {
  ...
  if (
    hardreset === true &&
    !setuptools.copy.data.muledump.ocl.configSets.settings[profile]
  )
    hardreset = undefined;
  if (typeof advanced === "undefined") advanced = false;
  if (typeof profile !== "string")
    profile = setuptools.data.muledump.ocl.configSets.active;
  var data = setuptools.app.muledump.ocl.get(profile);
  if (typeof data !== "object")
    data = setuptools.app.muledump.ocl.get(setuptools.config.oclDefault);
  if (data === false)
    setuptools.lightbox.error("Failed to load any OCL profile");
  ...
  setuptools.lightbox.build(
    ...
    ' \
            ...
            <div class="flex-container noFlexAutoWidth" style="justify-content: flex-start;">\
                ...
                <div class="flex-container noFlexAutoWidth">\
                    <select name="oclEnabled" class="setuptools oclEnabled" style="width: auto; margin-left: 15px;">\
                        <option value="0" ' +
      (setuptools.data.config.mulelogin === 0 ? "selected" : "") +
      '>No</option>\
                        <option value="1" ' +
      (setuptools.data.config.mulelogin === 1 ? "selected" : "") +
      '>Yes</option>\
                    </select>\
                </div>\
                ...
                <div class="flex-container noFlexAutoWidth">\
                    <select name="oclCopyLinks" class="setuptools oclCopyLinks" style="width: auto; margin-left: 15px;">\
                        <option value="0" ' +
      (setuptools.data.config.muleloginCopyLinks === 0 ? "selected" : "") +
      '>No</option>\
                        <option value="1" ' +
      (setuptools.data.config.muleloginCopyLinks === 1 ? "selected" : "") +
      '>Yes</option>\
                    </select>\
                </div>\
            </div>\
        ...
        <div class="ocl manager">\
            <div class="ocl configuration">\
                <div data-ocl="name">\
                    ...
                    <div>\
                        <input name="name" class="setuptools input ocl" value="' +
      profile +
      '" placeholder="Type profile name here" autocomplete="off" autocorrect="off" autocapitalize="off" spellcheck="false">\
                    </div>\
                </div>\
                ...
                <div data-ocl="mode">\
                    ...
                    <div>\
                        <select name="mode" class="setuptools menu ocl">\
                            <option value="0" ' +
      (data.params.mode === "browser" ? "selected" : "") +
      '>Browser</option>\
                            <option value="1" ' +
      (data.params.mode === "flash" ? "selected" : "") +
      '>Flash Projector</option>\
                            <option value="2" ' +
      (data.params.mode === "exalt" ? "selected" : "") +
      '>Exalt</option>\
                        </select>\
                    </div>\
                </div>\
                <div data-ocl="path">\
                    ...
                    <div>\
                        <input name="path" class="setuptools input ocl" value="' +
      data.params.path +
      '" placeholder="Browser URL, Flash Projector or Exalt exe path here" autocomplete="off" autocorrect="off" autocapitalize="off" spellcheck="false">\
                    </div>\
                </div>\
                <div data-ocl="client" style="' +
      (["browser", "exalt"].indexOf(data.params.mode) > -1
        ? "display: none;"
        : "") +
      '">\
                    <div>Game Client <strong class="help ml5">?</strong></div>\
                    <div>\
                        <input name="client" class="setuptools input ocl" value="' +
      (data.params.client || setuptools.config.oclClient) +
      '" placeholder="Type path to game client swf" autocomplete="off" autocorrect="off" autocapitalize="off" spellcheck="false">\
                    </div>\
                </div>\
                <div data-ocl="title" style="' +
      (["browser"].indexOf(data.params.mode) > -1 || advanced === false
        ? "display: none;"
        : "") +
      '">\
                    <div>Custom Window Title <strong class="help ml5">?</strong></div>\
                    <div>\
                        <input name="title" class="setuptools input ocl" value="' +
      (data.params.title || "") +
      '" placeholder="Optionally set game client window title" autocomplete="off" autocorrect="off" autocapitalize="off" spellcheck="false">\
                    </div>\
                </div>\
                <div data-ocl="ign" style="' +
      (["browser"].indexOf(data.params.mode) > -1 || advanced === false
        ? "display: none;"
        : "") +
      '">\
      ...
      '>Yes</option>\
                ...
                <div data-ocl="paths" style="' +
      (["exalt"].indexOf(data.params.mode) > -1 || advanced === false
        ? "display: none;"
        : "") +
      '">\
                    <div>Update Paths <strong class="help ml5">?</strong></div>\
                    <div>\
                        <input name="paths" class="setuptools input ocl" value="' +
      data.params.paths +
      '" placeholder="" autocomplete="off" autocorrect="off" autocapitalize="off" spellcheck="false">\
                    </div>\
                </div>\
                <div style="justify-content: flex-end;">\
                    <div class="setuptools link ocl advanced noclose menuStyle buttonStyle bright" style="width: 100px;" data-state="' +
      (advanced === false ? "false" : "true") +
      '">' +
      (advanced === false ? "Show" : "Hide") +
      ' Advanced</div>\
                    <div class="setuptools link ocl delete noclose menuStyle buttonStyle notice " style="width: 75px;">Delete</div>\
                    <div class="setuptools link ocl reset noclose menuStyle buttonStyle negative" style="width: ' +
      (hardreset === true ? "100" : "75") +
      'px;">' +
      (hardreset === true ? "Factory " : "") +
      'Reset</div>\
                    <div class="setuptools link ocl save noclose menuStyle buttonStyle positive mr0" style="width: 75px;">Save</div>\
                </div>\
            </div>\
            <div class="ocl profiles ' +
      (Object.keys(setuptools.data.muledump.ocl.configSets.settings).length > 8
        ? "scrollbar overflowyScroll"
        : "") +
      '">\
  ...
  //  create profile list
  Object.keys(setuptools.data.muledump.ocl.configSets.settings).forEach(
    function (profile) {
  ...
  var help = {
    ...
    path: [
      "<code>Exalt:</code> Path to Exalt exe (e.g. C:UsersYourAccountDocumentsRealmOfTheMadGodProductionRotMG Exalt.exe)",
      "<code>Flash Projector:</code> Path to Flash Projector exe (e.g. C:\\flashplayer_16_sa.exe)",
      "<code>Browser:</code> URL to launch the game",
    ],
    client: "URL or local path to game SWF file",
    admin: [
      "Config setting <code>adminparams</code> must be set to <code>true</code> in <code>/lib/mulelogin.au3</code>",
      "Run AutoIt with administrative privileges (may cause Windows UAC prompt)",
    ],
    paths: "Hostname to restrict updates of RotMG.sol files",
    ign: "Whether or not to include account guid/ign",
    title: [
      "Type in <code>false</code> to disable renaming the window entirely.",
      "Custom title for game client window.",
    ],
  };
    ...
    if (dom.length === 0) {
      ...
      pieces.forEach(function (piece) {
        $(
          '<div data-help="ocl ' +
            param +
            '" style="display:none;" class="flex-container help">'
        )
          .html(
  ...
  $("div.setuptools.link.ocl.reset").on("click.ocl.reset", function () {
    if (hardreset === true) {
      $.extend(
        true,
        setuptools.data.muledump.ocl.configSets.settings[profile],
        setuptools.copy.data.muledump.ocl.configSets.settings[profile]
      );
  ...
  $("div.setuptools.link.ocl.delete").on("click.ocl.delete", function () {
    if (
      setuptools.tmp.lightboxStatus &&
      setuptools.tmp.lightboxStatus.indexOf("ocl-delete") > -1
    )
      return;
    ...
    if (setuptools.data.muledump.ocl.configSets.active === profile)
      setuptools.data.muledump.ocl.configSets.active =
    ...
    setuptools.lightbox.status(
      $("div.setuptools.link.ocl.delete"),
      "Ok!",
      "ocl-delete"
    );
  });
  ...
  //  install help button
  $("div.setuptools.link.ocl.install").on(
    "click.ocl.install",
    setuptools.app.muledump.ocl.install
  );
  ...
  $("div.setuptools.link.ocl.save").on("click.ocl.save", function () {
    if (
      setuptools.tmp.lightboxStatus &&
      setuptools.tmp.lightboxStatus.indexOf("ocl-save") > -1
    )
      return;
    ...
    setuptools.app.muledump.ocl.manager(profile, advanced);
    setuptools.lightbox.status(
      $("div.ocl.configuration div.setuptools.link.save"),
      "Ok!",
      "ocl-save"
    );
  });
  ...
  $("div[data-oclProfile]").on("click.ocl.chooseProfile", function () {
    setuptools.app.muledump.ocl.manager(
      $(this).attr("data-oclProfile"),
      advanced
    );
  });

  //  prepare fresh profile for create profile button
  $("div.setuptools.link.ocl.createProfile").on(
    "click.ocl.createProfile",
    function () {
  ...
  $("div.setuptools.link.ocl.advanced").on("click.ocl.advanced", function () {
    ...
    $('div[data-help="ocl admin"]').slideUp(function () {
      this.remove();
    });
    $('div[data-help="ocl paths"]').slideUp(function () {
      this.remove();
    });
    ...
    if (mode === 1 || mode === 2)
      $('div[data-help="ocl title"]').slideUp(function () {
        this.remove();
      });
    if (mode === 1 || mode === 2)
      $('div[data-help="ocl ign"]').slideUp(function () {
        this.remove();
      });
    if (mode === 1 || mode === 2)
      $('div[data-ocl="title"] strong.help').html("?");
    if (mode === 1 || mode === 2)
      $('div[data-ocl="ign"] strong.help').html("?");
  });

  //  ocl enable menu
  $('select[name="oclEnabled"]').on("change.ocl.enabled", function () {
    setuptools.data.config.mulelogin =
      setuptools.data.config.mulelogin === 0 ? 1 : 0;
  ...
  $('select[name="oclCopyLinks"]').on("change.ocl.copylinks", function () {
    setuptools.data.config.muleloginCopyLinks =
      setuptools.data.config.muleloginCopyLinks === 0 ? 1 : 0;
...
setuptools.app.muledump.ocl.install = function () {
  setuptools.lightbox.build(
    ...
    ' \
        <div id="drawhelpBox">\
            Welcome to the One Click Login installation guide! Check out the <a href="' +
      setuptools.config.oneclickHelp +
      '" target="_blank">One Click Login wiki</a> for more comprehensive information. \
            \
      ...
      (setuptools.state.hosted === false
        ? " \
            ...
            "
        : ' \
                <br><strong>Non-Exalt Users</strong>: The latest version of One Click Login is available on Github: <a href="' +
          setuptools.config.url +
          '/lib/mulelogin.au3" target="_blank" download="mulelogin.au3">' +
          setuptools.config.url +
          '/lib/mulelogin.au3</a> \
                <br>\
                <br><strong>Exalt Users</strong>: The latest version of One Click Login is available on Github: <a href="' +
          setuptools.config.url +
          '/lib/mulelogin.au3" target="_blank" download="mulelogin.au3">' +
          setuptools.config.url +
          "/lib/mulelogin.au3</a> \
  ...
  setuptools.lightbox.settitle(
    "ocl-install",
    "One Click Login Installation Guide"
  );
  setuptools.lightbox.drawhelp(
    "ocl-install",
    setuptools.config.oneclickHelp,
    "OCL Installation Help"
  );
  setuptools.lightbox.goback(
    "ocl-install",
    setuptools.app.muledump.ocl.manager
  );
...
setuptools.app.muledump.ocl.save = function (profile, data) {
  ...
  if (typeof data.hidden !== "undefined")
    profiles[profile].hidden = data.hidden;
  delete data.default;
  delete data.name;
  delete data.hidden;
...
setuptools.app.muledump.ocl.router = function (guid) {
  if (setuptools.state.loaded === false || !guid)
    return setuptools.app.muledump.ocl.get();
  if (!setuptools.data.accounts.accounts[guid]) return false;
  return setuptools.app.muledump.ocl.get(
    setuptools.data.accounts.accounts[guid].oclProfile || undefined
  );
};
...
setuptools.app.muledump.ocl.params = function (profile) {
  ...
  if (ocl_params.paths != undefined && ocl_params.paths.length === 0)
    delete ocl_params.paths;

  var params = "";
  Object.keys(ocl_params).forEach(function (param) {
    if (typeof ocl_params[param] === "string")
      ocl_params[param] = ocl_params[param].replace(" ", "%20");
    params +=
      param + "=" + ocl_params[param] + setuptools.config.oclParamSeparator;
  });
...
setuptools.app.muledump.ocl.regenerate = function (guids) {
  ...
  if (!guids)
    guids = Object.keys(mules).filter(function (guid) {
  ...
  guids.forEach(function (guid) {
    ...
    if (setuptools.data.config.mulelogin === 1) {
      self.show();
      if (setuptools.data.config.muleloginCopyLinks === 1)
        self.attr("href", setuptools.app.muledump.ocl.mulelink(guid));
...
setuptools.app.muledump.ocl.accountMenu = function (position, data) {
  ...
  //  build menu options
  ["Default"]
    .concat(Object.keys(setuptools.data.muledump.ocl.configSets.settings))
    .forEach(function (profile) {
      options.push({
        class: "oclProfile",
        name:
          (data.oclProfile === profile ||
          (data.oclProfile === false && profile === "Default")
            ? "* "
            : "") + profile,
        callback: function (profile) {
          data.oclProfile =
            setuptools.app.muledump.ocl.get(profile) === false
              ? false
              : profile;
          setuptools.app.techlog(
            "Muledump/OCL account updated: " + data.username + " = " + profile,
            "force"
          );
        },
        callbackArg: profile,
...
setuptools.app.muledump.ocl.mulelink = function (guid) {
  if (setuptools.state.loaded === true) {
    ...
    if (data.params.ign === false || typeof guid !== "string")
      delete data.params.ign;
    if (data.params.ign === true)
      data.params.ign = setuptools.app.muledump.getName(guid);
    ...
    if (
      ...
      ocl_params = ocl_params.replace(
        /www.realmofthemadgod.com/g,
        "test.realmofthemadgod.com"
      );
  }
  ...
  return (
    ...
    sodium.to_hex(guid) +
    "-" +
    sodium.to_hex(window.accounts[guid]) +
    (setuptools.state.loaded === true ? "-" + ocl_params : "")
  );
};
...
setuptools.app.muledump.ocl.record = function (guid) {
  ...
  var path =
    profile.params.mode === "browser"
      ? profile.params.path
      : profile.params.client;
  setuptools.app.ga("send", "event", {
    eventCategory: "ocl",
...
setuptools.app.muledump.item = function (id, type, num, ext) {
  ...
  if (
    setuptools.data.config.equipSilhouettes === false ||
    !Array.isArray(ext) ||
    ext[0] !== "equipment" ||
    id > -1
  ) {
    ...
    if (setuptools.data.config.tooltipItems > 0)
    ...
    Object.filter(itemsSlotTypeMap, function (key, value) {
      if (classes[ext[2]] && value.slotType === classes[ext[2]][4][ext[1]])
        slotmapPos = value.sheet;
    });
    $r.addClass("silhouette25p");
    $r.css(
      "background-position",
      "-" + slotmapPos[0] + "px -" + slotmapPos[1] + "px"
    );
  }
  return $r;
};
...
setuptools.app.muledump.item_listing = function (arr, classname, classid) {
  ...
  for (var i = 0; i < arr.length; i++) {
    ...
    setuptools.app.muledump
      .item(itemid, undefined, undefined, [classname, i, classid])
      .appendTo($r);
  }
