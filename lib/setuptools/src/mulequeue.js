...
setuptools.app.mulequeue.main = async function () {
  //  some config sanity
  if (
    $.isNumeric(setuptools.data.config.mqConcurrent) === false ||
    setuptools.data.config.mqConcurrent > 30
  )
    setuptools.data.config.mqConcurrent = 1;

  //  check if we're already rate limited
  setuptools.app.mulequeue.state.rateLimitExpiration =
    await setuptools.storage.read("ratelimitexpiration");
  if (!setuptools.app.mulequeue.state.rateLimitExpiration)
    setuptools.app.mulequeue.state.rateLimitExpiration = Date.now();
  if (setuptools.app.mulequeue.state.rateLimitExpiration > new Date())
    setuptools.app.mulequeue.task.rateLimit(true);
  ...
  setInterval(
    setuptools.app.mulequeue.task.bgHealth,
...
setuptools.app.mulequeue.task.bgHealth = function () {
  ...
  if (
    ...
    new Date() >
      Number(setuptools.app.mulequeue.state.bgPing) +
        setuptools.data.config.mqBGTimeout * 1000
  ) {
...
setuptools.app.mulequeue.task.background = function () {
  ...
  if (
    ...
    if (
      setuptools.app.mulequeue.queue.length >
      setuptools.app.mulequeue.state.queuePeak
    )
      setuptools.app.mulequeue.state.queuePeak =
        setuptools.app.mulequeue.queue.length;

    //  if this is the top of a cycle, check if the minimum cycle delay has passed
    var lastRunDifference =
      (new Date() - setuptools.app.mulequeue.state.lastTaskFinished) / 1000;
    if (
      ...
      window.techlog(
        "MuleQueue/TaskWait - " +
          Math.ceil(window.accountLoadDelay - lastRunDifference) +
...
setuptools.app.mulequeue.task.saveConfig = async function (returnData) {
  var Config = {
    date: Date.now(),
    state: {
  ...
  await setuptools.storage.write("mulequeue:config", Config);
};

//  load mulequeue history and cache data and import it if necessary
setuptools.app.mulequeue.task.loadConfig = async function () {
  //  import any mulequeue cache data
  var ConfigJson = await setuptools.storage.read("mulequeue:config");
  var Config;
  try {
  ...
  if (typeof Config === "object" && Config !== null) {
    ...
    if (
      Date.now() - Config.date < setuptools.config.mqStaleCache &&
      ...
      setuptools.app.mulequeue.state.lastTaskFinished = new Date(
        setuptools.app.mulequeue.state.lastTaskFinished
      );
  ...
  var HistoryJson = await setuptools.storage.read("mulequeue:history");
  var History;
  try {
...
setuptools.app.mulequeue.task.rateLimitCancel = async function () {
  ...
  for (var task in setuptools.app.mulequeue.tasks) {
    if (setuptools.app.mulequeue.tasks.hasOwnProperty(task)) {
      ...
      $(
        'div.queuetask[data-listPos="' +
          setuptools.app.mulequeue.queue.indexOf(task) +
          '"] > div:first-child'
      )
...
setuptools.app.mulequeue.task.rateLimit = async function (soft) {
  function RateLimitTimer() {
    ...
    if (RateLimitRemaining <= 0) {
      ...
      $("#stickynotice").html(
        ' \
                <div class="flex-container">\
                    <div>Warning: Your account is <a href="' +
          setuptools.config.ratelimitHelp +
          '" class="rateLimited nopulse orange" target="_blank">rate limited</a> by Deca. \
                    You must wait ' +
          RateLimitTime +
          ' before you can reload your account.</div> \
      ...
      $("div.mulequeue.progress > div.percentage").html(
        ' \
                <div class="flex-container">\
                    <div>You are <a href="' +
          setuptools.config.ratelimitHelp +
          '" class="rateLimited nopulse" target="_blank">rate limited</a> and must wait ' +
          RateLimitTime +
      ...
      $("div.muledump.resetratelimit")
        .off("click.muledump.resetratelimit")
        .on("click.muledump.resetratelimit", function () {
          setuptools.app.mulequeue.task.rateLimitCancel();
        });
    }
  }
  ...
  if (soft !== true) {
    setuptools.app.mulequeue.state.rateLimitExpiration =
      Date.now() + setuptools.config.mqRateLimitExpiration;
  ...
  if (setuptools.app.mulequeue.queue.length > 0) {
    for (
      var index = setuptools.app.mulequeue.state.busy - 1;
      index >= 0;
      index--
    ) {
      setuptools.app.mulequeue.tasks[
        setuptools.app.mulequeue.queue[index]
      ].state = "queue";
  ...
  if (setuptools.data.config.animations === 1)
    $("#stickynotice").addClass("rateLimited nopulse");

  //  start the rate limit timer
  setuptools.app.mulequeue.state.rateLimitTimer = setInterval(
    RateLimitTimer,
    1000
  );
};
// handeling of the 5 min limit
setuptools.app.mulequeue.task.hardRateLimit = async function (soft) {
  function RateLimitTimer() {
    var RateLimitRemaining = (
      (setuptools.app.mulequeue.state.rateLimitExpiration - Date.now()) /
      1000 /
...
setuptools.app.mulequeue.task.guidsCheck = function (guids) {
  ...
  if (Array.isArray(guids) === false)
    setuptools.lightbox.error(
      'Data type for argument "guids" not valid.',
      1000
    );
  return guids;
};
...
setuptools.app.mulequeue.task.queue = function (guids, config) {
  ...
  if (typeof config === "function") {
    callback = config;
    config = {};
  } else if (typeof config !== "object" || Array.isArray(config) === true)
    config = {};
  ...
  for (var i = 0; i < guids.length; i++) {
    ...
    if (typeof setuptools.app.mulequeue.tasks[guids[i]] === "object") {
      ...
      if (setuptools.app.mulequeue.tasks[guids[i]].state === "running")
        continue;
    ...
    config.reloader = $(
      'div.mule[data-guid="' + guids[i] + '"] > div.button.reloader'
    );
    if (setuptools.data.config.animations === 1)
      config.reloader
        .empty()
...
setuptools.app.mulequeue.task.refresh = function (guids) {
  ...
  for (var x = 0; x < guids.length; x++)
    mules[guids[x]].query(false, true, true);
};
...
setuptools.app.mulequeue.task.start = function (guid, callback) {
  ...
  if (typeof setuptools.app.mulequeue.tasks[guid] === "undefined")
    setuptools.app.mulequeue.task.reload(guid);

  //  check if a task exists and make sure it is in the proper state
  if (
    typeof setuptools.app.mulequeue.tasks[guid] === "object" &&
    setuptools.app.mulequeue.tasks[guid].state !== "queue"
  )
    return;
  ...
  if (setuptools.data.config.animations === 1)
    setuptools.app.mulequeue.tasks[guid].reloader
      .empty()
      .append(setuptools.app.mulequeue.spinner.running("running"))
      .attr("title", "Mule data request running...");
  $('div.queuetask[data-listPos="' + taskIndex + '"] > div:first-child')
    .html(setuptools.app.mulequeue.spinner.running("running", true))
    .attr("title", "Mule data request running...");

  //  create the promise to run the task
  setuptools.app.mulequeue.tasks[guid].promise = setuptools.app.mulequeue.task
    .createPromise(guid)
    .then(function (response) {
      ...
      if (response.state === "finished" && task.state === "running") {
        ...
        setuptools.app.mulequeue.state.timePerTaskAverage = Math.round(
          (setuptools.app.mulequeue.state.timePerTaskAverage + task.runTime) / 2
        );
        ...
        if (response.status === "error") {
          if (setuptools.data.config.animations === 1)
            task.reloader
              .empty()
              .append(setuptools.app.mulequeue.spinner.running("error"))
              .attr("title", "Error loading account: " + response.errorMessage);
        } else {
          /*  now redundant  ///
                //  remove any errors
                var errors = $('#errors');
          ...
          task.reloader
            ...
            .attr(
              ...
              "last updated: " +
                new Date(
                  mules[response.guid].data.query.created
                ).toLocaleString()
            );
        }
        ...
        window.techlog(
          "MuleQueue/TaskFinish " +
            ...
            " - waited " +
            Math.floor((task.finishTime - task.creation) / 1000) +
            " seconds" +
            " - remaining tasks " +
            (setuptools.app.mulequeue.queue.length - 1),
          "force"
        );
        ...
        setuptools.app.mulequeue.task.history({
          ...
          ign:
            ...
            setuptools.app.config.determineFormat(setuptools.data.accounts) ===
              1
              ? setuptools.data.accounts.accounts[response.guid] &&
                setuptools.data.accounts.accounts[response.guid].ign
              : undefined,
        ...
        if (
          ...
          typeof setuptools.lightbox.menu.paginate.state.QueueHistory ===
            "object"
        )
        ...
        setuptools.app.mulequeue.queue.splice(
          setuptools.app.mulequeue.queue.indexOf(response.guid),
          1
        );
        ...
        if (setuptools.app.mulequeue.state.busy === 0)
          setuptools.app.mulequeue.state.active = false;
        ...
        if (setuptools.app.mulequeue.queue.length === 0) {
          ...
          setuptools.app.mulequeue.state.queuePeak = 0;
          if (
            typeof setuptools.lightbox.active["mulequeue-manager"] ===
              "object" &&
            typeof setuptools.app.mulequeue.queuemanager === "object"
          )
            setuptools.app.mulequeue.queuemanager.Modifiers.callbacks.update();
...
setuptools.app.mulequeue.task.history = async function (record) {
  if (Array.isArray(setuptools.app.mulequeue.history.data) === false)
    setuptools.app.mulequeue.history.data = [];
  if (typeof setuptools.app.mulequeue.history.stats === "undefined")
    setuptools.app.mulequeue.history.stats = {
      ok: 0,
      error: 0,
      cors: 0,
      accountInUse: 0,
  ...
  setuptools.app.mulequeue.history.data.splice(
  ...
  await setuptools.storage.write(
    "mulequeue:history",
    setuptools.app.mulequeue.history
  );
};

//  pause tasks and update the ui
setuptools.app.mulequeue.task.pause = function (callback) {
  if (setuptools.app.mulequeue.state.rateLimited === false)
    $("#mulequeue").removeClass("running").addClass("paused");
  ...
  for (var i = 0; i < pauseList.length; i++) {
    ...
    if (task.state === "queue") {
      if (setuptools.data.config.animations === 1)
        task.reloader
          .empty()
          .append(setuptools.app.mulequeue.spinner.running("pause"))
          .attr("title", "Task paused in MuleQueue");
      $('div.queuetask[data-listPos="' + i + '"] > div:first-child')
        .html(setuptools.app.mulequeue.spinner.running("pause", true))
        .attr("title", "Task paused in MuleQueue");
    }
  }
...
setuptools.app.mulequeue.task.resume = function (guids, callback) {
  ...
  if (guids.length === 0) {
    if (setuptools.app.mulequeue.state.rateLimited === false)
      $("#mulequeue").removeClass("paused").addClass("running");
    setuptools.app.mulequeue.state.paused = false;
  ...
  for (var i = 0; i < pauseList.length; i++) {
    ...
    if (task.state === "queue") {
      if (setuptools.data.config.animations === 1)
        task.reloader
          .empty()
          .append(setuptools.app.mulequeue.spinner.running("queue"))
          .attr("title", "Task is waiting in MuleQueue");
      $('div.queuetask[data-listPos="' + i + '"] > div:first-child')
        .html(setuptools.app.mulequeue.spinner.running("queue", true))
        .attr("title", "Task is waiting in MuleQueue");
    }
  }
  ...
  setuptools.app.mulequeue.state.running = true;
  setuptools.app.mulequeue.state.lastTaskFinished =
    new Date() - window.accountLoadDelay * 1000;
  if (typeof callback === "function") callback();
...
setuptools.app.mulequeue.task.cancel = function (guids, callback) {
  ...
  window.techlog(
    "MuleQueue/TaskCancel clearing " + guids.length + " tasks",
    "force"
  );
  ...
  for (var i = guids.length - 1; i >= 0; i--) {
    ...
    if (["queue", "error"].indexOf(task.state) > -1) {
      if (mules[guid] instanceof Mule && mules[guid].loginOnly !== true)
        setuptools.app.mulequeue.tasks[guid].reloader
          .empty()
          .text("\u21bb")
          .attr(
      ...
      setuptools.app.mulequeue.queue.splice(
        setuptools.app.mulequeue.queue.indexOf(guid),
        1
      );
      setuptools.app.mulequeue.queue
        .filter(function (item) {
...
setuptools.app.mulequeue.task.reorder = function (guids, position) {
  if (typeof guids === "undefined")
    setuptools.lightbox.error('Argument "guids" is required.', 1000);
  ...
  for (var i = 0; i < guids.length; i++) {
    ...
    if (taskIndex !== taskPosition) {
      window.techlog(
        "MuleQueue/Start pushing " + guids[i] + " to position " + taskPosition
      );
...
setuptools.app.mulequeue.menu = function () {
  ...
  //  set our options
  var YesIAmSplittingHairs =
    setuptools.app.mulequeue.queue.length === 1 ? "" : "s";
  var options = [
    {
      option: "hover",
      action: "close",
  ...
  new setuptools.lightbox.menu.context.create(
    "mulequeue-menu",
    false,
    MuleQueueButton,
    options
...
setuptools.app.mulequeue.ui.drawButtons = function () {
  ...
  html +=
    ' \
        <div class="fleft flex-container nohover mulequeue queueCount">' +
    setuptools.app.mulequeue.queue.length +
    ' Accounts in Queue</div>\
    ...
    " \
        " +
    (setuptools.app.mulequeue.state.rateLimited === true
      ? '<div class="fleft flex-container nohover mulequeue setuptools link resetratelimit noclose menuStyle negative buttonStyle" title="Reset Rate Limiter"><div style="font-weight: normal; overflow: hidden;">&#9735;</div></div>'
      : "") +
    " \
    ";
...
setuptools.app.mulequeue.ui.manager = function (guid) {
  ...
  function ListQueue(dummy, page) {
    ...
    if (page > setuptools.lightbox.menu.paginate.state.QueueList.lastPage)
      page = setuptools.lightbox.menu.paginate.state.QueueList.lastPage;
    ...
    var maxIndex =
      setuptools.data.config.accountsPerPage * page +
  ...
  function createRow(listPos) {
    ...
    var TimePerTask = Math.round(
      setuptools.app.mulequeue.state.timePerTaskAverage / 1000
    );
    var LastRunDiff =
      setuptools.app.mulequeue.state.lastTaskFinished === 0
        ? 0
        : Math.ceil(
            (Date.now() - setuptools.app.mulequeue.state.lastTaskFinished) /
              1000
          );
    ...
    var WaitTime = {
      estimation:
        (listPos + 1) * (AccountLoadDelay + TimePerTask) - LastRunDiff,
    };
    ...
    if (setuptools.app.mulequeue.state.rateLimited === true)
      WaitTime.estimation =
        ...
        (listPos > 0 ? AccountLoadDelay : 0);
    WaitTime.length = setuptools.app.mulequeue.ui.secondsToHms(
      WaitTime.estimation
    );

    if (listPos === setuptools.app.mulequeue.queue.length - 1)
      setuptools.app.mulequeue.state.estimateFinishTime = WaitTime.estimation;

    return (
      ' \
            <div class="flex-container queuetask noselect" style="justify-content: flex-start;" data-listPos="' +
      listPos +
      '" data-guid="' +
      QueueList[listPos] +
      '"> \
                <div class="flex-container textCenter" style="justify-content: flex-start;" title="Queue Position: ' +
      (listPos + 1) +
      '">' +
      (setuptools.data.config.animations === -1
        ? '<div class="menuStyle buttonStyle neutral textCenter mr0">' +
          (listPos + 1) +
          "</div>"
      ...
      typeof setuptools.data.accounts.accounts[QueueList[listPos]].ign ===
        "string"
        ? setuptools.data.accounts.accounts[QueueList[listPos]].ign
        : QueueList[listPos]) +
      '</div> \
  ...
  function QueueManagerContext() {
    ...
    $("div.setuptools.link.history")
      .unbind("click")
      .click(function () {
        setuptools.app.mulequeue.ui.history();
      });

    $("div.setuptools.link.nameswap")
      .unbind("click")
      .click(function () {
    ...
    $("div.setuptools.link.resetratelimit")
      .off("click.setuptools.resetratelimit")
      .on("click.setuptools.resetratelimit", function (e) {
        setuptools.app.mulequeue.task.rateLimitCancel();
      });
  }
  ...
  function QueueManagerUpdate(page) {
    if (
      ...
      page = setuptools.lightbox.menu.paginate.state.QueueList.currentPage;
    if (["string", "number", "undefined"].indexOf(typeof page) === -1) return;

    //  reset page state
    setuptools.app.mulequeue.queuemanager =
    ...
    if (typeof page === "undefined")
      setuptools.lightbox.menu.paginate.findPage(0, "QueueList");
    if (typeof page === "string")
      setuptools.lightbox.menu.paginate.findPage(page, "QueueList");
    if (typeof page === "number")
      setuptools.lightbox.menu.paginate.state.QueueList.currentPage = page;

    //  validate lastPage/currentPage boundary
    var newLastPage = Math.ceil(
      QueueManagerPaginate.PageList.length /
        setuptools.data.config.accountsPerPage
    );
    if (newLastPage > 0) newLastPage--;
    if (QueueManagerPaginate.currentPage > newLastPage)
      setuptools.lightbox.menu.paginate.findPage(
        QueueManagerPaginate.PageList.length - 1,
        "QueueList"
      );
    ...
    $("div.QueueList.list").html(
      ' \
            <div class="fleft cboth mb5 w100">' +
  ...
  setuptools.lightbox.build(
    "mulequeue-manager",
    ' \
        <div class="mulequeue progress w100 flex-container ' +
      AReduced +
      '">\
  ...
  setuptools.lightbox.drawhelp(
    "mulequeue-manager",
    "docs/setuptools/help/mulequeue/manager",
    "MuleQueue Help"
  );
  setuptools.lightbox.display("mulequeue-manager", {
    variant: "fl-MuleQueueManager",
  });
...
setuptools.app.mulequeue.ui.history = function () {
  ...
  var ListRecord = function (dummy, page) {
    if (typeof page !== "number") page = 0;
    var QueueHistory =
      setuptools.lightbox.menu.paginate.state.QueueHistory.PageList;

    if (page > setuptools.lightbox.menu.paginate.state.QueueHistory.lastPage)
      page = setuptools.lightbox.menu.paginate.state.QueueHistory.lastPage;
    ...
    if (html === "") {
      ...
      html =
        ...
        ';">Account</div> \
                    <div style="width: ' +
        Widths[2] +
        ';" class="textCenter">Waited</div> \
                    <div style="width: ' +
        Widths[3] +
        ';" class="textCenter">Runtime</div> \
                    <div style="width: ' +
        Widths[4] +
        ';" class="textCenter">Status</div> \
                </div>\
            ' +
        html;
    }
  ...
  var createRow = function (listPos) {
    ...
    var record = {
      ...
      status:
        typeof QueueRecord.errorMessage === "string"
          ? '<a href="#" class="mulequeue link viewError rateLimited nopulse noclose" data-recordId="' +
            listPos +
            '">' +
            QueueRecord.status +
            "</a>"
    ...
    html +=
      ' \
            <div class="flex-container historyitem noselect menuStyle buttonStyle neutral textLeft w100 select mr0 mt5" style="justify-content: space-between; border: 0;" data-listPos="' +
      listPos +
      '"> \
        ';
  ...
  var QueueHistoryContext = function () {
    $("a.mulequeue.link.viewError")
      .unbind("click")
      .click(function () {
        ViewError($(this).attr("data-recordId"));
      });

    $("div.mulequeue.link.manager")
      .unbind("click")
      .click(function () {
        setuptools.lightbox.close();
        setuptools.app.mulequeue.ui.manager();
    ...
    $("div.mulequeue.link.refeshHistory")
      .unbind("click")
      .click(function () {
        if (
          typeof setuptools.lightbox.menu.paginate.state.QueueHistory ===
  ...
  var QueueHistoryUpdate = function (page) {
    if (
      ...
      page = setuptools.lightbox.menu.paginate.state.QueueHistory.currentPage;
    if (["string", "number", "undefined"].indexOf(typeof page) === -1) return;
    var QueueHistoryPaginate =
      setuptools.lightbox.menu.paginate.state.QueueHistory;
    ...
    //  update currentPage
    if (typeof page === "undefined")
      setuptools.lightbox.menu.paginate.findPage(0, "QueueHistory");
    if (typeof page === "string")
      setuptools.lightbox.menu.paginate.findPage(page, "QueueHistory");
    if (typeof page === "number")
      setuptools.lightbox.menu.paginate.state.QueueHistory.currentPage = page;

    //  validate lastPage/currentPage boundary
    var newLastPage = Math.ceil(
      QueueHistoryPaginate.PageList.length /
        setuptools.data.config.accountsPerPage
    );
    if (newLastPage > 0) newLastPage--;
    if (QueueHistoryPaginate.currentPage > newLastPage)
      setuptools.lightbox.menu.paginate.findPage(
        QueueHistoryPaginate.PageList.length - 1,
        "QueueHistory"
      );
    ...
    $("div.QueueHistory.list").html(
      ' \
            <div class="fleft cboth mb5 w100">' +
        QueueHistoryPaginate.html.menu +
        '</div> \
            <div class="setuptools app list w100 cboth">\
                ' +
    ...
    //  update the customPage value
    if (customPageFocus)
      $('div.QueueList input[name="customPage"]')
        .focus()
        .val("")
        .val(customPageNumber);
  ...
  var ViewError = function (listPos) {
    ...
    setuptools.lightbox.display("mulequeue-history-viewError", {
      variant: "fl-Notice",
    });
  };
  ...
  setuptools.lightbox.display("mulequeue-history", {
    variant: "fl-MuleQueueHistory",
  });
  QueueHistoryPaginate.bind();
};
...
setuptools.app.mulequeue.spinner.running = function (type, returnHtml) {
  ...
  if (type === "queue" && setuptools.app.mulequeue.state.paused === true)
    type = "pause";
  if (type === "queue" && setuptools.app.mulequeue.state.rateLimited === true)
    type = "error";
