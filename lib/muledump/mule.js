(function ($, window) {
  ...
  var ROW = setuptools.data.config.rowlength
    ? setuptools.data.config.rowlength
    : 7;
  if (setuptools.data.config.temprowlength != 0)
    ROW = setuptools.data.config.temprowlength;
  var vaultlayout = setuptools.data.config.vaultlayout
    ? setuptools.data.config.vaultlayout
    : 0;
    ...
    l.on("click.mulelogin", function (e) {
      if (setuptools.data.config.muleloginCopyLinks === 0) {
        ...
        setTimeout(
          ...
          setuptools.config.muleloginCopyLinksTtl,
          this
        );
      }
  ...
  function addmulemenu(mule) {
    ...
    menu.html("&#8801;");
    menu
      .off("click.muledump.mulemenu")
      .on("click.muledump.mulemenu", function (e) {
        setuptools.app.muledump.mulemenu(e, mule.guid, menu, menu, {
          option: "pos",
          vpx: 26,
        });
      });
    if (setuptools.data.config.muleMenu === false)
      menu.css({ visibility: "hidden", width: 0, height: 0 });
    menu.appendTo(mule.dom);

    $("div.mule > div.name")
      .off("contextmenu.muledump.mulemenu")
      .on("contextmenu.muledump.mulemenu", function (e) {
    ...
    mule.dom
      ...
      .on("contextmenu.muledump.mulemenu", function (e) {
        if (
          ($(e.target)[0].localName === "img" &&
            $(e.target)[0].className === "portrait") ||
          ($(e.target)[0].localName === "a" &&
            $(e.target)[0].className === "button ocl")
        )
          return;
        setuptools.app.muledump.mulemenu(e, mule.guid, menu);
      });
  ...
  function arrangevaults(v, layout) {
    ...
    var VAULTORDER =
      setuptools.data.muledump.vaultbuilder.layouts[layout].vaultorder;
    var r = [],
      i,
      j;
    for (i = 0; i < VAULTORDER.length; i++) {
      ...
      if (typeof c !== "undefined") r.push(c);
        ...
        r.push(
          VAULTORDER[i] === 0
            ? 0
            : setuptools.data.muledump.vaultbuilder.layouts[layout]
                .vaultshowempty === true
            ? "-1"
            : setuptools.data.muledump.vaultbuilder.layouts[layout]
                .vaultcompressed === false
            ? "-1"
            : 0
        );
    }
  ...
  var Mule = function (guid) {
    ...
    if (
      ...
      this.dom.addClass("filteringEnabled");
    this.dom.appendTo($("#stage")).hide();

    //  make sure no secondary account data caches are kept if the main data cache is missing
    if (!localStorage[this.cache_id()])
      setuptools.app.muledump.cleanupDataForGuid(this.guid);
    ...
    this.totals = new Muledump_TotalsCounter(this.guid, true, function (self) {
      ...
      if (
        setuptools.app.muledump.totals.config.getKey("accountFilter").length ===
          0 ||
        (typeof self === "object" &&
          typeof self.guid === "string" &&
          setuptools.app.muledump.totals.config
            .getKey("accountFilter")
            .indexOf(self.guid) > -1)
      )
    ...
    if (
      Array.isArray(setuptools.data.options.disabled) === true &&
      setuptools.data.options.disabled.indexOf(this.guid) > -1
    ) {
      this.disabled = true;
    ...
    this.dom.on("click.muledump.disableMule", ":not(.item)", function (e) {
      if (
        ...
        setuptools.app.muledump.keys("shift", e) === false
      ) {
    ...
    this.dom.on("click.muledump.mule", function (e) {
      ...
      if (
        ...
        if (setuptools.tmp.updateFilterTimer1)
          clearTimeout(setuptools.tmp.updateFilterTimer1);
      ...
      //  remove account from account filter
        ...
        if (setuptools.tmp.updateFilterTimer2)
          clearTimeout(setuptools.tmp.updateFilterTimer2);
  ...
  Mule.prototype.error = function (s, callback, options) {
    ...
    $(
      'div.mule[data-guid="' + this.guid + '"] div.setuptools.link.errorAck'
    ).on("click.muledump.errorAck", function () {
      //  remove the bar
  ...
  Mule.prototype.queueFinish = function (
    guid,
    status,
    cache_only,
    errorMessage
  ...
  Mule.prototype.query = async function (
    ignore_cache,
    cache_only,
    freshness,
    MuleQueue
    ...
    if (
      ...
      setuptools.app.config.validateFormat(setuptools.data.accounts, 1) ===
        true &&
      setuptools.data.accounts.accounts[this.guid].cacheEnabled === false
    ) {
      ignore_cache = true;
    }
    var self = this;
    ...
    if (!ignore_cache || ignore_cache === false) {
      var c = "";
      try {
        c = await setuptools.storage.read(this.cache_id(), true);
        c = JSON.parse(c);
      } catch (e) {}
      if (c) {
        ...
        if (
          ...
          if (cacheAge >= setuptools.data.config.autoReloadDays) {
            cacheStale = true;
            window.techlog(
              "MuleQueue/StaleCache found for " + this.guid,
              "string"
            );
            setuptools.app.ga("send", "event", {
        ...
        if (
          ...
          setuptools.app.config.validateFormat(setuptools.data.accounts, 0) ===
            true ||
          (setuptools.app.config.validateFormat(setuptools.data.accounts, 1) ===
            true &&
            setuptools.data.accounts.accounts[this.guid].cacheEnabled === true)
        ) {
    ...
    $.ajax({
      ...
      url:
        realmApiURL +
        "/account/verify?" +
        $.param(setuptools.config.realmApiParams),
      method: "POST",
      data: $.param(CR),
      success: function (xml, textStatus, request) {
        ...
        if (accessToken) {
          ...
          xml ===
            "<Error>LOGIN ATTEMPT LIMIT REACHED, please wait 5 minutes before re-trying!</Error>"
        ) {
          self.queueFinish(
            self.guid,
            "hardRateLimited",
            "Request was rate limited by Appspot"
          );
          ...
          xml === "<Error>WebChangePasswordDialog.passwordError</Error>"
        ) {
      ...
      if (self.fails < setuptools.config.muledump.corsMaxAttempts) {
        ...
        if (xhr.status === 0) {
          self.queueFinish(
            ...
            "error",
            false,
            "Could not connect to Appspot (CORS error?)"
          );
          self.error(
            'There was a problem connecting to ROTMG servers. <a href="#" class="setuptools link cors">Click here</a> for help.'
          );
          $(".setuptools.link.cors")
            .off("click.setuptools.corsAssistant")
            .on("click.setuptools.corsAssistant", function () {
        ...
        if (xhr.status > 0) {
          self.queueFinish(
            ...
            "error",
            false,
            "Could not connect to Appspot due to HTTP Error " + xhr.status
          );
          self.error(
            "HTTP Error " +
              xhr.status +
              ...
              '" target="_blank">Click here</a> for help.'
          );
    ...
    function onResponse(xml, textStatus, request) {
      ...
      if (setuptools.state.extension === undefined) {
        if (!request.getResponseHeader("X-Jakcodex-CORS")) {
          ...
          if (["opera", "firefox"].indexOf(setuptools.browser) > -1)
            setuptools.app.muledump.notices.add(
              "New CORS Extension Available",
              setuptools.app.assistants.jakcodexcors
            );
        } else {
      ...
      date = new Date(
        Date.now() -
          (typeof setuptools.tmp.timeOffset === "number"
            ? setuptools.tmp.timeOffset
            : 0)
      );
      window.techlog("Response from api: " + xml, "api");
      ...
      if ((error = xml.match(/^<Error\/?>(.*?)<\/?Error>$/))) {
        ...
        if (
          ...
          self.log(
            "Your IP is being rate limited by Deca. Waiting 5 minutes to retry."
          );
          self.queueFinish(
            self.guid,
            "rateLimited",
          ...
          try {
            localStorage["muledump:ratelimitexpiration"] =
              window.RateLimitExpiration;
          } catch (e) {}
        } else if (
          error[1] === "Account is under maintenance" ||
          error[1] ===
            "This account has been suspended for breaching Terms of Service. Please contact our Support Desk for more details"
        ) {
          if (
            setuptools.state.loaded === true &&
          ...
          if ([1, 4].indexOf(setuptools.data.config.badaccounts) > -1)
            setuptools.app.config.userToggle(self.guid);

          //  delete invalid accounts
          if ([3, 5].indexOf(setuptools.data.config.badaccounts) > -1)
            self.deleteAccount = true;
          ...
          var endTime;
          if (regex)
            endTime = new Date(Date.now() + Number(regex[1]) * 1000).timeString(
              false
            );
          self.error(
            "Account in use - " +
              (endTime
                ? 'please try again or wait until " + endTime'
                : "Please logout of the account in-game and try again")
          );
          setuptools.app.ga("send", "event", {
            eventCategory: "detect",
        ...
        setuptools.app.uaTiming(
          ...
          self.guid,
          false,
          setuptools.app.uaTimingAggregate
        );
  ...
  var STATTAGS =
    "MaxHitPoints MaxMagicPoints Attack Defense Speed Dexterity HpRegen MpRegen".split(
      " "
    );
  var STATABBR = "HP MP ATT DEF SPD DEX VIT WIS".split(" ");
  Mule.prototype.parse = async function (data, skipCacheWrite) {
    this.canParse = true;
    ...
    function calculateStatTotals(data, guid) {
      ...
      result.CacheAge = Math.floor(
        (Date.now() - cacheDate.getTime()) / 1000 / 86400
      );
      ...
      else if (typeof data.Char === "object" && data.Char.length > 0) {
        char = data.Char;
      }
      ...
      if (typeof data.Account.Gifts === "string")
        result.TotalGifts = data.Account.Gifts.split(",").length;
      ...
      if (typeof data.Char !== "undefined") {
        ...
        if (data.Char.length) {
          ...
          for (var i = 0; i < data.Char.length; i++) {
            result.TotalFame =
              Number(result.TotalFame) + Number(data.Char[i].CurrentFame);
            result.TotalExp =
              Number(result.TotalExp) + Number(data.Char[i].Exp);
          ...
          result.TotalFame =
            Number(result.TotalFame) + Number(data.Char.CurrentFame);
      ...
      if (result.LastGoldPurchase.indexOf(":") > -1)
        result.LastGoldPurchase = result.LastGoldPurchase.substr(
          0,
          result.LastGoldPurchase.length - 3
        );
    ...
    if (
      typeof d.Servers === "object" &&
      typeof d.Servers.Server === "object" &&
      typeof d.Servers.Server.push === "function"
    )
      await setuptools.storage.write(
        "ServerList",
        JSON.stringify(d.Servers.Server, true, 5)
      );
    ...
    d = {
      ...
      Account: d.Account || {},
      OwnedSkins:
        typeof d.OwnedSkins === "object" &&
        typeof d.OwnedSkins.push === "function"
          ? d.OwnedSkins
          : typeof d.OwnedSkins === "object"
          ? []
          : d.OwnedSkins.split(","),
      SalesForce:
        typeof d.SalesForce === "object"
          ? d.SalesForce
          : d.SalesForce === "undefined"
          ? {}
          : parseQueryString(atob(d.SalesForce)),
    };
    ...
    for (var i in carr) {
      if (carr.hasOwnProperty(i)) {
        ...
        //  add ClassName
        carr[i].muledump.ClassName =
          typeof carr[i].ObjectType &&
          Array.isArray(classes[carr[i].ObjectType]) === true
            ? classes[carr[i].ObjectType][0]
            : "????";
    ...
    if (skipCacheWrite !== true) {
      try {
        ...
        setuptools.app.uaTiming(
          "mule",
          "writeAccountDataCache",
          "start",
          this.guid,
          false,
          setuptools.app.uaTimingAggregate
        );
        await setuptools.storage.write(this.cache_id(), data, true);
        setuptools.app.uaTiming(
          "mule",
          "writeAccountDataCache",
          "stop",
    ...
    if (this.loginOnly === false) {
      ...
      //  error bar
      this.errorBar = $("<div>")
        .addClass("errorBar stickynotice")
        .css({ "font-size": "12px" });
      if (setuptools.data.config.animations < 1)
        this.errorBar.addClass("AReduced");
      ...
      $name.on("click.muledump.muleAccountName", function (e) {
        ...
        if (
          setuptools.app.muledump.keys("ctrl", e) === true ||
          setuptools.app.muledump.keys("shift", e) === true
        )
          return;
        var $ao = $("#accopts");
        $ao.css({
      ...
      if (!this.opt("shrink")) {
        ...
        if (this.opt("accountinfo")) {
          stats = await calculateStatTotals(
            this.data.query.results.Chars,
            this.guid
          );
          $(
            '<div class="stats">\
                        <div class="flex-container"><div>Account Data Cache Age</div><div>' +
              stats.CacheAge +
              " " +
              (stats.CacheAge === 1 ? "day" : "days") +
              '</div></div> \
                        <div class="flex-container"><div>Account Fame</div><div>' +
              this.data.query.results.Chars.Account.Stats.Fame +
              '</div></div> \
                        <div class="flex-container"><div>All Time Account Fame</div><div>' +
              this.data.query.results.Chars.Account.Stats.TotalFame +
              '</div></div> \
                        <div class="flex-container"><div>Account Gold</div><div>' +
              this.data.query.results.Chars.Account.Credits +
              '</div></div> \
                        <div class="flex-container"><div>Gift Items</div><div>' +
              stats.TotalGifts +
              '</div></div> \
                        <div class="flex-container"><div>Last Gold Purchase</div><div>' +
              stats.LastGoldPurchase +
              '</div></div> \
                        <div class="flex-container"><div>Total Char Exp</div><div>' +
              stats.TotalExp +
              '</div></div> \
                        <div class="flex-container"><div>Total Char Fame</div><div>' +
              stats.TotalFame +
              '</div></div> \
                        <div class="flex-container"><div>Total Death Fame</div><div>' +
              stats.TotalDeathFame +
              '</div></div> \
                        <div class="flex-container"><div>Total Characters</div><div>' +
              stats.TotalChars +
              '</div></div> \
                        <div class="flex-container"><div>Total Character Slots</div><div>' +
              (data.query.results.other
                ? data.query.results.other.maxCharNum
                : "N/A") +
              '</div></div> \
                        <div class="flex-container"><div>Total Lifetime Characters</div><div>' +
              (data.query.results.other
                ? data.query.results.other.nextCharId - 1
                : "N/A") +
              '</div></div> \
                        <div class="flex-container"><div>Total Skins Unlocked</div><div>' +
              (this.data.query.results.Chars.OwnedSkins.length +
                Object.keys(classes).length) +
              " of " +
              Object.keys(skins).length +
              '</div></div>\
                        <div class="flex-container"><div>Total Time Active</div><div>' +
              stats.TotalActive +
              '</div></div> \
                        <!-- removed for now due to inaccurate data coming from deca -- <div class="flex-container"><div>Lifetime Gold Purchased</div><div>' +
              this.data.query.results.Chars.SalesForce.lifetime_spend +
              '</div></div>--> \
                        <div class="flex-container"><div>Total Unlocked Chests</div><div>' +
              stats.TotalChests +
              " of " +
              vaultorders[0].vaultorder.filter(function (item) {
                return item !== 0;
              }).length +
              "</div></div> \
                </div>"
        ...
        var TotalGifts = 0;
        if (typeof d.Account.Gifts === "string")
          TotalGifts = d.Account.Gifts.split(",").length;
        if (TotalGifts >= setuptools.config.giftsWarningThreshold) {
          //  check if we should alert the user
          if (
            setuptools.app.config.determineFormat(setuptools.data.accounts) ===
              0 ||
            setuptools.data.accounts.accounts[this.guid].giftsBugAck !== true
          ) {
            this.error(
              'Warning: Account is nearing the <a href="' +
                setuptools.config.giftChestsBugHelp +
                '" class="rateLimited nopulse" target="_blank">gift chests bug</a>!',
              function () {
                if (
                  setuptools.app.config.determineFormat(
      ...
      if (setuptools.data.config.debugging === true)
        setuptools.app.uaTiming(
          "mule",
          "parseCharacterSorting",
          "start",
          this.guid,
          false,
          setuptools.app.uaTimingAggregate
      ...
      if (this.opt("chsort") === "100") {
        ...
        if (skipCacheWrite !== true)
          setuptools.app.muledump.charsort.clean(this.guid);
        var CharList = setuptools.app.muledump.charsort.read(this.guid, true);
        if (Array.isArray(CharList) && CharList.length > 0) {
          CharList = setuptools.app.muledump.charsort.disabled(
            this.guid,
            setuptools.app.muledump.charsort.read(this.guid),
            CharList
          );
          this.createCounter();
          this.redoTotals = true;
          ...
          if (setuptools.data.options.totalsGlobal === false)
            CountingCharList = CharList;
        } else {
          //  reset the active list
          if (Array.isArray(CharList) && CharList.length === 0)
            setuptools.app.muledump.charsort.active(this.guid);
    ...
    if (
      ...
      d.Account &&
      d.Account.Stats &&
      d.Account.Stats.ClassStats &&
      d.Account.Stats.ClassStats.length > 0
    ) {
      var cswidth = setuptools.data.config.rowlength * 220;
      var cshtml =
        ' \
                <div class="classstats flex-container noFlexAutoWidth" style="flex-wrap: wrap; justify-content: flex-start; width: ' +
        cswidth +
        'px;">\
            ';
      var cslist = {};
        ...
        cshtml +=
          ' \
                    <div class="class flex-container noFlexAutoWidth ml5 mr5 mb5 mt5 menuStyle nohover" style="flex-wrap: wrap; justify-content: space-between;"> \
                        <div class="className flex-container noFlexAutoWidth" style="width: 41%;">' +
          skins[+stats.objectType][0] +
          '</div>\
      ...
      cshtml +=
        " \
            ...
            ";
      $('<div class="classstats flex-container">')
        .html(cshtml)
        .appendTo(this.dom);
    }

    //  main character loop
    if (typeof setuptools.tmp.wawawaTimers === "undefined")
      setuptools.tmp.wawawaTimers = {};
    if (typeof setuptools.tmp.mainCharLoopTimers === "undefined")
      setuptools.tmp.mainCharLoopTimers = {};
    ...
    if (this.totals.cache === false) {
      for (var z = 0; z < CountingCharList.length; z++) {
        ...
        for (var tec = 0; tec < 4; tec++)
          this.totals.count(eqc[tec] || "-1", "equipment");
        for (var tic = 4; tic < 12; tic++)
          this.totals.count(eqc[tic] || "-1", "inv");
        if (+CountingCharList[z].BackpackSlots >= 8)
          for (var tbc = 12; tbc < (eqc.length < 20 ? 20 : eqc.length); tbc++)
            this.totals.count(eqc[tbc] || "-1", "backpack");
      }
    }
    ...
    for (var i = 0; i < carr.length; i++) {
      ...
      if (
        ...
        if (
          ...
          this.data.query.results.PowerUpStats.ClassStats
        ) {
          if (!Array.isArray(this.data.query.results.PowerUpStats.ClassStats))
            this.data.query.results.PowerUpStats.ClassStats = [
              this.data.query.results.PowerUpStats.ClassStats,
            ];
          const hasExalts =
            this.data.query.results.PowerUpStats.ClassStats.find(
              (stats) => stats.class === c.ObjectType
            );
          if (hasExalts) {
        ...
        for (var t = 0; t < STATTAGS.length; t++) {
          ...
          if (!cl) {
            ...
            var avgd =
              s -
              Math.floor(
                cl[1][t] + ((cl[2][t] - cl[1][t]) * (+c.Level - 1)) / 19
              );
            stat(
              $s,
              "avg",
              (avgd > 0 ? "+" : "") +
                avgd +
                (exaltations && exaltations[t] !== 0 ? exaltations[t] : "")
            )
            ...
            stat(
              $s,
              "max",
              l2m + (exaltations && exaltations[t] !== 0 ? exaltations[t] : "")
      ...
      var tcount = [];
      var dobp =
        this.opt("backpack") && (c.BackpackSlots == 8 || c.BackpackSlots == 16);
      ...
      if (this.opt("equipment") || this.opt("inv") || dobp) {
        ...
        if (this.opt("equipment")) {
          ...
          if (this.loginOnly === false && !this.opt("shrink"))
            itc.append(
              setuptools.app.muledump.item_listing(
                eq.slice(0, 4),
                "equipment",
                c.ObjectType
              )
        ...
        if (this.opt("inv")) {
          ...
          if (this.loginOnly === false && !this.opt("shrink"))
            itc.append(
              setuptools.app.muledump.item_listing(eq.slice(4, 12), "inv")
            );
        }
        if (dobp) {
        ...
        if (
          ...
          for (var j = 0; j < items.length; j++) {
            ...
            setuptools.app.muledump
              .item(item[0], undefined, qty, undefined)
              .appendTo(r);
          }
      ...
      if (
        ...
        fbonus.push(
          "\nTotal Fame : " +
            iTotalFame.toLocaleString() +
            " Fame" +
            "\n\nFirst Born : " +
            d.Account.Stats.BestCharFame
        );
        ...
        if (apply_bonus(+c.CurrentFame) < d.Account.Stats.BestCharFame) {
          ...
          fbonus.push(
            "\nBase Fame Needed for First Born : " +
              lower.toLocaleString() +
              " (" +
              upperleft.toLocaleString() +
              " Base Fame Left)"
          );
        }
        ...
        if (iTotalFame > 0) {
          if (iTotalFame > 400) {
              ...
              $("<div>")
                .append(
                  $('<div style="color:gold;font-weight:bold">')
                    ...
                    .attr("title", fbonus.join(""))
                )
                .appendTo($c);
            } else {
              $("<div>")
                .append(
                  $('<div style="color:#39ef4e;font-weight:bold">')
                    .text(
                      " Total Fame = " + iTotalFame.toLocaleString() + " Fame "
                    )
                    .attr("title", fbonus.join(""))
            ...
            $("<div>")
              .append(
                $('<div style="color:#99f7a4;font-weight:bold">')
                  .text(
                    " Total Fame = " + iTotalFame.toLocaleString() + " Fame "
                  )
                  .attr("title", fbonus.join(""))
          ...
          $("<div>")
            .append(
              $('<div style="color:red;font-weight:bold">')
                .text(" Total Fame = " + iTotalFame.toLocaleString() + " Fame ")
                .attr("title", fbonus.join(""))
            )
            .appendTo($c);
        }
        ...
        var FPM =
          round(fame / v, 2) +
          " Base Fame/Minute\n" +
          round(iTotalFame / v, 2) +
          " Total Fame/Minute\nFame Boost : " +
          round((iTotalFame / fame) * 100, 2) +
          " %";
        ...
        if (TilesLeft > 0) {
          var vCart = round((4e6 - st[3] + 1) / round(st[3] / st[20], 0), 0),
            TilesTime = [];
          ...
          if (TilesLeft > 3e6) {
            ...
            var vExpl = round((1e6 - st[3] + 1) / round(st[3] / st[20], 0), 0),
              TilesTimeExpl = [];
          ...
          TilesData.push(
            TPM.toLocaleString() + " Tiles/Minute\n\n" + TilesLeft
          );
        }
          ...
          if (st[3] > 1e6) {
            if (st[3] > 4e6) {
              $("<div>")
                .append(
                  $('<div style="color:gold;font-weight:bold">')
                    .text(" Tiles = " + TilesDone)
                    .attr("title", TilesData)
                )
                .appendTo($c);
            } else {
              $("<div>")
        ...
        if (TratQuest.length == 0) {
          if (QuestMiss.length == 0) {
            $("<div>")
              .append(
                $('<div style="color:gold;font-weight:bold">')
                  .text(" Tunnel Rat = Complete")
                  .attr("title", QuestMiss.join(" "))
              )
              .appendTo($c);
          } else {
            $("<div>")
          ...
          if (TratQuest.length < 5) {
            $("<div>")
              .append(
                $('<div style="color:#39ef4e;font-weight:bold">')
                  .text(" Tunnel Rat = In Progress")
                  .attr("title", TratQuest.join(" "))
              )
              .appendTo($c);
        ...
        if (OKill < 1) {
          $("<div>")
            .append(
              $('<div style="color:red;font-weight:bold">').text(
                " Oryx Kills = " + OKill
              )
            )
            .appendTo($c);
          ...
          $("<div>")
            .append(
              $('<div style="color:gold;font-weight:bold">').text(
                " Oryx Kills = " + OKill.toLocaleString()
              )
            )
            .appendTo($c);
        }
        ...
        l2Acc =
          "Accurate : " +
          Math.ceil(l2Acc3).toLocaleString() +
          " Shots Left\nSharpshooter : " +
          Math.ceil(l2Acc2).toLocaleString() +
          " Shots Left\nSniper : " +
          Math.ceil(l2Acc).toLocaleString() +
          " Shots Left";
        ...
        GodData.push(
          "Monster Kills : " +
            ...
            st[8].toLocaleString() +
            "\n\nEnnemy of the Gods : " +
            Math.ceil(tenpercent).toLocaleString() +
            " Kills Left\nSlayer of the Gods : " +
            GodKillLeft.toLocaleString() +
            " Kills Left"
        );
          ...
          if (GodKillRatio > 10) {
            if (GodKillRatio > 50) {
              $("<div>")
                .append(
                  $('<div style="color:gold;font-weight:bold">')
                    .text(" God Kill Ratio = " + GodKillRatio + " %")
                    .attr("title", GodData)
                )
                .appendTo($c);
            } else {
        ...
        if (LvlUpLeft > 0) {
          if (LvlUpLeft > 900) {
            ...
            LvlUpLeft =
              "Team Player : " +
              LvlUpLeft2.toLocaleString() +
              " Level Up Left\nLeader of Men : " +
              LvlUpLeft.toLocaleString() +
              " Level Up Left";
          } else {
            LvlUpLeft =
              "Leader of Men : " +
          ...
          if (LvlUp > 100) {
            if (LvlUp > 1000) {
              $("<div>")
                .append(
                  $('<div style="color:gold;font-weight:bold">')
                    .text(" Level Up = " + LvlUp.toLocaleString())
                    .attr("title", LvlUpLeft)
                )
                .appendTo($c);
            } else {
              $("<div>")
          ...
          if (st[12] < 500) {
            if (st[12] < 1) {
              $("<div>")
                .append(
                  $('<div style="color:red;font-weight:bold">')
                    .text(" Quests Completed = " + QuestDone)
                    .attr("title", QuestList.join(" "))
                )
                .appendTo($c);
            } else {
              $("<div>")
      ...
      if (this.opt("pcstats") || this.opt("goals")) {
        f = true;
        $c.append(
          window.printstats(c, d, this.opt("goals"), this.opt("pcstats"), this)
        );
      }
      arr.push($c);
    ...
    function makechest(items, classname, index) {
      var il = setuptools.app.muledump.item_listing(
        items.slice(0, 8),
        classname
      );
      return $(
        '<div class="items" data-vaultId="' +
          (setuptools.data.muledump.vaultbuilder.layouts[vaultlayout]
            .vaultorder[index] || 0) +
          '">'
      ).append(il);
    }
    ...
    vaultlayout =
      this.opt("vaultlayout") &&
      setuptools.data.muledump.vaultbuilder.layouts[this.opt("vaultlayout")]
        ? this.opt("vaultlayout")
        : setuptools.data.options.vaultlayout;
    vaultwidth =
      typeof setuptools.data.muledump.vaultbuilder.layouts[vaultlayout] ===
        "object" &&
      setuptools.data.muledump.vaultbuilder.layouts[vaultlayout].vaultwidth
        ? setuptools.data.muledump.vaultbuilder.layouts[vaultlayout].vaultwidth
        : ROW;
    VAULTORDER = setuptools.data.muledump.vaultbuilder.layouts[vaultlayout]
      ? setuptools.data.muledump.vaultbuilder.layouts[vaultlayout].vaultorder
      : vaultorders[0];
    ...
    if (d.Account.Vault && this.totals.cache === false) {
      ...
      if (typeof chests === "object" && Object.keys(chests).length === 0)
        chests = ["-1"];
      if (Array.isArray(chests) === false) {
        setuptools.app.techlog(
          "Mule " + this.guid + " encountered bad chest data"
        );
    ...
    if (this.opt("vaults")) {
      ...
      for (var x = 0; x < chests.length; x++) {
        ...
        if (typeof chests[x] !== "string") {
          ...
          if (typeof chests[x] === "object")
            chests[x] = "-1,-1,-1,-1,-1,-1,-1,-1";
        }
    ...
    if (!(d.Account.MaterialStorage === undefined)) {
      ...
      if (
        d.Account &&
        d.Account.MaterialStorage &&
        d.Account.MaterialStorage.Chest
      ) {
    ...
    if (this.opt("materials") && !(d.Account.MaterialStorage === undefined)) {
      ...
      if (this.loginOnly === false && !this.opt("shrink"))
        this.dom.append($('<hr class="materials">'));
      if (Array.isArray(items) === true) {
        var marr = [];
        while (items.length) {
      ...
      if (setuptools.data.config.debugging === true)
        setuptools.app.uaTiming("mule", "parseMaterials", "stop", this.guid);
    }

    // ssnlspoils counting for totals
    var spoils = d.Account.TemporaryGifts;
    var items =
      typeof spoils === "string" ? spoils.split(",").reverse() : undefined;
    if (Array.isArray(items) === true && this.totals.cache === false) {
      for (var tg = 0; tg < items.length; tg++)
    ...
    if (this.opt("spoils")) {
      ...
      if (this.loginOnly === false && !this.opt("shrink"))
        this.dom.append($('<hr class="spoils">'));
      if (Array.isArray(items) === true) {
        var sarr = [];
        while (items.length) {
      ...
      if (setuptools.data.config.debugging === true)
        setuptools.app.uaTiming("mule", "parseSpoils", "stop", this.guid);
    }
    ...
    if (this.opt("potions")) {
      ...
      if (setuptools.data.config.debugging === true)
        setuptools.app.uaTiming("mule", "parsePotions", "stop", this.guid);
    }
    ...
    if (!(d.Char === undefined)) {
      for (let index = 0; index < d.Char.length; index++) {
        if (
          d.Char[index].Seasonal === "True" &&
          d.Char[index].Pet !== undefined &&
          d.Char[index].Pet.inv !== undefined
        )
          ssnlpetinv = d.Char[index].Pet.inv.split(";")[2];
      }

      var items =
        typeof ssnlpetinv === "string"
          ? ssnlpetinv.split(",").reverse()
          : undefined;
      if (Array.isArray(items) === true && this.totals.cache === false) {
        for (var tg = 0; tg < items.length; tg++)
    ...
    if (this.opt("ssnlpetinv") && !(d.Char === undefined)) {
      ...
      if (this.loginOnly === false && !this.opt("shrink"))
        this.dom.append($('<hr class="ssnlpetinv">'));
      if (Array.isArray(items) === true) {
        var sarr = [];
        while (items.length) {
      ...
      if (setuptools.data.config.debugging === true)
        setuptools.app.uaTiming("mule", "parseSsnlpetinv", "stop", this.guid);
    }
    ...
    if (!(d.Char === undefined)) {
      ...
      var items =
        typeof petinv === "string" ? petinv.split(",").reverse() : undefined;
      if (Array.isArray(items) === true && this.totals.cache === false) {
        for (var tg = 0; tg < items.length; tg++)
    ...
    if (this.opt("petinv") && !(d.Char === undefined)) {
      ...
      if (this.loginOnly === false && !this.opt("shrink"))
        this.dom.append($('<hr class="petinv">'));
      if (Array.isArray(items) === true) {
        var sarr = [];
        while (items.length) {
      ...
      if (setuptools.data.config.debugging === true)
        setuptools.app.uaTiming("mule", "parsePetinv", "stop", this.guid);
    }

    //  gift chests counting for totals
    var gifts = d.Account.Gifts;
    var items =
      typeof gifts === "string" ? gifts.split(",").reverse() : undefined;
    if (Array.isArray(items) === true && this.totals.cache === false) {
      for (var tg = 0; tg < items.length; tg++)
    ...
    if (this.loginOnly === false) {
      ...
      relayout(
        original === false || this.redoTotals === true ? undefined : false
      );
    }
    ...
    this.dom.on("click.mule.screenshot", function (e) {
      ...
      if (setuptools.app.muledump.keys(["ctrl", "shift"], e) === true)
        setuptools.app.muledump.exporter.canvas(dom);
    });

    //  hotkey for creating image of all chars
    this.dom.find("table.chars").on("click.chars.screenshot", function (e) {
      if (setuptools.app.muledump.keys(["ctrl", "shift"], e) === true)
        setuptools.app.muledump.exporter.canvas($(this)[0]);
    });

    //  hotkey for creating image of a char
    this.dom
      .find("table.chars > tr > td.cont")
      .on("click.char.screenshot", function (e) {
        if (setuptools.app.muledump.keys(["ctrl", "shift"], e) === true)
          setuptools.app.muledump.exporter.canvas($(this)[0]);
      });
    ...
    this.dom
      .find("table.vaults > tr > td.cont")
      .on("click.vault.screenshot", function (e) {
        if (setuptools.app.muledump.keys(["ctrl", "shift"], e) === true)
          setuptools.app.muledump.exporter.canvas($(this)[0]);
      });
