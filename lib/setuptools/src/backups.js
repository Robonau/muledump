 ...
 */
setuptools.app.backups.index = async function () {
    ...
    for (i = BackupList.length - 1; i >= 0; i--) {
      if (BackupList.hasOwnProperty(i)) {
        ...
        var BackupData = JSON.parse(
          await setuptools.storage.read(BackupList[i][1])
        );
        ...
        setuptools.lightbox.build(
          ...
          ' \
            ...
            '" \
                                data-CustomName="' +
            (typeof BackupList[i][5] === "string" && BackupList[i][5].length > 0
              ? BackupList[i][5]
              : "") +
            '" \
                                ' +
            (BackupList[i][1] === setuptools.tmp.SelectedBackupID
              ? "selected"
              : "") +
            ">\
                                " +
  ...
  setuptools.lightbox.drawhelp(
    "backups-index",
    "docs/setuptools/help/backups-manager/backups",
    "Backups Manager Help"
  );
  setuptools.lightbox.display("backups-index", {
    variant: "fl-BackupsManager",
  });
  ...
  $(".setuptools.link.backups.restore").click(function () {
    ...
    var BackupName = $('select[name="BackupName"] option:selected').attr(
      "data-BackupName"
    );
    var BackupCustomName = $('select[name="BackupName"] option:selected').attr(
      "data-CustomName"
    );
    var SuppliedName =
      typeof BackupCustomName === "string" && BackupCustomName.length > 0
        ? BackupCustomName
        : BackupName;
  ...
  $(".setuptools.link.backups.download").click(function () {
    ...
    var BackupFileName = $('select[name="BackupName"] option:selected').attr(
      "data-filename"
    );
    var BackupCustomName = $('select[name="BackupName"] option:selected').attr(
      "data-CustomName"
    );
    setuptools.app.backups.download(
      BackupID,
      BackupName,
      BackupFileName,
      BackupCustomName
  ...
  //  deletion option
  function bindDelete() {
    $(".setuptools.link.backups.delete")
      .unbind("click")
      .click(async function () {
        if (
          setuptools.tmp.lightboxStatus &&
          setuptools.tmp.lightboxStatus.indexOf("backup-delete") > -1
        )
          return;
        var BackupSelected = $('select[name="BackupName"] option:selected');
        var NextBackup =
          BackupSelected.next().length > -1
            ? BackupSelected.next()
            : BackupSelected.prev();
        var BackupID = BackupSelected.val();
        var BackupData = await setuptools.storage.read(BackupID);
        if (BackupData === false) {
          ...
          if (BackupData && typeof BackupData.meta === "object") {
            ...
            if (
              typeof BackupData.meta.protected === "undefined" ||
              BackupData.meta.protected === false
            ) {
              if ((await setuptools.storage.delete(BackupID)) === true) {
              ...
              setuptools.lightbox.status(this, "Protected!", "backup-delete");
          } else {
            ...
            if ((await setuptools.storage.delete(BackupID)) === true) {
              ...
              setuptools.lightbox.status(this, "Warning!", "backup-delete");
          }
        }
      });
  }
  ...
  async function updateProtection() {
    ...
    var SelectedBackupName = $(
      'select[name="BackupName"] option:selected'
    ).text();
    var SelectedBackupData = JSON.parse(
      await setuptools.storage.read(SelectedBackupID)
    );
    ...
    if (typeof SelectedBackupData === "object" && SelectedBackupData.meta) {
      ...
      if (typeof SelectedBackupData.meta.protected === "undefined")
        SelectedBackupData.meta.protected = false;
      if (SelectedBackupData.meta.protected === false) {
        $(".setuptools.link.backups.protect").removeClass("negative");
        $(".setuptools.link.backups.delete")
          .removeClass("deleteDisabled disabled truly")
          .attr("title", "Delete Backup");
        bindDelete();

        protectButton.text("Protect").attr("title", "Enable Backup Protection");
        protectButton.click(function () {
          setuptools.app.backups.protect(
            SelectedBackupID,
            SelectedBackupName,
            true
          );
        ...
        $(".setuptools.link.backups.delete")
          .addClass("deleteDisabled disabled truly")
          .unbind("click")
      ...
      $(".setuptools.link.backups.protect")
        .text("Examine")
        .attr("title", "Problem detected with backup");
  ...
  setuptools.lightbox.display("backups-archive-init", {
    afterOpen: function () {
      setuptools.app.backups.archive.create(function (content, name) {
        ...
        $("a.setuptools.link.download").on(
          "click.setuptools.backup.download",
          function (e) {
            e.preventDefault();
 ...
 * Rename a backup
 */
setuptools.app.backups.rename = async function (BackupID) {
  var BackupDataJSON = await setuptools.storage.read(BackupID);
  ...
  setuptools.lightbox.drawhelp(
    "backups-rename",
    "docs/setuptools/help/backups-manager/upload",
    "Backup Upload Help"
  );
  setuptools.lightbox.settitle("backups-rename", "Rename Backup");
  setuptools.lightbox.display("backups-rename", {
    variant: "fl-BackupsModify",
  });
...
setuptools.app.backups.auto = function () {
  if (setuptools.data.config.automaticBackups === true) {
    ...
    if (
      ...
      window.techlog(
        "setuptools.app.backups.auto - created automatic backup",
        "force"
      );
    }
  }
...
setuptools.app.backups.upload = function (manual) {
  function ParseUploadedFile(reader) {
    ...
    if (typeof UploadFileJSON === "object" && UploadFileJSON.meta) {
      $(".setuptools.link.backups.uploadData.save")
        .html("Restore Backup")
        .removeClass("noclose")
        .click(function () {
          setuptools.app.backups.restore({
            ...
            data: UploadFileJSON,
            backupName:
              typeof UploadFileJSON.meta.name === "string"
                ? UploadFileJSON.meta.name
                : "User-Uploaded",
            saveExisting: $('input[name="restoreSaveExisting"]').prop(
              "checked"
            ),
          });
        });
      ...
      $(".setuptools.app.backups.uploadResults").html(
        " \
                Found " +
          Object.keys(UploadFileJSON.accounts.accounts).length +
          " accounts, \
                " +
          Object.keys(UploadFileJSON.config).length +
          " settings, and \
                " +
          Object.keys(UploadFileJSON.options).length +
          " options \
            "
      );
        ...
        if (files && files.meta && files.meta.format === 1) {
          ...
          $(".setuptools.app.backups.uploadResults").html(
            " \
                        Found " +
              Object.keys(
                (files.setuptools && files.setuptools.accounts.accounts) || {}
              ).length +
              " accounts, \
                        " +
              Object.keys(files.accounts || {}).length +
              " data files, \
                        " +
              Object.keys((files.setuptools && files.setuptools.config) || {})
                .length +
              " settings, and \
                        " +
              Object.keys((files.setuptools && files.setuptools.options) || {})
                .length +
              " options \
                    "
          );
          ...
          if (
            ...
            if (files.accounts && files.setuptools)
              $("div.secondaryOptions > div").show();

            $(".setuptools.link.backups.uploadData").on(
              "click.setuptools.restore.archive",
              function () {
                var options = {
                  method: "upload",
                ...
                if (
                  $(this).hasClass("save") ||
                  $(this).hasClass("restoreSetupTools")
                ) {
                  options.data = files.setuptools;
                  if (!$(this).hasClass("restoreSetupTools"))
  ...
  var DoFiles = false;
  if (
    setuptools.config.devForcePoint !== "backups-upload" &&
    manual !== true &&
    window.File &&
    window.FileReader &&
    window.FileList &&
    ...
    if (
      typeof setuptools.tmp.FileReaderCapable === "undefined" &&
      manual !== true
    ) {
      setuptools.lightbox.build(
        "backups-upload",
        "File uploads are not supported by your browser. Please instead paste the contents of the backup file below. This can only be used with a SetupTools JSON backup."
      );
    } else {
      setuptools.lightbox.build(
        "backups-upload",
        "Please paste the contents of the backup file below. This can only be used with a SetupTools JSON backup."
      );
    }
      ...
      reader.onload = function () {
        if (reader.error) {
          ...
          setuptools.lightbox.error(
            "Failed to upload file with error: " + reader.error,
            23
          );
        } else {
 ...
 */
setuptools.app.backups.restore = async function (options) {
  options = $.extend(
    true,
    {
      method: undefined,
  ...
  if (options.method === "local" || options.method === "upload") {
    ...
    var BackupFile =
      options.method === "local"
        ? await setuptools.storage.read(options.backupId)
        : options.backupId;
    var count = 0;
    var methods = 1;
    ...
    setuptools.lightbox.drawhelp(
      "backups-restore-confirmed",
      "docs/setuptools/help/backups-manager/restore",
    ...
    if (BackupFile) {
      ...
      if (BackupData && typeof BackupData === "object") {
        ...
        if (
          ...
          setuptools.lightbox.error(
            "Parsed data is not a valid backup object.",
            23
          );
            ...
            if (
              ...
              if (username.length > 0 && password.length > 0) {
                ...
                if (username.length > 0) {
                  ...
                  if (
                    !username.match(setuptools.config.regex.email) &&
                    !username.match(setuptools.config.regex.guid)
                  ) {
        ...
        if (badEntries.length > 0 && options.badEntriesForce !== true) {
          setuptools.lightbox.build(
            ...
            " \
                        Warning: There were " +
              badEntries.length +
              ' invalid accounts in your backup. They have been removed. <br><br> \
          ...
          setuptools.lightbox.display("backups-restore-confirmed", {
            variant: "fl-BackupRestore",
          });
          $(".setuptools.link.backups.restoreCancelled").click(
            setuptools.app.backups.index
          );
        ...
        if (setuptools.state.loaded === true && options.saveExisting === true)
          ExistingBackupObject = await setuptools.app.config.backup(true);
        if (
          setuptools.state.loaded === true &&
          options.saveExisting === true &&
          ExistingBackupObject.status === false &&
        ...
        if (typeof BackupData.options === "object")
          window.options = BackupData.options;
        if (typeof BackupData.additional === "object") {
          if (typeof BackupData.additional.options === "object")
            window.options = BackupData.additional.options;

          if (typeof BackupData.additional.mulequeue === "object") {
            $.extend(
              true,
              setuptools.app.mulequeue.state,
              BackupData.additional.mulequeue.state
            );
            setuptools.app.mulequeue.history =
              BackupData.additional.mulequeue.history;
          }
        }
        ...
        if (setuptools.state.loaded === true)
          BackupData.accounts.meta.version =
            setuptools.data.accounts.meta.version + 1;

        //  update our local data
        setuptools.data = BackupData;
        if (
          (await setuptools.app.config.save("BackupsManager/Restore")) === true
        ) {
          //  save other configs
          ...
          setuptools.lightbox.build(
            "backups-restore-confirmed",
            "Failed to save restored configuration."
          );
          setuptools.lightbox.display("backups-restore-confirmed");
          return;
        }
    ...
    if (typeof options.archive === "object") {
      if (
        options.archive.accounts &&
        Object.keys(options.archive.accounts).length > 0
      ) {
        Object.keys(options.archive.accounts).filter(async function (guid) {
          if (options.archive.accounts[guid] === null) return;
          var data = JSON.stringify(options.archive.accounts[guid]);
          bytes += data.length;
          if (
            (await setuptools.storage.write("muledump:" + guid, data, true)) ===
            true
          )
            restored++;
        });
    ...
    if (count === methods) {
      ...
      setuptools.lightbox.build(
        ...
        "Backup " +
          options.backupName +
          " has been restored. <br><br>This window will reload in a few seconds."
      );
      setuptools.lightbox.display("backups-restore-confirmed");
    ...
    setuptools.lightbox.build(
      "backups-restore-confirmed",
      "Backup " + options.backupName + " restored no data."
    );
    setuptools.lightbox.display("backups-restore-confirmed");
  } else
    setuptools.lightbox.error(
      "Restore method " + options.method + " is not a valid option.",
      20
    );
};
...
setuptools.app.backups.restoreConfirm = function (BackupID, BackupName) {
  if (typeof BackupID === "undefined" || typeof BackupName === "undefined")
    setuptools.lightbox.error(
      "Required arguments BackupID or BackupName are missing.",
      19
    );
  if (!BackupID.match(setuptools.config.regex.backupId))
    setuptools.lightbox.error(
      "BackupID was tainted with an unrelated key name",
      18
    );

  setuptools.lightbox.build(
    "backups-restore-confirm",
    " \
 ...
 */
setuptools.app.backups.delete = async function (BackupID, BackupName) {
  if (typeof BackupID === "undefined" || typeof BackupName === "undefined")
    setuptools.lightbox.error(
      "Required arguments BackupID or BackupName missing.",
      17
    );
  if (!BackupID.match(setuptools.config.regex.backupId))
    setuptools.lightbox.error(
  ...
  if (BackupData === false) {
    setuptools.lightbox.build(
      "backup-delete",
      "Specified BackupID " + BackupID + " was not located."
    );
  } else {
    BackupData = JSON.parse(BackupData);
    if (BackupData && typeof BackupData.meta === "object") {
      ...
      if (
        ...
        if ((await setuptools.storage.delete(BackupID)) === true) {
          setuptools.lightbox.build(
            "backup-delete",
            "Successfully deleted backup " + BackupName + "."
          );
        } else
          setuptools.lightbox.build(
            "backup-delete",
            "Failed to delete backup " + BackupName + "."
          );
      } else
        setuptools.lightbox.build(
          "backup-delete",
      ...
      if ((await setuptools.storage.delete(BackupID)) === true) {
        setuptools.lightbox.build(
          "backup-delete",
          "Invalid backup data located. The object has been removed."
        );
      } else
        setuptools.lightbox.build(
          "backup-delete",
          "Invalid backup data located and it could not be deleted."
        );
    }
  }
  ...
  setuptools.lightbox.drawhelp(
    "backup-delete",
    "docs/setuptools/help/backups-manager/delete",
    "Backup Deletion Help"
  );
 ...
 */
setuptools.app.backups.protect = async function (
  BackupID,
  BackupName,
  BackupProtected
) {
  if (
    typeof BackupID === "undefined" ||
    typeof BackupName === "undefined" ||
    typeof BackupProtected === "undefined"
  )
    setuptools.lightbox.error(
      "Required arguments BackupID, BackupName, or protection state are missing.",
      15
    );
  if (!BackupID.match(setuptools.config.regex.backupId))
    setuptools.lightbox.error(
  ...
  var BackupData = JSON.parse(await setuptools.storage.read(BackupID));
  if (typeof BackupData === "object" && BackupData.meta) {
    //  if this key is missing let's add it with the default value
    if (typeof BackupData.meta.protected === "undefined")
      BackupData.meta.protected = false;
    ...
    if (BackupData.meta.protected === BackupProtected) {
      setuptools.lightbox.build(
        "backups-protect",
        "Protection state is already set to " + ProtectionState + "."
      );
    } else {
      //  change backup state
      ...
      if (
        ...
        setuptools.lightbox.build(
          "backups-protect",
          "Failed to change protection state."
        );
    }
    ...
    setuptools.lightbox.drawhelp(
      "backups-protect",
      "docs/setuptools/help/backups-manager/protect",
      "Backup Protection Help"
    );
 ...
 * Download a backup
 */
setuptools.app.backups.download = async function (
  BackupID,
  BackupName,
  BackupFileName,
  BackupCustomName
  ...
  try {
    BackupData = JSON.stringify(
      JSON.parse(await setuptools.storage.read(BackupID)),
      null,
      4
    );
  } catch (e) {}
  ...
  if (BackupData) {
    ...
    setuptools.lightbox.build(
      ...
      " \
            Backup " +
        (typeof BackupCustomName === "string"
          ? BackupCustomName
          : BackupFileName) +
        ' is ready for download. \
            ...
            <br><br><a download="' +
        FileName +
        '.json" href="data:text/json;base64,' +
        btoa(BackupData) +
        '" class="setuptools config download noclose">Download Backup</a> \
        '
    );
  } else {
    if (ParseError === true) {
      setuptools.lightbox.build(
        "backups-download",
        "There was a problem decoding the backup from " + BackupName + "."
      );
    } else
      setuptools.lightbox.build(
        "backups-download",
        "No backup exists with the name " + BackupName + "."
      );
  }
  ...
  if (BackupData) {
    await setuptools.storage.write("backupAssistant", Date.now());
 ...
 */
setuptools.app.backups.renameHtml = function (
  page,
  BackupID,
  BackupDataJSON,
  callback,
  ...
  setuptools.lightbox.build(
    ...
    ' \
        <br><br><div class="setuptools app fleft cboth w100"> \
            <input name="BackupCustomName" value="' +
      (typeof BackupData.meta.name === "string" ? BackupData.meta.name : "") +
      '" placeholder="Custom Backup Name" class="cfleft mr5"> \
  ...
  return async function () {
    var Process = async function (self) {
      ...
      await setuptools.storage.write(BackupID, BackupDataSON);

      //  update the download link
      $("a.setuptools.config.download").attr(
        "href",
        "data:text/json;base64," + btoa(BackupDataSON)
      );
 ...
 */
setuptools.app.backups.archive.create = async function (guids, callback) {
  var scope = false;
  if (guids === true) {
    scope = true;
    guids = undefined;
  ...
  if (setuptools.state.loaded === true) {
    backup = await setuptools.app.config.backup();
    zip.file("setuptools.json", backup.BackupData);
  }

  //  grab account data and insert files
  guids.filter(async function (guid) {
    var data = JSON.stringify(
      JSON.parse(await setuptools.storage.read("muledump:" + guid, true)),
      null,
      5
    );
 ...
 * Create a backup
 */
setuptools.app.backups.create = async function (silent) {
  var BackupObject = await setuptools.app.config.backup();

  if (BackupObject.status === false)
    setuptools.lightbox.build(
      "backups-create",
      '<span class="setuptools error">Warning</span>: Failed to save backup to browser storage.<br>&nbsp;<br>'
    );

  setuptools.lightbox.build(
    "backups-create",
    " \
        Backup has been created with name " +
      BackupObject.BackupName +
      ". \
        <br><br><span class=\"setuptools config download acknowledge\">Right click and choose 'Save link as...' to download.</span> \
    "
  );
  ...
  var backupRenameBind = setuptools.app.backups.renameHtml(
    ...
    function (BackupObject) {
      $("a.setuptools.config.download").attr(
        "download",
        "muledump-" + BackupObject.BackupCustomName + ".json"
      );
    },
    true
  );
  ...
  if (setuptools.state.firsttime === true)
    setuptools.lightbox.build(
      "backups-create",
      '<br><br>Welcome to Jakcodex/Muledump :)<br><br>Once ready you must <a href="#" class="setuptools link reload noclose">reload</a> Muledump.'
    );
  if (setuptools.state.firsttime === false)
    setuptools.lightbox.goback("backups-create", setuptools.app.backups.index);
  ...
  if (setuptools.state.firsttime === true)
    $(".setuptools.link.reload").click(function () {
      location.reload();
    });
  await setuptools.storage.write("backupAssistant", Date.now());
  backupRenameBind();
 ...
 */
setuptools.app.backups.cleanup = async function () {
  var backups = setuptools.app.backups.listAll();
  ...
  for (i = backups.length - 1; i >= 0; i--)
    if (backups.hasOwnProperty(i)) {
      var BackupData = JSON.parse(await setuptools.storage.read(backups[i][1]));
      if (
        typeof BackupData.meta.protected === "undefined" ||
        BackupData.meta.protected === false
      )
        Candidates.push(backups[i]);
      if (BackupData.meta.protected === true && BackupData.meta.auto === true)
        Protected.push(backups[i]);
    }

  //  if backup length exceeds the maximum then let's clean up
  if (Candidates.length > setuptools.data.config.maximumBackupCount) {
    window.techlog(
      "SetupTools/Backups Cleaning up " +
        (Candidates.length - setuptools.data.config.maximumBackupCount) +
        " backups",
      "force"
    );
    for (
      var i = setuptools.data.config.maximumBackupCount;
      i < Candidates.length;
      i++
    ) {
      window.techlog(
        "SetupTools/Backups Deleting " + Candidates[i][1],
        "force"
      );
      await setuptools.storage.delete(Candidates[i][1]);
    }
  }
  ...
  if (Protected.length > setuptools.data.config.maximumBackupCount) {
    window.techlog(
      "SetupTools/Backups Cleaning up " +
        (Protected.length - setuptools.data.config.maximumBackupCount) +
        " backups",
      "force"
    );
    for (
      ...
      await setuptools.storage.delete(Protected[i][1]);
    }
  }
};
