...
setuptools.app.introduction = function () {
  ...
  setuptools.lightbox.build(
    ...
    ' \
      ...
      '/docs/setuptools/index" class="setuptools noclose" target="_blank">SetupTools</a> in the docs. \
        ...
        <br><br>Be sure to check out the <a href="' +
      setuptools.config.url +
      '/REQUIREMENTS" target="_blank" class="red">Requirements</a> before starting. \
  ...
  if (typeof setuptools.originalAccountsJS === "object") {
    if (
      setuptools.app.config.validateFormat(setuptools.originalAccountsJS, 0) ===
      true
    ) {
      setuptools.lightbox.build(
        "introduction",
  ...
  setuptools.lightbox.drawhelp(
    ...
    "SetupTools Introduction"
  );
  setuptools.lightbox.settitle(
    "introduction",
    "Muledump SetupTools Introduction"
  );
...
setuptools.app.index = async function (config) {
  ...
  if ((await setuptools.app.checknew()) === true) {
    ...
    setuptools.lightbox.build(
      ...
      " \
            ...
            <br><br><a href=\"" +
        setuptools.config.url +
        '" class="setuptools noclose" target="_blank">Overview</a> | \
  ...
  if ((await setuptools.app.checknew()) === true) {
    setuptools.lightbox.drawhelp(
      "main",
      "docs/setuptools/help/first-time",
      "SetupTools Help"
    );
  } else
    setuptools.lightbox.drawhelp(
      "main",
      "docs/setuptools/help/index",
      "SetupTools Help"
    );
  setuptools.lightbox.display(
    "main",
    $.extend(true, {}, config, { variant: variant })
  );
  $(".setuptools.accounts.manage").on(
    "click.setuptools.manage",
    setuptools.app.accounts.manager
  );
  $(".setuptools.accounts.groups").on(
    "click.setuptools.groups",
    setuptools.app.groups.manager
  );
  $(".setuptools.config.restore").on(
    "click.setuptools.restore",
    setuptools.app.backups.index
  );
  $(".setuptools.config.backup").on(
    "click.setuptools.backup",
    setuptools.app.backups.index
  );
  $(".setuptools.config.settings").on(
    "click.setuptools.settings",
    setuptools.app.config.settings
  );
  $(".setuptools.config.import").on(
    "click.setuptools.import",
    setuptools.app.accounts.AccountsJSImport
  );
  $(".setuptools.app.intro").on(
    "click.setuptools.introduction",
    setuptools.app.introduction
  );
  $(".setuptools.config.totals").on("click.setuptools.totals", function () {
    setuptools.lightbox.insert("totalsmenu-settings", "goback", [
      "totalsmenu-settings",
      setuptools.app.index,
    ]);
    setuptools.app.muledump.totals.menu.settings();
  });
  $(".setuptools.config.ocl").on(
    "click.setuptools.ocl",
    setuptools.app.muledump.ocl.manager
  );
  $(".setuptools.config.devtools").on(
    "click.setuptools.devtools",
    setuptools.app.devtools.ui
  );
};

//  check if this is a new user
setuptools.app.checknew = async function () {
  ...
  return (
    ...
    (typeof setuptools.window.accounts === "object" &&
      ...
      setuptools.window.accounts.email &&
      setuptools.window.accounts.email2)
  );
};
  ...
  //  backups manager
    ...
    if (matches[1] === "upload") {
      ...
      if (BackupList.length > 0) {
        ...
        setuptools.app.backups.restoreConfirm(
          BackupMeta[1],
          BackupMeta[5] || BackupMeta[4]
        );
...
//  wrap analytics var
setuptools.app.ga = async function (
  command,
  action,
  value1,
  value2,
  ...
  if (command === "init") {
    if (
      ...
      (setuptools.data.userid === false ||
        !setuptools.data.userid.match(setuptools.config.regex.gaUserId))
    ) {
      setuptools.app.muledump.notices.add(
        "Review Usage Analytics options",
        setuptools.app.gaReview
      );
      setuptools.data.userid = setuptools.seasalt.hash.sha256(
        Date.now().toString()
      );
      setuptools.data.userid =
        setuptools.data.userid.substr(0, 20) +
        setuptools.data.userid.substr(setuptools.data.userid.length - 20, 20);
      setuptools.app.config.save("UsageAnalytics/first-run");
      if (setuptools.state.loaded === false)
        await setuptools.storage.write("userid", setuptools.data.userid);
      return;
    }
    ...
    if (setuptools.data.config.gaPing === true)
      setuptools.tmp.gaInterval = setInterval(
        setuptools.app.ga,
        setuptools.config.gaInterval,
        "send",
        { hitType: "pageview", page: "#ping" }
      );
    ...
    analytics("send", "event", {
      ...
      eventLabel:
        setuptools.version.major +
        "." +
        setuptools.version.minor +
        "." +
        setuptools.version.patch,
    });
  ...
  if (command === "timing") {
    ...
    if (typeof action === "object") {
      ...
      if (
        Object.keys(
          Object.filter(action, function (key, value) {
            return (
  ...
  if (
    ...
    (action === "event" &&
      [
        ...
        "genericServerError",
        "export-imgur-error",
        "export-imgur-networkError",
        "export-imgur-httpError",
        "export-img-loadError",
    ...
    (action === "event" &&
      ["ocl"].indexOf(value1.eventCategory) > -1 &&
      setuptools.data.config.gaOptions === true) ||
    ...
    (action === "event" &&
      ["runtimeMode", "rateLimited", "stars", "browser"].indexOf(
        value1.eventCategory
      ) > -1) ||
    //  pings require that gaPing be enabled
    (typeof action === "object" &&
      action.hitType === "pageview" &&
      action.page === "#ping" &&
...
setuptools.app.gaReview = function () {
  setuptools.lightbox.build(
    ...
    '\
        <div class="fleft cboth" style="text-align: justify;">\
            ...
            You can read more about the collected data at our <a href="' +
      setuptools.config.url +
      '/privacy-policy" target="_blank">Privacy Policy</a> page. \
            ...
            ' +
      (setuptools.state.loaded === false
        ? "<br><br><strong>Accounts.js Users:</strong><br><br>You can disable Usage Analytics by adding <code>ga = false;</code> to accounts.js."
        : "") +
      "\
            <br>&nbsp;\
        </div>\
  ...
  $(".setuptools.link.gaOptOut").click(function () {
    ...
    setuptools.lightbox.build(
      "gaReview-optOut",
      "Usage Analytics have been disabled."
    );
  ...
  if (
    ...
    ((window &&
      ...
      (typeof navigator === "object" &&
        navigator.hardwareConcurrency < setuptools.config.perfMinCPUs))
  ) {
    setuptools.app.muledump.notices.add(
...
setuptools.app.uaTiming = function (category, key, action, uid, ga) {
  ...
  if (typeof uid === "string") {
    ...
    if (typeof timing.aggregate[nameKey] === "undefined")
      timing.aggregate[nameKey] = new JTimerAggregate();
    aggregate = timing.aggregate[nameKey];
    nameKey += "-" + uid;
  }
  if (typeof ga === "undefined") ga = true;
  if (
    ["stop", "cancel"].indexOf(action) > -1 &&
    typeof timing[nameKey] === "undefined"
  )
    return;
  if (typeof setuptools.tmp.activeRuntime === "undefined")
    setuptools.tmp.activeRuntime = 0;

  //  start a timer
  if (action === "start" && typeof timing[nameKey] === "undefined")
    timing[nameKey] = new JTimer({
      name: nameKey,
      ...
      stopCallback: function (self) {
        //  calculate runtime
        setuptools.tmp.activeRuntime =
          setuptools.tmp.activeRuntime + self.runtime;

        //  send to ga if enabled
        if (self.options.custom.ga === true)
          setuptools.app.ga("timing", {
  ...
  if (action === "start" && timing[nameKey] instanceof JTimer)
    timing[nameKey].restart();
...
setuptools.app.recordConsole = function () {
  setuptools.app.techlog(
    "Muledump/recordConsole background task starting",
    "norecord"
  );
  clearInterval(setuptools.tmp.recordConsoleTask);
  setuptools.tmp.recordConsoleTask = setInterval(async function () {
    if (
      !Array.isArray(setuptools.tmp.consoleHistory) ||
      setuptools.tmp.consoleHistory.length === 0
    )
      return;
    var consoleHistory = JSON.parse(await setuptools.storage.read("console"));
    if (!Array.isArray(consoleHistory)) consoleHistory = [];
    consoleHistory = consoleHistory.concat(setuptools.tmp.consoleHistory);
    setuptools.app.techlog(
      "Muledump/recordConsole committing " +
        setuptools.tmp.consoleHistory.length +
        " records",
      "norecord"
    );
    ...
    if (setuptools.data.config.recordConsole === -1) {
      ...
      if (maxcount < setuptools.config.recordConsoleMinimum)
        maxcount = setuptools.config.recordConsoleMinimum;
      if (maxcount < consoleHistory.length)
        splice = consoleHistory.length - maxcount;
    }

    if (splice > 0) consoleHistory.splice(0, splice);

    //  write history
    await setuptools.storage.write("console", consoleHistory);
...
setuptools.app.techlog = function (msg, type, force) {
  ...
  if (
    (setuptools.data.config.debugging === true &&
      ["info", "norecord"].indexOf(type) > -1) ||
    ["force"].indexOf(type) > -1 ||
    force === true
  )
    console.log(ts, "-", msg);

  if (
    ["norecord", "api", "aggregate"].indexOf(type) === -1 &&
    typeof setuptools.data.config.recordConsole === "number" &&
    setuptools.data.config.recordConsole !== 0
  ) {
    if (!Array.isArray(setuptools.tmp.consoleHistory))
      setuptools.tmp.consoleHistory = [];
...
setuptools.app.help = function () {
  setuptools.lightbox.build(
    ...
    ' \
      ...
      '/Frequently-Asked-Questions" target="_blank">Frequently Asked Questions</a></li> \
            <li><a href="' +
      setuptools.config.wikiUrl +
      '/Muledump-CORS-Adapter" target="_blank">Muledump CORS Adapter</a></li> \
            <li><a href="' +
      setuptools.config.wikiUrl +
      '/Installation-and-Setup" target="_blank">Installation and Setup Guide</a></li> \
            <li><a href="' +
      setuptools.config.wikiUrl +
      '/Steam-Users-Setup-Guide" target="_blank">Steam Users Setup Guide</a></li> \
            <li><a href="' +
      setuptools.config.wikiUrl +
      '/Kongregate-Users-Setup-Guide" target="_blank">Kongregate Users Setup Guide</a></li> \
            <li><a href="' +
      setuptools.config.wikiUrl +
      '/One-Click-Login" target="_blank">One Click Login Guide</a></li> \
            <li><a href="' +
      setuptools.config.wikiUrl +
      '/Totals" target="_blank">Totals Overview</a></li> \
            <li><a href="' +
      setuptools.config.url +
      '/docs/muledump/totals-manager" target="_blank">Totals Detailed Usage Info</a></li>\
            <li><a href="' +
      setuptools.config.url +
      '/docs/setuptools/" target="_blank">Muledump SetupTools Info</a></li>\
            <li><a href="' +
      setuptools.config.wikiUrl +
      '/Rate-Limiting" target="_blank">Rate Limiting</a></li> \
            <li><a href="' +
      setuptools.config.url +
      '#support-and-contributions" target="_blank">Getting Support</a></li>\
        ...
        <div class="help hotkeys">\
            <ol>\
                ...
                <li>Shift+Click a mule to add or remove it from the <a href="' +
      setuptools.config.url +
      '/docs/muledump/totals-manager#account-filter-members" target="_blank">Account Filter</a></li>\
            ...
            <ol> \
                ...
                <li>Shift+Click an item to add it to the <a href="' +
      setuptools.config.url +
      '/docs/muledump/totals-manager#permanently-hidden-items" target="_blank">Hidden Items Filter</a></li>\
            ...
            <ol> \
                ...
                <li>Click and Drag items to sort them in <a href="' +
      setuptools.config.url +
      '/docs/muledump/totals-manager#item-group-sorting" target="_blank">Item Group Sorting</a> section</li>\
  ...
  $(".muledump.link.viewSystemsReport").on(
    "click.muledump.viewSystemsReport",
    function () {
      setuptools.lightbox.close();
      setuptools.lightbox.override(
        "settings-viewSystemsReport",
        "goback",
        setuptools.app.help
      );
      setuptools.app.devtools.viewSystemsReport();
...
setuptools.app.calculateCpuTime = function (pretty) {
  ...
  cpuTime =
    cpuTime >= 60
      ? (cpuTime / 60).toFixed(2) + " Minutes"
      : cpuTime.toFixed(2) + " Seconds";
  return cpuTime;
};
  ...
  $.ajax(setuptools.config.timesyncUrl + "?" + $.param(opts), {
    ...
    success: function (data) {
      if (typeof data === "object") {
        ...
        if (typeof setuptools.tmp.timeOffset !== "number")
          setuptools.tmp.timeOffset = 0;
        setuptools.app.techlog(
          "SetupTools/TimeSync clock offset: " +
            setuptools.tmp.timeOffset +
            "; delay: " +
            timing["system-timesync"].runtime
        );
      }
    },
  });
