(function ($, window) {
  ...
  function gen_option(o, $targ, guid) {
    ...
    if (guid ? window.mules[guid].opt(o) : options[o])
      $inp.attr("checked", "checked");
    // label for checkbox
    var ltext = radio ? opt.label : opt;
    var $lab = $("<label>")
      .attr("for", "check_" + oas + o)
      .text(ltext);
    ...
    for (var i in ritems) {
      if (ritems.hasOwnProperty(i)) {
        ...
        if ((onacc ? mule.opt(rname) : options[rname]) === i)
          $inp.attr("checked", "checked");
        $lab = $("<label>")
          .attr("for", "radio_" + oas + o + i)
          .text(ritems[i]);
  ...
  function option_updated(o) {
    ...
    if (!(o in globalopts)) {
      for (var i in options) {
        ...
        if (typeof options[i] === "object" && o in options[i])
          delete options[i][o];
      }

      for (i in window.mules) {
        if (window.mules.hasOwnProperty(i)) window.mules[i].query(false, true);
      }
    }
  ...
  async function options_init() {
    ...
    var c = await setuptools.storage.read("muledump:options", true);
    if (c) {
      try {
        c = JSON.parse(c);
        for (var k in c) {
          if (c.hasOwnProperty(k)) options[k] = c[k];
        }
      } catch (e) {}
    }
    ...
    if (setuptools.state.hosted === true) {
      ...
      if (typeof options.updatecheck === "undefined")
    ...
    $("#accopts")
      .off(
        "mouseenter.muledump.options.accopts mouseleave.muledump.options.accopts"
      )
      .on("mouseleave.muledump.options.accopts", function () {
        setuptools.tmp.accoptsMouseLeaveTimer = setTimeout(
          function (self) {
            $(self).hide();
          },
          300,
      ...
      .on("mouseenter.muledump.options.accopts", function () {
        clearTimeout(setuptools.tmp.accoptsMouseLeaveTimer);
      });
    ...
    $("#options")
      .off("mouseenter.options.display mouseleave.options.display")
      .hover(
        function () {
    ...
    var ScrollLock = new Muledump_ElementScrollLock(
      "#options",
      ".handle.options",
      { name: "options", autoStart: false }
    );
    $(".handle.options")
      .off("mouseenter.options.menuButton mouseleave.options.menuButton")
      .on("mouseenter.options.menuButton", function () {
  ...
  function options_set(key, value, guid) {
    ...
    if (typeof guid === "string") {
      if (value !== null && typeof options[guid] === "undefined")
        options[guid] = {};
      if (value !== null) {
        options[guid][key] = value;
      } else if (options[guid] && options[guid][key]) delete options[guid][key];
      if (options[guid] && Object.keys(options[guid]).length === 0)
        delete options[guid];
    } else options[key] = value === null ? copy[key] : value;
