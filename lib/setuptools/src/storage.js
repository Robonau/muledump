...
setuptools.storage.db = new Dexie("storageDatabase");

setuptools.storage.db.version(1).stores({
  store: "key, value",
});

//  write to localStorage
setuptools.storage.write = async function (key, value, skipPrefix) {
  //  check if storage encryption is enabled
  if (
    setuptools.state.encryption.storage === true &&
    setuptools.config.noEncryptKeys.indexOf(key) === -1
  ) {
    key =
      setuptools.config.keyPrefix +
      ":encrypted:" +
      setuptools.seasalt.hash.sha512(key);
    skipPrefix = true;
  }

  var data = value;
  if (Array.isArray(data) === true || typeof data === "object")
    data = JSON.stringify(data, true, 5);
  ...
  try {
    ...
    await setuptools.storage.db.store.put({
      key: key,
      value: setuptools.storage.compression.deflate(data),
    });
  } catch (e) {
    return false;
  }
...
setuptools.storage.read = async function (key, skipPrefix) {
  //  check if storage encryption is enabled
  if (
    setuptools.state.encryption.storage === true &&
    setuptools.config.noEncryptKeys.indexOf(key) === -1
  ...
  try {
    ...
    result = setuptools.storage.compression.inflate(
      (await setuptools.storage.db.store.where("key").equals(key).first()).value
    );
  } catch (e) {}
...
setuptools.storage.delete = async function (key, skipPrefix) {
  //  check if storage encryption is enabled
  if (
    setuptools.state.encryption.storage === true &&
    setuptools.config.noEncryptKeys.indexOf(key) === -1
  ...
  try {
    ...
    await setuptools.storage.db.store.where("key").equals(key).delete();
  } catch (e) {
    return false;
  }
  return true;
...
setuptools.storage.test = async function () {
  if (setuptools.config.devForcePoint === "storage-test") return false;
  await setuptools.storage.write("test", "test");
  console.log(await setuptools.storage.write("test", "test"));
  console.log(await setuptools.storage.read("test"));
  return (await setuptools.storage.read("test")) === "test";
};
...
setuptools.storage.report = function (options) {
  ...
  for (var key in window.localStorage) {
    if (window.localStorage.hasOwnProperty(key)) {
      keys[key] = Number(
        ((window.localStorage[key].length * 2) / 1024).toFixed(2)
      );
      used += keys[key];
      var decompressed = setuptools.storage.compression.inflate(
  ...
  for (var key2 in keys) {
    if (keys.hasOwnProperty(key2)) {
      if (
        key2.match(
          /^muledump:(.*?@.*|(?:steamworks|kongregate|kabam):.+)$/i
        ) !== null &&
        key2.indexOf("muledump:setuptools") === -1
      )
        data["muledump-accountDataCache"] += Number(keys[key2].toFixed(2));
      if (key2.match(/^muledump:setuptools:(?:preview:)?.+$/i) !== null)
        data["setuptools-all"] += Number(keys[key2].toFixed(2));
      if (
        key2.match(/^muledump:setuptools:(?:preview:)?configuration$/i) !== null
      )
        data["setuptools-config"] += Number(keys[key2].toFixed(2));
      if (
        key2.match(/^muledump:setuptools:(?:preview:)?muledump-backup-.+$/i) !==
        null
      )
        data["setuptools-backups"] += Number(keys[key2].toFixed(2));
      if (
        key2.match(/^muledump:setuptools:(?:preview:)?mulequeue:.+$/i) !== null
      )
        data["setuptools-mulequeue"] += Number(keys[key2].toFixed(2));
      if (key2.match(/^muledump:setuptools:(?:preview:)?cache:.+$/i) !== null)
        data["setuptools-cache"] += Number(keys[key2].toFixed(2));
      data.all += Number(keys[key2].toFixed(2));
    }
  }

  for (var key3 in data)
    if (data.hasOwnProperty(key3)) data[key3] = Number(data[key3].toFixed(2));

  data.other = Number(
    (
      data.all -
      data["setuptools-all"] -
      data["muledump-accountDataCache"]
    ).toFixed(2)
  );

  data.free = Number(((1 - data.all / maximum) * 100).toFixed(2));
  if (data.all >= maximum * 0.8)
    warn.storageSpace =
      "Local Storage is low on free space (" + data.free + "% free)";
  if (data.all >= maximum * 0.93)
    warn.storageSpace =
      "Local Storage is very low on free space (" + data.free + "% free)";
  if (data.all >= maximum * 0.95)
    warn.storageSpace = "Local Storage is full (" + data.free + "% free)";
...
---------------------------------------------\n\
Muledump Account Data:         " +
        data["muledump-accountDataCache"] +
        " KB\n\
SetupTools Configuration:      " +
        data["setuptools-config"] +
        " KB\n\
SetupTools Backups:            " +
        data["setuptools-backups"] +
        " KB\n\
SetupTools MuleQueue:          " +
        data["setuptools-mulequeue"] +
        " KB\n\
SetupTools Cache:              " +
        data["setuptools-cache"] +
        " KB\n\
SetupTools Total Data Usage:   " +
        data["setuptools-all"] +
        " KB\n\
Other Usage:                   " +
        data.other +
...
" +
  ...
  return {
    ...
    keys: keys,
    summary: data,
    warning: typeof warn.storageSpace === "string",
    warningText: warn.storageSpace,
  };
};
...
setuptools.storage.crypto.encrypt = function (string, key) {
  ...
  if (typeof key === "undefined")
    key = setuptools.app.mulecrypt.config.keychain.active;
 ...
 */
setuptools.storage.compression.formats = $.extend(
  true,
  {},
...
setuptools.storage.compression.test = function (f) {
  ...
  var format = Object.keys(setuptools.storage.compression.formats)[
    f || setuptools.config.compressionFormat
  ];
  ...
  setuptools.app.techlog(
    "Storage/Compression - selfTest is running with format: " + format
  );
  ...
  //  perform the test
  try {
    var deflated = setuptools.storage.compression.formats[format].deflate(
      string,
      null,
      true
    );
    var inflated = setuptools.storage.compression.formats[format].inflate(
      deflated,
      null,
      true
    );
    ...
    setuptools.app.techlog(
      "Storage/Compression - selfTest encountered an error: " + e
    );
    setuptools.storage.compression.selfTest = false;
    return false;
  }
  ...
  if (setuptools.storage.compression.selfTest === false) {
    ...
    setuptools.app.techlog(
      "Storage/Compression - selfTest returned true: " +
        ((1 - deflated.length / string.length) * 100).toFixed(2) +
        "%"
    );
  }
...
setuptools.storage.compression.deflate = function (input, confirm, force) {
  ...
  if (!(confirm === null && force === true)) {
    if (force === true)
      setuptools.app.techlog(
        "Storage/Compression - this is not the way to bypass selfTest"
      );
    if (setuptools.storage.compression.selfTest === undefined)
      setuptools.storage.compression.test();
  ...
  try {
    var compressed =
      setuptools.storage.compression.formats[format].deflate(input);
  } catch (e) {
    setuptools.app.techlog(
      "Storage/Compression - failed to compress input: " + e
    );
    return input;
  }
...
setuptools.storage.compression.inflate = function (deflated) {
  ...
  if (typeof setuptools.storage.compression.formats[format] !== "object") {
    setuptools.app.techlog(
      "Storage/Compression - supplied input format is not supported: " +
        +meta[1]
    );
  ...
  try {
    var decompressed = setuptools.storage.compression.formats[format].inflate(
      deflated.substr(5)
    );
  } catch (e) {
    setuptools.app.techlog(
      "Storage/Compression - failed to decompress input: " + e
    );
    return undefined;
  }
  return decompressed;
    ...
    if (localStorage.hasOwnProperty(key))
      if (
        (setuptools.data.config.compression === false &&
          setuptools.storage.compression.isCompressed(
