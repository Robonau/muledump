...
setuptools.lightbox.menu.context.create = function (
  name,
  nested,
  track,
  options,
  ...
  if (
    typeof setuptools.tmp.contextMenuOpen === "object" &&
  ...
  var bindings = [];
  var menu = $(
    '<div class="setuptools div menu noselect scrollbar ' + name + '">'
  );
  var html = "";
  var customPos = {
    h: "left",
  ...
  for (var i in options) {
    if (options.hasOwnProperty(i)) {
      ...
      if (typeof options[i].override === "string")
        options[i].override = [options[i].override];
      if (Array.isArray(options[i].override) === false)
        options[i].override = [];

      //  default format is a link and it includes no option key
      if (!options[i].option) {
        html +=
          '<div class="link"><a href="#" class="setuptools ' +
          options[i].class +
          '" ' +
          attributes +
          ">" +
        ...
        //  create an actual link
          ...
          html +=
            ...
            options[i].href +
            '" class="setuptools menuLink ' +
            options[i].class +
            '" ' +
            (options[i].target ? 'target="' + options[i].target + '"' : "") +
            " " +
            featherlightAttr +
            ">" +
            options[i].name +
        ...
        //  modify default positioning parameters
          ...
          if (options[i].vpx && options[i].hpx) {
            ...
            if (
              typeof position[options[i].v] === "string" &&
              typeof options[i].vpx !== "undefined"
            )
              position[options[i].v] = options[i].vpx;
            if (
              typeof position[options[i].h] === "string" &&
              typeof options[i].hpx !== "undefined"
            )
              position[options[i].h] = options[i].hpx;
          }
        }
        ...
        //  allow an option for closing on mouseleave
          ...
          if (options[i].action === "close") {
            doHoverClose =
              typeof options[i].delay === "number" ? options[i].delay : 500;
            doHoverCloseTimer =
              typeof options[i].timer === "string"
                ? options[i].timer
                : "menuHoverClose";
          }
        } else if (options[i].option === "close") {
          if (typeof options[i].callback === "function")
            doClose.callback = options[i].callback;
          doClose.delay =
            typeof options[i].delay === "number" ? options[i].delay : 0;
        }
        //  allow skipping actions
        else if (options[i].option === "skip") {
          if (typeof options[i].value === "string") {
            if (skipOption[options[i].value] === false)
              skipOption[options[i].value] = true;
          }
        }
        ...
        //  allow adding custom classes
          ...
          if (typeof options[i].value === "string")
            customClasses = options[i].value;
        }
        //  side channel additions
        else if (options[i].option === "channel") {
          if (typeof options[i].keyup === "function")
            channel.keyup = options[i].keyup;
        }
        //  support input options
        else if (options[i].option === "input") {
          html += '<div class="input">' + options[i].value + "</div>\n";
          if (typeof options[i].binding === "function")
            options[i].binding = [options[i].binding];
          if (typeof options[i].binding === "object")
            bindings = bindings.concat(options[i].binding);
        }
        //  support select options
          ...
          html +=
            '<div class="mb5" style="color: #0094ff; font-weight: bold; z-index: 5000;">' +
            options[i].name +
            "</div>";
          html +=
            '<select name="stmenu-' +
            options[i].name +
            '" class="' +
            (options[i].class ? options[i].class : "") +
            '">\n';
          var selected;
        ...
        //  watch for content state
          ...
          if (
            ...
            watch = {
              ...
              value: options[i].value,
              callback:
                typeof options[i].callback === "function"
                  ? options[i].callback
                  : function () {
                      setuptools.lightbox.menu.context.close(name);
                      return false;
        ...
        //  override the default action of closing the menu on interaction
          ...
          if (typeof options[i].callback === "function")
            doAfterClose = options[i].callback;
          if (typeof options[i].callbackArg === "string")
            doAfterCloseArgs = [options[i].callbackArg];
          if (Array.isArray(options[i].callbackArg) === true)
            doAfterCloseArgs = options[i].callbackArg;
        }
        //  context menus on menu items
        else if (options[i].option === "context") {
          if (typeof options[i].callback === "function")
            doContext = options[i].callback;
        }
      }
    }
  }
  ...
  //  display the menu
  if (
    typeof customClasses === "string" &&
    customClasses.match(/smallMenuCells/) === null
  )
    customClasses += " smallMenuCells";
  ...
  if (typeof setuptools.tmp.contextSelf !== "object")
    setuptools.tmp.contextSelf = {};
  if (typeof setuptools.tmp.contextMenuOpen !== "object")
    setuptools.tmp.contextMenuOpen = {};
  setuptools.tmp.contextSelf[name] = self;
  setuptools.tmp.contextMenuOpen[name] = true;

  //  process custom bindings
  if (typeof bindings === "object")
    for (var i in bindings) if (bindings.hasOwnProperty(i)) bindings[i]();
  ...
  function findPosition() {
    ...
    if (typeof watch === "object") {
      ...
      //  select the element
      watch.track = $(
        "." + watch.class + "[" + watch.attr + '="' + watch.value + '"]'
      );

      //  watch for: missing
      if (watch.for === "missing" && watch.track.length === 0) {
        window.techlog(
          "Lightbox/ContextMenu - findPosition - executing watch callback",
          "force"
        );
    ...
    if (
      ...
      if (!reposition) {
        window.techlog(
          "Lightbox/ContextMenu - findPosition - closing on reposition init",
          "force"
        );
        setuptools.lightbox.menu.context.close(name);
        return;
      ...
      track = $(
        "." +
          reposition.parentClass +
          "[" +
          reposition.attr +
          '="' +
          reposition.value +
          '"]'
      ).find("." + reposition.menuClass);
      position = track.offset();
      if (typeof position !== "object") {
        window.techlog(
          "Lightbox/ContextMenu - findPosition - closing on reposition failure",
          "force"
        );
        setuptools.lightbox.menu.context.close(name);
        return;
    ...
    if (skipOption.reposition === false) {
      ...
      if (
        customPos.v !== "bottom" &&
        position.top + height >= w.pageYOffset - w.outerHeight(true)
      ) {
        css.top = "initial";
      ...
      //  adjust menu position if this is going to render right of the window
        ...
        customPos.h !== "right" &&
        position.left + width >= w.pageXOffset - w.outerWidth(true)
      ) {
        css.left = "initial";
    ...
    if (skipOption.reposition !== true && !setuptools.tmp.intervalContext[name])
      setuptools.tmp.intervalContext[name] = setInterval(findPosition);
  }
  ...
  $(".setuptools.div.menu." + name + " a.menuLink").click(function () {
    if (typeof doClose.callback === "function") {
      if (
        !setuptools.tmp.doCloseSkip ||
        (Array.isArray(setuptools.tmp.doCloseSkip) &&
          setuptools.tmp.doCloseSkip.indexOf(name) === -1)
      ) {
        ...
        delete setuptools.tmp.doCloseSkip[
          setuptools.tmp.doCloseSkip.indexOf(name)
        ];
  ...
  if (typeof doContext === "function")
    cell.contextmenu(function (e) {
      doContext(this, e);
    });

  //  bind scrollLock
  if (typeof doScrollLock === "string")
    doScrollLock = new Muledump_ElementScrollLock(
            ...
            if (options.hasOwnProperty(a)) {
              if (MenuOption.hasClass(options[a].class)) {
                ...
                if (option === "LinkOption") {
                  ...
                  if (
                    typeof doAfterClose === "function" &&
                    options[a].override.indexOf("afterClose") === -1
                  )
                    doAfterClose.apply(null, doAfterCloseArgs);
                  return;
  ...
  if (doHoverClose) {
    $(".setuptools.div.menu." + name)
      ...
      .on("mouseenter", function () {
        clearTimeout(setuptools.tmp.muleMenuMouseLeaveTimer);
      });
  }
...
setuptools.lightbox.menu.context.closeSkip = function (name) {
  if (typeof setuptools.tmp.doCloseSkip === "undefined")
    setuptools.tmp.doCloseSkip = [];
  if (setuptools.tmp.doCloseSkip.indexOf(name) === -1)
    setuptools.tmp.doCloseSkip.push(name);
};
...
setuptools.lightbox.menu.context.keyup = function (name, e, selectorSuffix) {
  ...
  if (e.keyCode === 38 || e.keyCode === 40) {
    if (setuptools.tmp.contextMenuOpen[name] === true) {
      var menuSelector = $(
        ".setuptools.div.menu." + name + " > div.link > a" + selectorSuffix
      );
      if (menuSelected.length === 0) {
        menuSelected =
          e.keyCode === 38 ? menuSelector.last() : menuSelector.first();
...
setuptools.lightbox.menu.context.close = function (name, keep) {
  ...
  if (setuptools.tmp.contextSelf) {
    ...
    if (
      !keep &&
      (setuptools.tmp.intervalContext[name] ||
        setuptools.tmp.contextMenuOpen[name] === true)
    ) {
      CloseMenu(name);
      return;
    ...
    for (var i in setuptools.tmp.contextMenuOpen)
      if (setuptools.tmp.contextMenuOpen.hasOwnProperty(i))
        if (!keep || (keep === true && i !== name)) CloseMenu(i);
  }
};

//  create a pagination object
setuptools.lightbox.menu.paginate.create = function (
  PageList,
  ActionItem,
  ActionContainer,
  ActionSelector,
  ...
  if (typeof Modifiers.search.key === "undefined")
    Modifiers.search.key = ["username"];
  if (typeof Modifiers.search.placeholder === "undefined")
    Modifiers.search.placeholder = "Search by Name";

  var lastPageRaw = Number(
    PageList.length /
      (Modifiers.recordsPerPage || setuptools.data.config.accountsPerPage)
  );
  var currentPage =
    typeof setuptools.lightbox.menu.paginate.state[ActionContainer] === "object"
      ? setuptools.lightbox.menu.paginate.state[ActionContainer].currentPage
      : 0;
  ...
  state.html.menu =
    ' \
        <div class="setuptools div pageControls full"> \
            ...
            <div class="editor control customPage"><input name="customPage" value="' +
    (state.currentPage > -1 ? state.currentPage + 1 : 1) +
    '"><span class="noselect"> of ' +
    (state.lastPage + 1) +
    " page" +
    (state.lastPage + 1 > 1 ? "s" : "") +
    '</span></div> \
            ...
            ' +
    (typeof state.Modifiers.pageButtons === "object" &&
    typeof state.Modifiers.pageButtons.html === "string"
      ? state.Modifiers.pageButtons.html
      : "") +
    " \
        </div> \
    ";
  ...
  state.html.search =
    ' \
        <div class="setuptools div pageControls">\
            <div class="editor control searchName" title=""><input name="searchName" placeholder="' +
    state.Modifiers.search.placeholder +
  ...
  state.bind = function (ActionContextOptions, skip) {
    ...
    setuptools.lightbox.menu.paginate.pageUpdate(
      false,
      ActionContext,
    ...
    //  goto page
    $(
      "div." + ActionContainer + " div.setuptools.div.pageControls div.gotoPage"
    ).click(function () {
      var inputSelector =
        "div." + ActionContainer + ' div.customPage input[name="customPage"]';
    ...
    $(
      "div." +
        ActionContainer +
        " div.setuptools.div.pageControls div.firstPage"
    ).click(function () {
    ...
    //  last page
    $(
      "div." + ActionContainer + " div.setuptools.div.pageControls div.lastPage"
    ).click(function () {
    ...
    //  next page
    $(
      "div." + ActionContainer + " div.setuptools.div.pageControls div.nextPage"
    ).click(function () {
    ...
    $(
      "div." +
        ActionContainer +
        " div.setuptools.div.pageControls div.previousPage"
    ).click(function () {
    ...
    if (state.Modifiers.search.container !== false)
...
setuptools.lightbox.menu.search.searchExecute = function (
  state,
  searchTerm,
  skip
) {
  ...
  var searchName = $(
    (typeof state.Modifiers.search.container === "string"
      ? state.Modifiers.search.container
      : "div." + state.ActionContainer) +
      ' div.searchName input[name="searchName"]'
  );
  ...
  setuptools.lightbox.menu.paginate.findPage(
    searchIndex,
    state.ActionContainer
  );

  //  close and update
  searchName.val("");
  if (skip === true) return;
  $(state.ActionSelector).html(
    state.ActionCallback(state.ActionItem, state.currentPage)
  );
  setuptools.lightbox.menu.paginate.pageUpdate(
    true,
    state.ActionContext,
    state.ActionContextOptions
  );
};

//  bind search by name ui elements
setuptools.lightbox.menu.search.bind = function (
  state,
  skip,
  altContainer,
  altPosition,
  ...
  var searchName = $(
    (typeof state.Modifiers.search.container === "string"
      ? state.Modifiers.search.container
      : container) + ' div.searchName input[name="searchName"]'
  );

  //  specify a default search execute method if none provided
  if (typeof state.Modifiers.search.execute === "undefined")
    state.Modifiers.search.execute =
      setuptools.lightbox.menu.search.searchExecute;
  ...
  searchName.unbind("keyup").keyup(function (e) {
    ...
    if (!altContainer || state.Modifiers.search.keyup === true)
      setuptools.lightbox.menu.context.keyup(name, e, ".searchExecuteByName");
    ...
    for (var i in PageList) {
      if (PageList.hasOwnProperty(i)) {
        ...
        if (value.length > 1 && PageList[i].match(exactRegex))
          hits.exact.push(PageList[i]);

        //  partial match occurring
        var partialRegex = new RegExp(".*?" + value + ".*?", "i");
        if (
          hits.exact.indexOf(PageList[i]) === -1 &&
          value.length > 1 &&
          PageList[i].match(partialRegex)
        )
          hits.partial.push(PageList[i]);
    ...
    if (hits.partial.length > 0 || hits.exact.length > 0) {
      ...
      if (Array.isArray(state.Modifiers.search.skip) === true) {
        for (var i = 0; i < state.Modifiers.search.skip.length; i++)
          options.push({
            option: "skip",
      ...
      if (
        Array.isArray(state.Modifiers.search.skip) === true &&
        state.Modifiers.search.skip.indexOf("reposition") > -1
      )
        options.push({
          option: "skip",
          value: "reposition",
      ...
      if (typeof state.Modifiers.search.css === "object")
        options.push({
          option: "css",
          css: state.Modifiers.search.css,
        });

      if (typeof state.Modifiers.search.class === "string")
        options.push({
          option: "class",
  ...
  searchName.unbind("blur").blur(function () {
    ...
    setTimeout(
      function () {
        ...
        if (
          child.hasClass("link") === false &&
          child.parent().hasClass("link") === false
        ) {
          setuptools.lightbox.menu.context.close("search");
          self.val("");
        }
...
setuptools.lightbox.menu.paginate.findPage = function (
  searchIndex,
  ActionContainer
) {
  var state = setuptools.lightbox.menu.paginate.state[ActionContainer];
  ...
  if (typeof searchIndex === "string")
    searchIndex = PageList.indexOf(searchIndex);
  state.lastPage = PageList.length / setuptools.data.config.accountsPerPage;
  state.currentPage = Math.floor(
    state.lastPage * (searchIndex / PageList.length)
  );
...
setuptools.lightbox.menu.paginate.pageUpdate = function (
  close,
  ActionContext,
  ActionContextOptions
) {
...
setuptools.lightbox.expander = function (target, button, config) {
  ...
  $(button).on("click.lightbox.expander", function () {
    ...
    if (box.hasClass("expanded") === false) {
      if (config.collapseHeight === undefined)
        config.collapseHeight = box.outerHeight();
      box.animate({ height: height }, function () {
        $(this)
          .css({ "overflow-y": config.transOverflow })
          .addClass("expanded " + config.customClass);
      });
      ...
      box.animate({ height: config.collapseHeight }, function () {
        $(this)
          .css({ "overflow-y": config.initialOverflow })
          .removeClass("expanded " + config.customClass);
      });
...
setuptools.lightbox.tooltip = function (parent, content, modifiers) {
  ...
  if (typeof modifiers.css === "object")
    css = $.extend(true, css, modifiers.css);

  //  adjust menu position if this is going to render right of the window
  var parentOuter = parent !== false ? parent.outerWidth() : 0;
  var width = tooltip.outerWidth(true);
  var height = tooltip.outerHeight(true);
  if (position.left + width >= window.innerWidth - 20)
    css.left = css.left - width + parentOuter;
  if (css.top < window.pageYOffset)
    css.top += ((parent && parent.outerHeight(true)) || 0) + height;
...
setuptools.lightbox.status = function (self, string, duration, id, callback) {
  ...
  if (!Array.isArray(setuptools.tmp.lightboxStatus))
  ...
  self.find("span").fadeOut(duration, function () {
    $(this).remove();
    self.text(text);
    setuptools.tmp.lightboxStatus.splice(
      setuptools.tmp.lightboxStatus.indexOf(id),
      1
    );
...
//  create a lightbox
setuptools.lightbox.create = async function (data, config, title, page) {
  ...
  if (typeof data === "string") {
    ...
    if (typeof config !== "object") {
      ...
      if ((await setuptools.app.checknew()) === true) {
        if (typeof config.closeIcon === "undefined") config.closeIcon = "";
        if (typeof config.closeOnClick === "undefined")
          config.closeOnClick = false;
      }

      if (typeof config.closeOnEsc === "undefined") config.closeOnEsc = false;
      if (typeof config.otherClose === "undefined")
        config.otherClose =
          "a.setuptools:not(.noclose), .setuptools.error, .setuptools.link:not(.noclose)";
      ...
      if (typeof config.closeOnClick === "undefined")
        config.closeOnClick = "background";
      ...
      if (typeof config.beforeOpen === "undefined")
        config.beforeOpen = function () {
          setuptools.lightbox.menu.context.close();
        };
      if (typeof config.afterOpen === "undefined")
...
setuptools.lightbox.display = function (page, config) {
  ...
  //  create the lightbox from the build data
    ...
    setuptools.lightbox.inserts = setuptools.lightbox.inserts.filter(function (
      data
    ) {
        ...
        if (typeof setuptools.lightbox.builds[page][i] === "object") {
          ...
          if (setuptools.lightbox.builds[page][i].iam === "goback") {
            ...
            if (
              ...
              setuptools.lightbox.overrides[page]["goback"]
            ) {
              gobackData.text1 =
                typeof setuptools.lightbox.overrides[page]["goback"].data ===
                  "object" &&
                setuptools.lightbox.overrides[page]["goback"].data.text1
                  ? setuptools.lightbox.overrides[page]["goback"].data.text1
                  : gobackData.text1;
              gobackData.text2 =
                typeof setuptools.lightbox.overrides[page]["goback"].data ===
                  "object" &&
                setuptools.lightbox.overrides[page]["goback"].data.text2
                  ? setuptools.lightbox.overrides[page]["goback"].data.text2
                  : gobackData.text2;
              gobackData.callback =
                setuptools.lightbox.overrides[page]["goback"].callback;
            }
              ...
              ' \
                            <div class="setuptools bottom container w100 cboth"> \
                                <div class="cfleft h100"> \
                                    ...
                                    ' +
              gobackData.text1 +
              ' <a href="#" class="setuptools goback">' +
              gobackData.text2 +
              "</a> \
                                </div> \
                            </div> \
            ...
            var url =
              (drawhelpData.link.match(/^https?:\/\/.*$/i) === null
                ? setuptools.config.url + "/"
                : "") + drawhelpData.link.replace(/.md$/i, "");
            setuptools.lightbox.builds[page].push(
              ' \
                            <a class="drawhelp docs' +
                (setuptools.state.firsttime === true ? " noclose" : "") +
                '" \
                            title="' +
                drawhelpData.title +
                '" \
    ...
    if (setuptools.config.goHomeSkip.indexOf(page) === -1) {
      ...
      setuptools.lightbox.builds[page].push(
        '<a href="#" class="drawhelp gohome" title="Home"' +
          (typeof drawhelpData === "object"
            ? ' style="right: ' + widthOffset + 'px;"'
            : "") +
          ">&#9751;</a>"
      );
    }
    ...
    setuptools.lightbox.create(
      setuptools.lightbox.builds[page].join(""),
      config,
      title,
      page
    );
    ...
    if (typeof gobackData === "object")
      $(".setuptools.goback").click(function (e) {
        e.preventDefault();
        gobackData.callback();
      });

    //  bind any help button
    if (typeof drawhelpData === "object")
      $(".drawhelp.docs").click(function (e) {
        if (
          ...
          !$(e.target)
            ...
            .match(
              new RegExp(
                "^https?://" + setuptools.config.hostedDomain + "/.*$",
                "i"
              )
            )
        )
...
setuptools.lightbox.override = function (
  targetPage,
  targetAction,
  callback,
  data
) {
  if (typeof setuptools.lightbox.overrides[targetPage] === "undefined")
    setuptools.lightbox.overrides[targetPage] = {};
...
setuptools.lightbox.cancel = function (page) {
  return (
    typeof setuptools.lightbox.builds[page] === "object" &&
    setuptools.lightbox.builds[page].splice(0)
  );
};
...
setuptools.lightbox.drawhelp = function (page, link, title) {
  if (!page || !link || !title)
    setuptools.lightbox.error("Invalid data supplied to drawhelp.", 11);
  if (!setuptools.lightbox.builds[page]) setuptools.lightbox.builds[page] = [];
  setuptools.lightbox.builds[page].push({
    iam: "drawhelp",
    link: link,
...
setuptools.lightbox.goback = function (page, callback, text1, text2) {
  ...
  if (typeof callback !== "function")
    setuptools.lightbox.error(
      "The callback value for goback is not valid.",
      10
    );
...
setuptools.lightbox.insert = function (page, callback, args) {
  ...
  if (
    setuptools.lightbox.inserts.filter(function (data) {
      return !(
        data[0] !== page ||
        (data[0] === page && data[1] !== callback) ||
        (data[0] === page &&
          data[1] === callback &&
          JSON.stringify(data[2]) !== JSON.stringify(args))
      );
    }).length > 0
  )
    return false;
...
setuptools.lightbox.error = function (message, code) {
  ...
  setuptools.lightbox.create(
    ' \
        <p><span class="setuptools error">Error ' +
      code +
      "</span> - " +
      message +
      '</p> \
...
setuptools.lightbox.drawHelp = function (e, drawhelpData, self, options) {
  ...
  function AjaxFailure(url, drawhelpData, LightboxConfig) {
    setuptools.lightbox.build(
      "drawhelp-error",
      'Failed to load help docs. <br><br><a href="' +
        url +
        '" target="_blank">Click here</a> to go to the help doc page.'
    );
    if (setuptools.state.hosted === false)
      setuptools.lightbox.build(
        "drawhelp-error",
        "<br><br>If all else fails check the docs/ folder in your Muledump install."
      );
    if (options.net === true)
      setuptools.lightbox.build(
        "drawhelp-error",
        "<br><br>Finally, the specific error indicates you may be having an Internet connection issue."
      );
  ...
  var LightboxConfig = {
    variant: "setuptools-medium nobackground",
    openSpeed: 0,
    closeSpeed: 0,
  ...
  $.ajax(url)
    .done(function (data) {
      ...
      if (
        setuptools.config.devForcePoint !== "drawhelp-ajax" &&
        HelpSection.find("div#title")
      ) {
        ...
        HelpSection.find("a").each(function () {
          ...
          if (!Href.match(/^(?:http|#)/i)) {
            ...
            $(this).attr(
              "href",
              setuptools.config.url.replace(/(muledump-preview|muledump)/, "") +
                Href
            );
          }
        ...
        if (options.goback)
          setuptools.lightbox.build(
            "drawhelp",
            '<a href="#" class="drawhelp goback" title="Start of Help"' +
              (typeof options.goback === "object"
                ? ' style="right: 30px;"'
                : "") +
              ">&#9751;</a>"
          );
        ...
        setuptools.lightbox.build(
          "drawhelp",
          '<div id="drawhelpBox">' + HelpSection.html() + "</div>"
        );
        ...
        $("a.drawhelp.goback").on("click.drawhelp.goback", function (e) {
          setuptools.lightbox.drawHelp(
            e,
            setuptools.tmp.goback.data,
            setuptools.tmp.goback.self
          );
        });
    ...
    .fail(function () {
      AjaxFailure(url, drawhelpData, LightboxConfig, true);
    });
};
