...
setuptools.app.assistants.account = function (url, extraopts, callback) {
  ...
  if (
    !extraopts.type ||
    typeof setuptools.state.assistant[extraopts.type] !== "boolean"
  ) {
    window.techlog(
      "MD/accountAssistant encountered unknown type " + extraopts.type,
      "force"
    );
    return;
  }
  ...
  if (
    extraopts.type === "ageVerify" &&
    setuptools.state.assistant[extraopts.type] === false
  ) {
    setuptools.lightbox.build(
      "muledump-account-assistant",
      ' \
				You need to verify your age in order to use this account. \
				<br><br>Click to submit: <a href="' +
        url +
        '" target="_blank" class="setuptools app link popupComplete">' +
        url +
        '</a> \
    ...
    setuptools.lightbox.settitle(
      "muledump-account-assistant",
      "Kongregate Age Verification"
    );
    setuptools.lightbox.drawhelp(
      "muledump-account-assistant",
      "docs/muledump/ageVerify",
    ...
    setuptools.lightbox.build(
      ...
      " \
        ...
        '</strong>\
				<br><br>Click to submit: <a href="' +
        url +
        '" target="_blank" class="setuptools app link popupComplete">ROTMG Account TOS Verification</a> \
				<br><br>You can disable the account assistant in the <a href="#" class="setuptools app link settingsmanager">Settings Manager</a>. \
			'
    );
    setuptools.lightbox.settitle(
    ...
    setuptools.lightbox.drawhelp(
      "muledump-account-assistant",
      "docs/muledump/tos",
      "TOS Verification Help"
    );
  } else if (
    extraopts.type === "migration" &&
    setuptools.state.assistant[extraopts.type] === false
  ) {
    setuptools.lightbox.build(
      "muledump-account-assistant",
    ...
    setuptools.lightbox.drawhelp(
      "muledump-account-assistant",
      "docs/muledump/migration",
      "Account Migration Help"
    );
  } else {
    //  hmm
    return;
  ...
  setuptools.lightbox.display("muledump-account-assistant", {
  ...
  $(".setuptools.link.settingsmanager").click(function () {
    setuptools.app.config.settings("accountAssistant");
  });
  if (typeof callback === "function")
    $(".setuptools.link.popupComplete").click(function (e) {
      $.get($(this).attr("href"));
      callback(e);
    });
};
...
setuptools.app.assistants.cors = function (force) {
    ...
    " \
      ...
      ["chrome", "opera"].indexOf(setuptools.browser) > -1
        ? ' \
            ...
            <br><br>Please consider upgrading to <a href="https://www.firefox.com/" target="_blank">Firefox</a> and use <a href="' +
          setuptools.config.url +
          '/muledump.html" target="_blank">Muledump Online</a>.  \
      ...
      (setuptools.state.extension !== true &&
      ["firefox"].indexOf(setuptools.browser) > -1
        ? ' \
  ...
  if (setuptools.data.config.corsAssistant === 1)
    setuptools.lightbox.build(
      "muledump-cors",
      ' \
...
setuptools.app.assistants.xhrError = function (xhr) {
  ...
  setuptools.lightbox.build(
    ...
    ' \
        ...
        <br><br><strong class="negative">' +
      message +
      '</strong> \
        <br><br>Please try again. <a href="' +
      setuptools.config.httpErrorHelp +
      '" target="_blank">Click here</a> for more information.\
    '
  );
  setuptools.lightbox.display("muledump-xhrError", {
    afterClose: function () {
      setuptools.state.notifier = false;
    },
  });
...
setuptools.app.assistants.backups = async function (manual) {
  if (
    setuptools.data.config.backupAssistant === 0 ||
    setuptools.state.notifier === true
  )
    return;
  var lastBackup = await setuptools.storage.read("backupAssistant");
  if (
    manual === true ||
    !lastBackup ||
    Date.now() - lastBackup >= setuptools.data.config.backupAssistant * 86400000
  ) {
    setuptools.state.notifier = true;
    setuptools.lightbox.build(
      "backup-assistant",
...
setuptools.app.assistants.rendersupdate = function (data) {
    ...
    ' \
      ...
      ' \
            ...
            <div class="flex-container" style="justify-content: space-evenly;">\
                ...
                <div style="width: 50%;"><span style="font-weight: bold;">' +
      data.currentRenders +
      '</span> <span class="generic text dark">(' +
      data.currentRendersDate.toLocaleString() +
      ')</span></div>\
            </div>\
            <div class="flex-container" style="justify-content: space-evenly;">\
      ...
      data.latestRenders +
      '</span> <span class="generic text dark">(' +
      data.latestRendersDate.toLocaleString() +
      ")</span></div>\
            </div>\
    "
  );
    ...
    setuptools.lightbox.build(
      ...
      ' \
            ...
            <div class="flex-container">\
                <a download="renders.png" href="' +
        setuptools.config.githubRawUrl +
        "/" +
        data.commit.sha +
        '/lib/renders.png"><div class="setuptools link menuStyle menuSmall bright noclose textCenter">\
                    /lib/renders.png\
                </div></a>\
                <a download="constants.js" href="' +
        setuptools.config.githubRawUrl +
        "/" +
        data.commit.sha +
        '/lib/constants.js"><div class="setuptools link menuStyle menuSmall bright noclose textCenter">\
                    /lib/constants.js\
                </div></a>\
                <a download="sheets.js" href="' +
        setuptools.config.githubRawUrl +
        "/" +
        data.commit.sha +
        '/lib/sheets.js"><div class="setuptools link menuStyle menuSmall bright noclose textCenter mr0">\
                    /lib/sheets.js\
                </div></a>\
            </div>\
            <div>&nbsp;<br>Tired of having to update these files or Muledump yourself? Why not check out <a href="' +
        setuptools.config.url +
        '/muledump.html" target="_blank">Muledump Online</a>?</div>\
        '
    );
  }
  ...
  setuptools.lightbox.drawhelp(
    "rendersupdate-assistant",
    "docs/muledump/rendersupdate",
    "Renders Version Help"
  );
  setuptools.lightbox.close();
...
setuptools.app.assistants.jakcodexcors = function () {
  ...
  if (setuptools.browser === "chrome") {
    ...
    setuptools.lightbox.build(
      "jakcodexcors-extension-notice",
      " \
            Jakcodex/Muledump provides a CORS extension for " +
        variant +
        " users.\
            <br><br>\
            <div class='flex-container'>\
                <a href='" +
        (setuptools.config.corsURL[setuptools.browser] ||
          setuptools.config.corsURL.default) +
        "' target='_blank'>\
                    <div class='menuStyle formStyle textCenter mr0 cboth' style='width: 300px;'>Jakcodex/Muledump CORS Adapter for " +
        variant +
        "</div>\
                </a>\
            </div>\
    ...
    setuptools.lightbox.settitle(
      "jakcodexcors-extension-notice",
      "New " + variant + " Extension Available"
    );
  }

  setuptools.lightbox.display("jakcodexcors-extension-notice", {
    variant: "fl-Notice",
  });
};
