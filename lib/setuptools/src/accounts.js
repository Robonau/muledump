...
setuptools.app.accounts.start = function () {
  setuptools.lightbox.build(
    "accounts-start",
    '<span>Do you have <a href="#" class="setuptools initsetup page2">one or a few accounts</a> or <a href="#" class="setuptools initsetup page3">many accounts</a>?</span>'
  );
  setuptools.lightbox.goback("accounts-start", setuptools.app.index);
  setuptools.lightbox.drawhelp(
    "accounts-start",
    "docs/setuptools/help/accounts-manager/index",
    "Account Setup Help"
  );
...
setuptools.app.accounts.manager = function () {
  ...
  function ListAccounts(dummy, page) {
    ...
    var AccountsList =
      setuptools.lightbox.menu.paginate.state.AccountsList.PageList;

    //  prepare an empty row if no accounts exist
    if (AccountsList.length === 0)
      AccountsList.push(
        $.extend(true, {}, setuptools.objects.account, {
          username: "",
          password: "",
        })
      );

    if (page > setuptools.lightbox.menu.paginate.state.AccountsList.lastPage)
      page = setuptools.lightbox.menu.paginate.state.AccountsList.lastPage;
  ...
  function createRow(listPos) {
    ...
    return (
      ' \
            <div class="cell noselect" data-listPos="' +
      listPos +
      '"> \
                <div><input class="text" name="username" type="text" placeholder="Account Email or ID" value="' +
      AccountsList[listPos].username +
      '" ' +
      autocomplete +
      '></div> \
                <div><input class="text" name="password" type="password" placeholder="Account Password" value="' +
      AccountsList[listPos].password +
      '" data-reveal="false" ' +
      autocomplete +
      '></div> \
                <div><div class="setuptools div accountControls">\
                    <div data-option="accountMenu" title="Account Menu">&#8801;</div> \
                    <div data-option="toggleEnable" class="' +
      (AccountsList[listPos].enabled === true
        ? ' selected" title="Account Enabled'
  ...
  function AccountsListUpdate(direction) {
    ...
    if (direction === "reset")
      setuptools.lightbox.menu.paginate.findPage(0, "AccountsList");
    ...
    $("div.AccountsList.list").html(
      " \
        ...
        ListAccounts(
          undefined,
          setuptools.lightbox.menu.paginate.state.AccountsList.currentPage
        ) +
        " \
            </div> \
        "
  ...
  function AccountsContext() {
    ...
    $("div.editor.control.massSwitch")
      ...
      .click(async function () {
        ...
        setuptools.lightbox.display("accounts-massSwitch", {
          variant: "fl-AccountsSmall nobackground",
        });

        $("div.massSwitch > div.link").click(async function () {
          //  gather data
          if (
            setuptools.tmp.lightboxStatus &&
            setuptools.tmp.lightboxStatus.indexOf("accounts-ms") > -1
          )
            return;
          var AccountsList =
            setuptools.lightbox.menu.paginate.state.AccountsList.PageList;
          ...
          if (validKeys.indexOf(key) === -1)
            setuptools.lightbox.error(
              "Specified key for massSwitch is not valid.",
              33
            );
          ...
          if (typeof newValue === "undefined")
            setuptools.lightbox.error(
              "Specified type for massSwitch is not valid.",
              33
            );

          //  process the action
          for (var i in AccountsList)
            if (AccountsList.hasOwnProperty(i)) AccountsList[i][key] = newValue;
          ...
          $(".controller.notifier.saveNotice")
            .removeClass("empty")
            .addClass("notice saveAccounts")
            .css({ "margin-top": "5px" })
            .text("Don't forget to save changes")
            .css({ cursor: "initial" });
          if (setuptools.data.config.animations === 1)
            $(".controller.saveAccounts:not(.saveNotice)").addClass(
              "savePulse"
            );
        });
      });
    ...
    $("div.setuptools.app.list input").focus(
      setuptools.lightbox.menu.context.close
    );

    //  save input changes to the temporary object in realtime
    $("div.setuptools.app.list > div.cell > div > input")
      .unbind("keyup")
      .keyup(function (e) {
        var AccountsList =
          setuptools.lightbox.menu.paginate.state.AccountsList.PageList;
    ...
    $("div.setuptools.accountControls > div")
      .unbind("click")
      .click(async function () {
        ...
        if (name === "accountMenu") {
          if ($(this).hasClass("selected") === true) {
            ...
            $(this)
              ...
              .each(function (index, element) {
                if ($(element).attr("data-option") === "accountMenu")
          ...
          if (setuptools.data.config.mulelogin === 1)
            options.push({
              ...
              name:
                "OCL: " +
                (setuptools.app.muledump.ocl.get(
                  AccountsList[listPos].oclProfile
                ) !== false
                  ? AccountsList[listPos].oclProfile
                  : "Default"),
              callback: function () {
                setuptools.app.muledump.ocl.accountMenu(
                  $("div.setuptools.menu.accountMenu"),
                  AccountsList[listPos]
                );
              },
            });
          ...
          options.push(
            {
              ...
              name:
                (cell.find('input[name="password"]').attr("data-reveal") ===
                "true"
                  ? "Hide"
                  : "Reveal") + " Password",
              callback: function () {
            ...
            },
            {
              class: "autoReload",
              name:
                (AccountsList[listPos].autoReload === true
                  ? "Disable"
                  : "Enable") + " Auto Reload",
              callback: function (listPos) {
                AccountsList[listPos].autoReload = !(
                  AccountsList[listPos].autoReload === true
                );
              },
              callbackArg: listPos,
            },
            {
              class: "cacheEnabled",
              name:
                (AccountsList[listPos].cacheEnabled === true
                  ? "Disable"
                  : "Enable") + " Data Cache",
              callback: function (listPos) {
                AccountsList[listPos].cacheEnabled = !(
                  AccountsList[listPos].cacheEnabled === true
                );
              },
              callbackArg: listPos,
            },
            {
              class: "loginOnly",
              name:
                (AccountsList[listPos].loginOnly === true
                  ? "Disable"
                  : "Enable") + " Login Only",
              callback: function (listPos) {
                AccountsList[listPos].loginOnly = !(
                  AccountsList[listPos].loginOnly === true
                );
              },
              callbackArg: listPos,
            },
            {
              ...
              name:
                (AccountsList[listPos].testing === true
                  ? "Disable"
                  : "Enable") + " Testing Server",
              callback: function (listPos) {
                AccountsList[listPos].testing = !(
                  AccountsList[listPos].testing === true
                );
              },
              callbackArg: listPos,
  ...
  setuptools.lightbox.build(
    ...
    ' \
      ...
      (setuptools.state.loaded === true
        ? '<div class="controller exportAccountsJS menuStyle cfright">Export Accounts.js...</div>'
        : "") +
      " \
            </div>\
        </div> \
  ...
  setuptools.lightbox.drawhelp(
    "accounts-manager",
    "docs/setuptools/help/accounts-manager/manager",
    "Accounts Manager Help"
  );
  setuptools.lightbox.settitle("accounts-manager", "Accounts Manager");
  setuptools.lightbox.display("accounts-manager", {
    variant: "fl-AccountsManager",
  });

  AccountsPaginate.bind();

  //  add new account
  $("div.AccountsList > div.newAccount").click(async function () {
    //  check if there's already an empty row
    var data = setuptools.lightbox.menu.paginate.state.AccountsList;
    if (
      AccountsList.length > 0 &&
      AccountsList[AccountsList.length - 1].username === "" &&
      AccountsList[AccountsList.length - 1].password === ""
    ) {
      AccountsListUpdate("add");
      $(
        'div.setuptools.app.list > div.cell:last-child > div > input[name="username"]'
      ).focus();
      return;
    }
    ...
    AccountsListUpdate("add");
    $(
      'div.setuptools.app.list > div.cell[data-listPos="' +
        newPos +
  ...
  $("div.AccountsList > div.controller").click(async function () {
    ...
    if ($(this).hasClass("saveAccounts") === true) {
      ...
      if (
        ...
        setuptools.data.accounts = setuptools.app.config.convert(
          ...
          if (username.length > 0 && password.length > 0) {
            ...
            if (username.length > 0) {
              ...
              if (
                ...
                setuptools.app.config.createUser(
                  username,
                  AccountsList[i],
                  "set",
                  1
      ...
      if (Object.keys(setuptools.data.accounts.accounts).length === 0) {
        setuptools.lightbox.build(
          "accounts-manager-emptySave",
          "No valid accounts were located!"
        );
        ...
        if (
          (await setuptools.app.config.save("AccountsManager/Save")) === false
        ) {
          setuptools.lightbox.build(
            "accounts-manager-saveError",
...
setuptools.app.accounts.ExportDeepCopy = async function (guid, returnData) {
  var storageKey = await setuptools.storage.read("muledump:" + guid, true);
  if (typeof storageKey === "undefined") {
    setuptools.lightbox.build(
      "accounts-deepcopy-download",
      "No account data for " +
        guid +
        " is stored locally. <br><br>Please load this account in Muledump and try again"
    );
  ...
  setuptools.lightbox.build(
    ...
    " \
      ...
      btoa(DeepCopyData) +
      '" class="setuptools config download noclose">Download Deep Copy</a> \
        '
  );
...
setuptools.app.accounts.saveConfirm = function () {
  ...
  setuptools.lightbox.drawhelp(
    "accounts-saveConfirm",
    "docs/setuptools/help/new-user-setup",
    "Welcome to Muledump!"
  );
  setuptools.lightbox.display("accounts-saveConfirm", {
    variant: "setuptools-medium",
  });
  $(".setuptools.app.loadaccounts").click(async function () {
    location.reload();
  });
...
setuptools.app.accounts.AccountsJSExport = function () {
  ...
  setuptools.lightbox.build(
    ...
    ' \
        ...
        <br><br><a download="accounts.js" href="data:text/javascript;base64,' +
      AccountsJSData +
      '" class="setuptools export accountsjs noclose">Download Backup</a> \
    '
  );
...
setuptools.app.accounts.AccountsJSImport = function () {
  setuptools.lightbox.build(
    ...
    ' \
        ...
        <br><br><div class="setuptools link enable menuStyle menuSmall noclose fleft cboth">' +
  ...
  if (Object.keys(setuptools.originalAccountsJS).length > 0) {
    setuptools.lightbox.build(
      "accounts-accountsjs-index",
      ' \
            <div class="setuptools link import accountsJS ' +
        menuClass +
        ' menuSmall fleft mr5 noclose">Import Accounts.js</div> \
            <div class="setuptools link import upload ' +
        menuClass +
        ' menuSmall fright noclose">Upload Accounts.js</div> \
        '
    );
  } else {
    setuptools.lightbox.build(
      ...
      ' \
            <div class="setuptools link import upload ' +
        menuClass +
        ' menuSmall fleft noclose">Upload Accounts.js</div> \
        '
    );
  }
  ...
  if (
    setuptools.data.config.enabled === true &&
    Object.keys(setuptools.originalAccountsJS).length === 0
  ) {
  ...
  setuptools.lightbox.drawhelp(
    "accounts-accountsjs-index",
    "docs/setuptools/help/accounts-manager/accountsjs-import",
    "Accounts.js Import Help"
  );
  setuptools.lightbox.settitle(
    "accounts-accountsjs-index",
    "Accounts.js Import"
  ...
  if (setuptools.state.loaded === false)
    setuptools.lightbox.override(
      "accounts-accountsjs-import",
      "goback",
      setuptools.app.accounts.AccountsJSImport
    );
  if (setuptools.state.loaded === false)
    setuptools.lightbox.override(
      "accounts-accountsjs-upload",
      "goback",
      setuptools.app.accounts.AccountsJSImport
    );
  setuptools.lightbox.display("accounts-accountsjs-index", {
    variant: "fl-Index",
  });
  ...
  if (setuptools.data.config.enabled === false) {
    $(".setuptools.link.enable").click(function () {
      ...
      $(this)
        ...
        .text("SetupTools Enabled");
      $(".setuptools.link.import").removeClass("noclose");
      $(".setuptools.link.import.accountsJS").click(
        setuptools.app.accounts.AccountsJSImportLocal
      );
      $(".setuptools.link.import.upload").click(
        setuptools.app.accounts.AccountsJSImportUpload
      );
    });
  } else {
    $(".setuptools.link.enable")
      .unbind("click")
      .css({ "pointer-events": "none", cursor: "default" })
      .text("SetupTools Enabled");
    $(".setuptools.link.import").removeClass("noclose");
...
setuptools.app.accounts.AccountsJSImportLocal = function () {
  ...
  if (FoundKeys > 0) {
    setuptools.lightbox.build(
      "accounts-accountsjs-import",
      " \
            Proceed with import of " +
        FoundKeys +
        ' located accounts in Accounts.js? \
  ...
  setuptools.lightbox.display("accounts-accountsjs-import", {
    variant: "fl-AccountsJSImport",
  });

  $(".setuptools.link.accounts.importCancelled").click(async function () {
    if (setuptools.state.loaded === false) return;
    setuptools.app.accounts.manager();
  });

  $(".setuptools.link.accounts.importConfirmed").click(async function () {
    //  this click implies setuptools should be enabled
    setuptools.data.config.enabled = true;

    //  load accounts into configuration
    if (typeof setuptools.tmp.userAccounts === "undefined")
    ...
    for (var i in setuptools.originalAccountsJS) {
      if (setuptools.originalAccountsJS.hasOwnProperty(i)) {
        ...
        setuptools.tmp.userAccounts.push(
          $.extend(true, {}, setuptools.objects.account, {
            username: i,
    ...
    if (window.accountLoadDelay)
      setuptools.data.config.accountLoadDelay = window.accountLoadDelay;
...
setuptools.app.accounts.AccountsJSImportUpload = function (manual) {
  ...
  function ParseUploadedFile(FileContents) {
    ...
    for (i = 0; i < UploadFile.length; i++) {
      if ((matches = UploadFile[i].match(setuptools.config.regex.accountsJS))) {
        ...
        if (
          ...
          if (matches[2].match(/^(true|false)$/))
            UploadParse.settings[matches[1]] = matches[2] === "true";
          if (matches[2].match(/^([0-9]*?)$/))
            UploadParse.settings[matches[1]] = Number(matches[2]);
        }
    ...
    $(".setuptools.app.initsetup.uploadResults").html(
      "Found " +
        Object.keys(UploadParse.accounts).length +
        " accounts and " +
        Object.keys(UploadParse.settings).length +
        " settings"
    );
    ...
    $(".setuptools.link.uploadData.save")
      .html("Review Import")
      .click(function () {
        if (Object.keys(UploadParse.accounts).length > 0) {
          ...
          $(".setuptools.app.initsetup.uploadResults").html(
            "You must load a valid accounts.js first."
          );
        }
      });
  }
  ...
  if (
    setuptools.config.devForcePoint !== "restorejs-upload" &&
    manual !== true &&
    window.File &&
    window.FileReader &&
    window.FileList &&
    ...
    if (
      ...
      setuptools.lightbox.build(
        "accounts-accountsjs-upload",
        "Please paste the contents of the accounts.js file below."
      );
    }
  ...
  setuptools.lightbox.drawhelp(
    "accounts-accountsjs-upload",
    "docs/setuptools/help/accounts-manager/accountsjs-upload",
    "Accounts.js Upload Help"
  );
  setuptools.lightbox.settitle(
    "accounts-accountsjs-upload",
    "Accounts.js Import"
  ...
  setuptools.lightbox.display("accounts-accountsjs-upload", {
    variant: "fl-AccountsJSImport",
  });
  ...
  if (DoFiles === true) {
    ...
    $(".setuptools.app.initsetup.switchToUpload").click(async function () {
