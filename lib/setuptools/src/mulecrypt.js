...
setuptools.app.mulecrypt.init = function (callback) {
  if (typeof callback !== "function")
    setuptools.lightbox.error(0, "Encryption init callback must be a function");
  window.sodium = {
    ...
    onload: async function () {
      ...
      var secretbox = new SeaSalt_AEAD_SecretBox(
        secret,
        setuptools.seasalt.aead.xchacha.key("aead"),
        {
      ...
      if (decrypted === "test") {
        ...
        setuptools.app.techlog("Crypto/Init - libsodium test failure", "force");

      //  test for an encrypted installation
      var keychain = await setuptools.storage.read("mulecrypt:keychain");
      try {
      ...
      if (
        ...
        setuptools.app.mulecrypt.ui.startupPrompt(function (callback) {
          ...
          keychain.secretbox = setuptools.seasalt.secretbox.repackage(
            keychain.secretbox,
            userPassword,
            userPassword
          );
      ...
      else callback(); //  honestly, it's formatted this way to see if Batman is paying attention (turns out he's not)
    },
  };
};
...
setuptools.app.mulecrypt.getSecretBox = function (userPassword) {
  ...
  setuptools.app.mulecrypt.config.userPassword =
    setuptools.seasalt.hash.sha256(userPassword);
  ...
  for (var i = 0; i < boxes.length; i++) {
    if (
      setuptools.seasalt.secretbox.check(
        boxes[i],
        setuptools.app.mulecrypt.config.userPassword
      )
    ) {
      setuptools.secretbox = setuptools.seasalt.secretbox.repackage(
        boxes[i],
        setuptools.app.mulecrypt.config.userPassword,
        setuptools.app.mulecrypt.config.userPassword
      );
...
setuptools.app.mulecrypt.error = function (message, title, variant) {
  ...
  setuptools.lightbox.goback(
    "mulecrypt-error",
    setuptools.app.mulecrypt.ui.index
  );
...
setuptools.app.mulecrypt.passwordStrength = function (password) {
  ...
  //  symbol chars
  if (
    (matches.symbol = password.match(/[-!$%^&*()_+|~=`{}\[\]:#";'@<>?,.\/]/g))
  )
    strength++;
  ...
  for (var i in matches)
    if (matches.hasOwnProperty(i))
      if (
        typeof matches[i] === "object" &&
        matches[i] !== null &&
        matches[i].length
      )
        for (var x = 0; x < matches[i].length; x++)
          if (chars.indexOf(matches[i][x].toLowerCase()) === -1)
            chars.push(matches[i][x].toLowerCase());
  ...
  if (chars.length < setuptools.config.mcMinimumEntropy)
    strength = strength - 3;

  //  weak strength but extremely long is given a bonus
  if (
    strength === 1 &&
    password.length >= setuptools.config.mcMinimumKeyLength * 2
  )
    strength++;
...
setuptools.app.mulecrypt.check = function (password) {
  ...
  if (
    setuptools.state.encryption.sodium === false ||
    typeof setuptools.data.accounts.meta.encryptionTestKey !== "string"
  )
    return;

  //  check if we can decrypt the test key
  return (
    setuptools.seasalt.aead.xchacha.decrypt(
      setuptools.data.accounts.meta.encryptionTestKey,
      setuptools.app.mulecrypt.hashKey(password)
    ) === "test"
  );
};
...
setuptools.app.mulecrypt.encrypt = function (newPassword, oldPassword) {
  ...
  if (typeof newPassword !== "string") {
    setuptools.app.mulecrypt.error(
      "You must supply a password to encrypt with."
    );
    return;
  }
  ...
  if (
    ...
    setuptools.app.mulecrypt.error(
      "You must supply the original password to re-encrypt data."
    );
    return;
  }
  ...
  if (
    (setuptools.data.config.encryption === false &&
      setuptools.data.accounts.meta.encryption === true) ||
    (setuptools.data.config.encryption === true &&
      setuptools.data.accounts.meta.encryption !== true)
  ) {
    setuptools.app.mulecrypt.error(
      "There is a sanity issue between client configuration and accounts configuration."
    );
    return;
  }
...
setuptools.app.mulecrypt.ui.error = function () {
  setuptools.lightbox.build(
    "mulecrypt-error",
    "Your browser does not support modern encryption. This feature is disabled."
  );
...
setuptools.app.mulecrypt.ui.startupPrompt = function (callback, errorMessage) {
  ...
  setuptools.lightbox.drawhelp(
    "mulecrypt-startupPrompt",
    "docs/setuptools/help/mulecrypt/startup",
    "Mulecrypt Startup Help"
  );
  setuptools.lightbox.display("mulecrypt-startupPrompt", {
    variant: "fl-Introduction",
  });

  //  clean up installation
  $("div.setuptools.link.mulecrypt.startupErase").click(async function () {
    setuptools.lightbox.build(
      "mulecrypt-startupPromp-erase",
      ' \
    ...
    setuptools.lightbox.drawhelp(
      "mulecrypt-startupPromp-erase",
      "docs/setuptools/help/mulecrypt/reset",
      "Mulecrypt Help"
    );
    setuptools.lightbox.display("mulecrypt-startupPromp-erase", {
      variant: "fl-Notice",
    });
  });

  //  process userPassword
  $("div.setuptools.link.mulecrypt.startupGo").click(async function () {
    setuptools.app.mulecrypt.getSecretBox(
      $('input[name="userPassword"]').val()
    );
    if (typeof setuptools.secretbox === "undefined") {
      setuptools.app.mulecrypt.ui.startupPrompt(
        callback,
        "Password did not match any stored keys."
      );
    } else callback();
  });
};
...
setuptools.app.mulecrypt.ui.setup = function () {
  ...
  setuptools.lightbox.display("encryption-setup", {
    variant:
      "fl-MuleCrypt " +
      (setuptools.data.config.animations === 1 ? "fl-MuleCryptAnimated" : ""),
  });

  //  help the user download a backup
  $(".setuptools.link.downloadBackup").click(async function () {
  ...
  $(".setuptools.link.encrypt").click(async function () {
  ...
  $(".setuptools.link.generateKey").click(async function () {
    var pool =
      "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789[];',./<>?:\"{}\\|!@#$%^&*()-=_+`~";
    var key = "";
    for (var i = 0; i < 48; i++)
      key += pool.shuffle().charAt(Math.floor(Math.random() * pool.length));
  ...
  $("input").keyup(function () {
    ...
    //  compare passwords
    if (
      strength >= setuptools.config.mcMinimumKeyStrength &&
      confirmPassword.length >= setuptools.config.mcMinimumKeyLength &&
      password === confirmPassword
    ) {
      $(".setuptools.link.encrypt")
        .removeClass("negative")
        .addClass("positive")
        .text("Encrypt Account Data");
    } else {
