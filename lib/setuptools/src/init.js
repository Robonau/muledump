...
setuptools.init.main = async function (window) {
  setuptools.window = window;
  setuptools.app.uaTiming("startup", "stInitMain", "start");

  if (setuptools.data.muledump.totals.configSets.active === "Default")
  ...
  if (setuptools.state.preview === true)
    $("#previewnotice").html(
      "<div>Muledump Preview v" + window.VERSION + "</div>"
    );

  //  store a clone of the original accounts.js object if present
  if (
    typeof window.accounts === "object" &&
  ...
  if ((await setuptools.storage.test()) === false) {
      ...
      setuptools.lightbox.create(
        ' \
          ...
          (setuptools.state.hosted === true
            ? "<br><br>Muledump Online requires local storage support. Download the latest release instead to get started."
            : "") +
          " \
        "
      );
    ...
    if ((await setuptools.app.checknew()) === true) {
      ...
      window.techlog(
        "Notice: You may ignore any accounts.js file not found errors because SetupTools is running",
        "norecord",
        true
      );
      ...
      if (await setuptools.storage.read("configuration"))
        $.extend(
          true,
          setuptools.data,
          JSON.parse(await setuptools.storage.read("configuration"))
        );

      //  setuptools in bypass mode
      if (
        setuptools.config.devForcePoint !== "bad-config" &&
        ...
        if (
          ...
          window.techlog(
            "SetupTools/Init - Client config importing " +
              Object.keys(userConfiguration).length +
              " keys",
            "force"
          );
        ...
        var userid = await setuptools.storage.read("userid");
        if (
          typeof userid === "string" &&
          userid.match(setuptools.config.regex.gaUserId)
        )
          setuptools.data.userid = userid;
        setuptools.app.ga("init");
        setuptools.app.ga("send", "event", {
          eventCategory: "runtimeMode",
        ...
        if (
          ...
          setuptools.lightbox.drawhelp(
            "bypass-help",
            "docs/setuptools/help/bypass",
            "Setup Tools Help"
          );
          setuptools.lightbox.display("bypass-help");
          $(".setuptools.app.settings.manager").click(
        ...
        window.techlog(
          "SetupTools/Init - Loaded stored configuration",
          "force"
        );
        window.techlog(
          "Notice: You may ignore any accounts.js file not found errors because setuptools loaded successfully",
          "norecord",
          true
        );
        ...
        if (setuptools.data.config.backupAssistant === 1)
          setTimeout(function () {
        ...
        var ActiveAccounts = Object.keys(
        ...
        if (ActiveAccounts === 0) {
          ...
          setuptools.lightbox.drawhelp(
            "noaccounts-help",
            "docs/setuptools/help/no-accounts",
            "Account Management Help"
          );
          setuptools.lightbox.display("noaccounts-help");
          $(".setuptools.app.accounts.manager").click(
            setuptools.app.accounts.manager
          );
          $(".setuptools.app.initsetup.accountsjsImport").click(
        ...
        setuptools.config.devForcePoint !== "bad-config" &&
        (typeof setuptools.data.config.enabled === "undefined" ||
          setuptools.data.config.enabled === false)
      ) {
        //  load options
        window.options_init(setuptools.state.hosted);
        ...
        window.techlog(
          "SetupTools/Init - Problem detected with stored configuration"
        );
        setuptools.lightbox.create(
          "There seems to be a problem with your configuration."
        );
      }
...
setuptools.init.accounts = function () {
  setuptools.app.mulecrypt.system(async function () {
    ...
    if (setuptools.state.loaded === true) {
      ...
      window.accounts = setuptools.app.config.convert(
      ...
      if (setuptools.data.config.groupsMergeMode > 0) {
        ...
        if (
          typeof groupAccounts === "object" &&
          Object.keys(groupAccounts).length > 0
        ) {
      ...
      if (window.accountLoadDelay === 0 || window.accountLoadDelay === -1) {
        ...
        if (AccountLength >= 10 && AccountLength < 20)
          window.accountLoadDelay = window.accountLoadDelay === -1 ? 0 : 2;
        if (AccountLength >= 20)
          window.accountLoadDelay = window.accountLoadDelay === -1 ? 6 : 10;
        window.techlog(
          "SetupTools/InitAccounts - Dynamically set accountLoadDelay to " +
            window.accountLoadDelay +
            " seconds",
          "force"
        );
      }
    ...
    if ((await setuptools.app.checknew()) === false) {
      ...
      for (var option in setuptools.data.options)
        if (setuptools.data.options.hasOwnProperty(option))
          if (
            typeof setuptools.app.muledump.totals.config.getKey(
              option,
              undefined,
              null
            ) !== "undefined"
            ...
            setuptools.app.muledump.totals.config.setKey(
              option,
              setuptools.data.options[option]
            );

      //  remove accountFilter members if the guid isn't loaded
      var accountFilterChanged = [];
      if (Array.isArray(setuptools.data.options.accountFilter))
        setuptools.data.options.accountFilter.filter(function (guid) {
          if (typeof window.accounts[guid] === "undefined")
            accountFilterChanged.push(guid);
        });
      if (accountFilterChanged.length > 0) {
        //setuptools.app.muledump.totals.config.activate('Default');
        for (var x = 0; x < accountFilterChanged.length; x++)
          setuptools.data.options.accountFilter.splice(
            setuptools.data.options.accountFilter.indexOf(
              accountFilterChanged[x]
            ),
            1
          );
        setuptools.app.muledump.totals.config.setKey(
          "accountFilter",
          setuptools.data.options.accountFilter
        );
      }
    }
    ...
    for (var i in mules)
      if (mules.hasOwnProperty(i))
        if (
          (setuptools.state.loaded === true &&
            setuptools.app.config.userExists(i) === false) ||
          setuptools.app.config.userEnabled(i) === false
        ) {
          mules[i].dom.remove();
          delete mules[i];
        }
    ...
    if ((await setuptools.app.checknew()) === false) {
      ...
      if (
        Array.isArray(setuptools.data.options.accountFilter) &&
        setuptools.data.options.accountFilter.length > 0
      ) {
        setuptools.data.options.accountFilter.filter(function (guid) {
...
setuptools.init.uiModifiers = function () {
  ...
  if (setuptools.data.config.animations === 1)
    $("body").removeClass("AReduced");
  if (setuptools.data.config.animations < 1)
    $("#stickynotice").addClass("AReduced");
  ...
  if (setuptools.data.config.menuPosition === 0)
    $("#topContainer").css({ "flex-flow": "row-reverse" });
  if (setuptools.data.config.menuPosition === 1)
    $("#topContainer").css({ "justify-content": "center" });
};
