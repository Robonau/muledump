...
class Muledump_ElementScrollLock {
  constructor(track, hook, config) {
    ...
    this.name =
      "ScrollLock" + (typeof this.config.name === "string")
        ? "." + this.config.name
        : "";
  ...
  start() {
    this.ScrollPos = 0;
    $(window)
      .off("scroll." + this.name)
      .on("scroll." + this.name, [this], function (e) {
        var self = e.data[0];
...
class Muledump_TotalsCounter {
  constructor(guid, cache, callback) {
    ...
    this.log =
      setuptools &&
      setuptools.app &&
      typeof setuptools.app.techlog === "function"
    ...
    if (typeof cache === "undefined" && typeof json === "string")
      this.cache = true;
  ...
  async read(key) {
    if (typeof setuptools === "object")
      return await setuptools.storage.read(key);
  ...
  async write(key, value) {
    ...
    if (typeof setuptools === "object")
      return await setuptools.storage.write(key, value);
  ...
  count(id, type, qty, force) {
    ...
    if (typeof this.data.types[type] === "object")
      this.data.types[type][id] =
        typeof this.data.types[type][id] === "number"
          ? this.data.types[type][id] + qty
          : qty;
    this.data.totals[id] =
      typeof this.data.totals[id] === "number"
        ? this.data.totals[id] + qty
        : qty;
  ...
  import(tracker, excluded) {
    ...
    if (Array.isArray(tracker) === true) {
      if (excluded === true && this.excluded.indexOf(tracker) === -1)
        this.excluded = this.excluded.concat(tracker);
      if (excluded === false)
      ...
      if (tracker.length === 1) {
        tracker =
          mules[tracker[0]].totals instanceof Muledump_TotalsCounter
            ? mules[tracker[0]].totals
            : undefined;
    ...
    for (var guid in window.mules)
      if (window.mules.hasOwnProperty(guid))
        if (
          ...
          ((window.mules[guid].loaded === false &&
            window.mules[guid].loginOnly !== true) ||
    ...
    if (
      tracker instanceof Muledump_TotalsCounter &&
      //  this verifies that we are eligible for mode 0 if the source is already imported
      !(
        this.sources.indexOf(tracker.guid) > -1 &&
        !this.sourceData[tracker.guid]
      )
    ) {
      this.busy = true;
      this.aggregate = true;
      ...
      if (this.sources.indexOf(tracker.guid) > -1) {
        ...
        if (
          ...
          new SeaSalt_Hashing(
            JSON.stringify(this.sourceData[tracker.guid].meta),
            "sha256",
            "hex"
          ).toString() ===
            new SeaSalt_Hashing(
              JSON.stringify(tracker.data.meta),
              "sha256",
              "hex"
            ).toString()
        ) {
          this.log(
            "Muledump_TotalsCounter/Import skipping identical source - " +
              tracker.guid +
              " - " +
              JSON.stringify(this.sourceData[tracker.guid].meta)
          );
        ...
        Object.filter(
          this.sourceData[tracker.guid].types,
          function (type, data) {
            for (var item in data) {
              if (data.hasOwnProperty(item)) {
                if (self.data.types[type][item])
                  self.data.types[type][item] =
                    self.data.types[type][item] - data[item];
                if (self.data.totals[item])
                  self.data.totals[item] = self.data.totals[item] - data[item];
              }
            }
          }
        );
    ...
    //  import mode 2: reimport of all tracker sources
      ...
      for (var guid in window.mules) {
        if (
          ...
          this.excluded.indexOf(guid) === -1 &&
          window.mules[guid].disabled !== true &&
          (window.mules[guid].loaded === true ||
            window.mules[guid].loginOnly === true)
        ) {
          if (!(window.mules[guid].totals instanceof Muledump_TotalsCounter)) {
            this.error =
              "Import only accepts instances of Muledump_TotalsCounter as its argument";
            continue;
          }
  ...
  toString(pretty) {
    ...
    try {
      json =
        pretty === true
          ? JSON.stringify(this.data, true, 5)
  ...
  //  delete data cache
  async clear() {
    return await setuptools.storage.delete(this.cache_key);
  }
}
...
class Muledump_MouseDirection {
  constructor(config) {
    ...
    this.runtimeId = setuptools.seasalt.hash
  ...
  on() {
    ...
    $(window).on("mousemove." + this.namespace, function (e) {
      ...
      if (self.config.target.class) {
        if (typeof self.config.target.class === "string")
          self.config.target.class = [self.config.target.class];
      ...
      if (self.travel.h === true) {
        self.approach.h =
          e.offsetX / $(e.target).outerWidth() >= 0.5 - avariance;
      } else {
        self.approach.h =
          e.offsetX / $(e.target).outerWidth() <= 0.5 + avariance;
      }
      ...
      if (self.travel.v === true) {
        self.approach.v =
          e.offsetY / $(e.target).outerHeight() >= 0.5 - avariance;
      } else {
        self.approach.v =
          e.offsetY / $(e.target).outerHeight() <= 0.5 + avariance;
      }
...
class Muledump_Dragging {
  constructor(config) {
    ...
    //  convert to an array
    if (typeof this.config.swap === "string")
    ...
    this.namespace =
      "muledump.dragging." + this.config.name + Date.now().toString();
  ...
  callback_router(route, parent, self, e, target, force) {
    ...
    var truth = true;
    if (typeof this.config.callbacks[route] === "function")
      this.config.callbacks[route] = [this.config.callbacks[route]];
    if (Array.isArray(this.config.callbacks[route]))
      this.config.callbacks[route].filter(function (callback) {
        if (force !== true)
          if (typeof callback !== "function" || truth !== true) return;
      ...
      $(window)
        ...
        .on("mousemove." + parent.namespace, function (e) {
          ...
          if (parent.config.mode === "drag") {
            ...
            if (
              parent.MouseDirection.approach.h === false &&
              parent.MouseDirection.approach.v === false &&
            ...
            if (
              (parent.MouseDirection.approach.h === true &&
                parent.MouseDirection.travel.h === true) ||
              (parent.MouseDirection.rel.v === false &&
                parent.MouseDirection.travel.v === true)
            )
        ...
        .on("mouseup." + parent.namespace, function (e) {
          ...
          if (parent.config.mode === "swap") {
            ...
            if (parent.clickIndexDown > parent.clickIndexUp) {
              c.target.insertAfter(
                $($(parent.targetName).get(parent.clickIndexDown))
              );
            } else {
              c.target.insertBefore(
                $($(parent.targetName).get(parent.clickIndexDown))
              );
            }
