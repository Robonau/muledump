...
setuptools.app.devtools.compressionUtility = function () {
    ...
    var html =
      ' \
                <div class="flex-container" style="justify-content: space-between;">\
                    ...
                    <div class="flex-container" style="padding: 2px 4px; width: 50%; justify-content: center; background-color: #333;">' +
      (setuptools.data.config.compression === true ? "On" : "Off") +
      "</div>\
                </div>\
            ";
      ...
      html +=
        ' \
                <div class="flex-container" style="justify-content: space-between;">\
                    ...
                    <div class="flex-container" style="padding: 2px 4px; width: 50%; justify-content: center; background-color: #111;">' +
        done +
        " of " +
        total +
        " objects</div>\
  ...
  setuptools.lightbox.build(
    ...
    ' \
            ...
            <div class="flex-container syncReport" style="flex-wrap: wrap;">' +
      syncReport() +
      "</div>\
        "
  );
  ...
  setuptools.lightbox.drawhelp(
    "compression-utility",
    "docs/setuptools/help/compression-utility",
    "Compression Utility Help"
  );
  setuptools.lightbox.display("compression-utility", { variant: "fl-Index" });

  //  analyze files and report how many are out of sync
  $(".setuptools.link.analyze").on(
    "click.setuptools.link.analyze",
    function () {
      $("div.syncReport").html(
        syncReport(setuptools.storage.compression.analyze().length)
      );
    }
  );

  //  synchronize all objects
  $(".setuptools.link.synchronize").on(
    "click.setuptools.link.synchronize",
    function () {
      ...
      outOfSync.filter(async function (key) {
        setuptools.app.techlog("CompressionUtility/Synchronize object: " + key);
        $("div.syncReport").html(
          syncReport(outOfSync.indexOf(key) + 1, outOfSync.length)
        );
        await setuptools.storage.write(
          key,
          await setuptools.storage.read(key, true),
          true
        );
      });
    }
...
setuptools.app.devtools.viewSystemsReport = function () {
  ...
  setuptools.lightbox.build(
    ...
    ' \
            <div class="flex-container systemsReport">\
                ...
                <div>' +
      report.summary["muledump-accountDataCache"].toFixed(2) +
      ' KB</div>\
            </div>\
            <div class="flex-container systemsReport">\
                <div>SetupTools Configuration</div>\
                <div>' +
      report.summary["setuptools-config"].toFixed(2) +
      ' KB</div>\
            </div>\
            <div class="flex-container systemsReport">\
                <div>SetupTools Backups</div>\
                <div>' +
      report.summary["setuptools-backups"].toFixed(2) +
      ' KB</div>\
            </div>\
            <div class="flex-container systemsReport">\
                <div>SetupTools MuleQueue</div>\
                <div>' +
      report.summary["setuptools-mulequeue"].toFixed(2) +
      ' KB</div>\
            </div>\
            <div class="flex-container systemsReport">\
                <div>SetupTools Cache</div>\
                <div>' +
      report.summary["setuptools-cache"].toFixed(2) +
      ' KB</div>\
            </div>\
            <div class="flex-container systemsReport">\
                <div>SetupTools Total Data Usage</div>\
                <div>' +
      report.summary["setuptools-all"].toFixed(2) +
      ' KB</div>\
            </div>\
            <div class="flex-container systemsReport">\
                <div>Other Usage</div>\
                <div>' +
      report.summary.other.toFixed(2) +
      ' KB</div>\
            </div>\
      ...
      '%</div>\
            ...
            <div class="flex-container systemsReport" style="padding-top: 10px; margin-top: 7px; border-top: 2px solid #333;">\
                ...
                <div>' +
      setuptools.app.calculateCpuTime(true) +
      "</div>\
            </div>\
        "
  );
  ...
  setuptools.lightbox.drawhelp(
    "settings-viewSystemsReport",
    "docs/muledump/systems-report",
    "Systems Report Help"
  );
  setuptools.lightbox.display("settings-viewSystemsReport", {
    variant: "fl-Index select",
  });
};
...
setuptools.app.devtools.ui = function () {
  ...
  $("div.setuptools.link.vaultbuilder").on(
    "click.devtools.vaultbuilder",
    setuptools.app.muledump.vaultbuilder.ui
  );
  $("div.setuptools.link.systemsreport").on(
    "click.devtools.vaultbuilder",
    setuptools.app.devtools.viewSystemsReport
  );
  $("div.setuptools.link.configviewer").on(
    "click.devtools.configviewer",
    setuptools.app.devtools.configViewer
  );
  $("div.setuptools.link.consoleviewer").on(
    "click.devtools.consoleviewer",
    setuptools.app.devtools.consoleViewer
  );
  $("div.setuptools.link.compressionutility").on(
    "click.devtools.compressionutility",
 ...
 */
setuptools.app.devtools.consoleViewer = async function () {
  var ListRecord = function (dummy, page) {
    if (typeof page !== "number") page = 0;
    var ConsoleHistory =
      setuptools.lightbox.menu.paginate.state.ConsoleHistory.PageList;
    if (page > setuptools.lightbox.menu.paginate.state.ConsoleHistory.lastPage)
      page = setuptools.lightbox.menu.paginate.state.ConsoleHistory.lastPage;
    $('div.ConsoleHistory div.customPage input[name="customPage"]').val(
      page + 1
    );
    ...
    var maxIndex =
      setuptools.config.recordConsolePerPage * page +
  ...
  var createRow = function (listPos) {
    ...
    return (
      ' \
            <div class="flex-container muledump consoleviewer m5 p5" data-listPos="' +
      listPos +
      '">\
                <div style="width: 14%;" class="mr5">' +
      ConsoleRecord[0] +
      '</div>\
                <div style="width: 14%;" class="mr5">' +
      ConsoleRecord[1] +
      '</div>\
                <div style="width: 8%;" class="mr5">' +
      ConsoleRecord[2] +
      '</div>\
                <div style="width: 64%;">' +
      ConsoleRecord[3] +
  ...
  var ConsoleHistoryUpdate = function (page) {
    if (
      !page &&
      typeof setuptools.lightbox.menu.paginate.state.ConsoleHistory === "object"
    )
      page = setuptools.lightbox.menu.paginate.state.ConsoleHistory.currentPage;
    if (["string", "number", "undefined"].indexOf(typeof page) === -1) return;
    var ConsoleHistoryPaginate =
      setuptools.lightbox.menu.paginate.state.ConsoleHistory;
    ...
    if (typeof page === "undefined")
      setuptools.lightbox.menu.paginate.findPage(0, "ConsoleHistory");
    if (typeof page === "string")
      setuptools.lightbox.menu.paginate.findPage(page, "ConsoleHistory");
    if (typeof page === "number")
      setuptools.lightbox.menu.paginate.state.ConsoleHistory.currentPage = page;

    //  validate lastPage/currentPage boundary
    var newLastPage = Math.ceil(
      ConsoleHistoryPaginate.PageList.length /
        setuptools.config.recordConsolePerPage
    );
    if (newLastPage > 0) newLastPage--;
    if (ConsoleHistoryPaginate.currentPage > newLastPage)
      setuptools.lightbox.menu.paginate.findPage(
        ConsoleHistoryPaginate.PageList.length - 1,
        "ConsoleHistory"
      );
    ...
    $("div.ConsoleHistory.list").html(
      ' \
            <div class="flex-container ml5 mb5 mr5">' +
        ConsoleHistoryPaginate.html.menu +
        '</div> \
  ...
  var data = JSON.parse(await setuptools.storage.read("console"));
  ...
  setuptools.lightbox.settitle(
    "devtools-consoleViewer",
    "Muledump Console Viewer"
  );
  setuptools.lightbox.goback(
    "devtools-consoleViewer",
    setuptools.app.devtools.ui
  );
  setuptools.lightbox.display("devtools-consoleViewer", {
    variant: "fl-ConsoleViewer select",
  });

  ConsoleHistoryPaginate.bind();
  $("div.setuptools.link.consoleviewer.reload").on(
    "click.consoleviewer.reload",
    setuptools.app.devtools.consoleViewer
  );
};
...
setuptools.app.devtools.configViewer = function (mode) {
    ...
    setuptools.lightbox.build(
      ...
      ' \
            <div class="flex-container muledump configviewer m5 p5">\
                <div class="flex-container noFlexAutoWidth" style="width: 40%; margin-right: 15px;">' +
        (!mode || mode === "server" ? "setuptools.config" : "setuptools.data") +
        "." +
        key +
        '</div>\
        ...
        (data.length > 120
          ? ' \
                    <textarea class="setuptools config value w100 scrollbar" rows="6">' +
            (typeof config[key] === "object"
              ? JSON.stringify(config[key], null, 5)
              : config[key]) +
            "</textarea>\
                "
          : ' \
                    <input class="setuptools input config value w100" value=\'' +
            (typeof config[key] === "object"
              ? JSON.stringify(config[key])
              : config[key]) +
            "'>\
  ...
  $("div.setuptools.link.serverconfig").on(
    "click.setuptools.serverconfig",
    function () {
      setuptools.app.devtools.configViewer("server");
    }
  );
  $("div.setuptools.link.clientconfig").on(
    "click.setuptools.clientconfig",
    function () {
