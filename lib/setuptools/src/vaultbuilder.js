...
setuptools.app.muledump.vaultbuilder.vaultChangeTimer = function (ttl) {
  ...
  if (setuptools.tmp.vaulteditoridchangetimer)
    clearTimeout(setuptools.tmp.vaulteditoridchangetimer);
  setuptools.tmp.vaulteditoridchangetimer = setTimeout(
    setuptools.app.muledump.vaultbuilder.pushChanges,
    ttl
  );
};
...
setuptools.app.muledump.vaultbuilder.drawVaultLayout = function (layout, guid) {
  if (isNaN(+layout) && typeof layout !== "string")
    layout = setuptools.data.muledump.vaultbuilder.config.layout;
  if (typeof guid !== "string")
    guid = setuptools.data.muledump.vaultbuilder.config.guid;
  if (typeof guid === "string")
  ...
  state.data.vaultorder.filter(function (vaultid) {
    ...
    if (
      ...
      mules[guid].data.query.results.Chars.Account.Vault.Chest.length >= vaultid
    )
      state = "opened";
    ...
    if (
      ...
      vaultid === 1 &&
      typeof mules[guid].data.query.results.Chars.Account.Vault === "object" &&
      typeof mules[guid].data.query.results.Chars.Account.Vault.Chest ===
        "string"
    )
      state = "opened";
    ...
    html +=
      ...
      '" data-vaultId="' +
      vaultid +
      '"><span><input name="vaultid" data-originalValue="' +
      vaultid +
      '" value="' +
      vaultid +
      '"></span></div>';
  ...
  $("#vaultbuilder").css({
    width:
      setuptools.data.muledump.vaultbuilder.layouts[layout].vaultwidth * 56 +
      112,
  });
  ...
  $("div.layoutInfo div.layoutname").text(
    setuptools.data.muledump.vaultbuilder.layouts[layout].layoutname
  );
  $("div.layoutInfo div.layout").text(layout);
  $("div.layoutInfo div.width").html(
    " \
        " +
      setuptools.data.muledump.vaultbuilder.layouts[layout].vaultwidth +
      " rows wide \
        <br>" +
      Math.ceil(
  ...
  $('input[name="vaultid"]')
    ...
    .on(
      "keydown.vaultidvalidation",
      setuptools.app.muledump.vaultbuilder.validateNumericInput
    )
    ...
    //  lazy save changes
    .on(
      "change.updatetimer keyup.updatetimer",
      setuptools.app.muledump.vaultbuilder.vaultChangeTimer
    )
    ...
    .on("keyup.blur", function (e) {
      ...
      if (e.key === "Delete") {
        ...
        $(this)
          ...
          .parent()
          .attr("data-vaultId", 0)
          .removeClass("conflict opened closec")
          .addClass("blank");
      }
    });
...
setuptools.app.muledump.vaultbuilder.findByName = function (name) {
  ...
  Object.keys(setuptools.data.muledump.vaultbuilder.layouts).filter(function (
    index
  ) {
    var data = setuptools.data.muledump.vaultbuilder.layouts[index];
    if (data.layoutname.match(new RegExp("(" + name + ")", "i")) !== null) {
...
setuptools.app.muledump.vaultbuilder.findNewId = function () {
  ...
  var newId =
    Math.max.apply(
      null,
      Object.keys(setuptools.data.muledump.vaultbuilder.layouts)
    ) + 1;

  //  return it if it meets the minimum requirements or send the smallest allowed id
  return newId >= setuptools.config.vaultbuilderMinimumId
    ? newId
...
setuptools.app.muledump.vaultbuilder.ui = function (layout, guid) {
  ...
  if (typeof setuptools.data.muledump.vaultbuilder.layouts[layout] !== "object")
    layout = setuptools.data.options.vaultlayout;
  if (typeof layout === "string") layout = +layout;
  if (isNaN(+layout)) layout = setuptools.data.options.vaultlayout;
  if (!isNaN(+layout))
    setuptools.data.muledump.vaultbuilder.config.layout = layout;
  if (
    typeof setuptools.data.muledump.vaultbuilder.layouts[layout] !== "object"
  ) {
    layout = 0;
  ...
  setuptools.lightbox.build(
    ...
    ' \
        <div class="flex-container">\
            <div id="vaultbuilder" class="flex-container noFlexAutoWidth" style="width: ' +
      (width + 112) +
      ' px;"> \
                ...
                <div class="flex-container manager" style="width: ' +
      width +
      'px;"></div>\
            </div>\
        </div>\
  ...
  setuptools.lightbox.drawhelp(
    "vaultbuilder",
    "docs/setuptools/help/vaultbuilder",
    "Vault Builder Help"
  );
  ...
  $("div.setuptools.link.vaultorder")
    .off("click.vaultorder.options")
    .on("click.vaultorder.options", function (e) {
      var key = $(this).attr("data-key");
      setuptools.data.muledump.vaultbuilder.layouts[layout][key] =
        !setuptools.data.muledump.vaultbuilder.layouts[layout][key];
      setuptools.tmp.vaultbuilderState.data[key] =
        setuptools.data.muledump.vaultbuilder.layouts[layout][key];
      $(this)
        .find("div:last-child")
        .text(
          setuptools.data.muledump.vaultbuilder.layouts[layout][key] === true
            ? "On"
            : "Off"
        );
    });
  ...
  $('input[name="existingVaultWidth"]')
    .off("keydown.inputvalidation")
    .on(
  ...
  $("div.setuptools.link.vaultbuilder.loadExisting")
    ...
    .on(
      "click.loadExisting mouseover.loadExisting",
      setuptools.app.muledump.vaultbuilder.menu.load
    );

  $("div.setuptools.link.vaultbuilder.options")
    .off("click.options mouseover.options")
    .on(
      "click.options mouseover.options",
      setuptools.app.muledump.vaultbuilder.menu.options
    );
};
 ...
 */
setuptools.app.muledump.vaultbuilder.tasks.create = async function (
  name,
  width,
  height
) {
  ...
  if (layout >= setuptools.config.vaultbuilderMinimumId) {
    ...
    setuptools.data.muledump.vaultbuilder.config.layout = layout;
    setuptools.data.muledump.vaultbuilder.layouts[layout] = $.extend(
      true,
      {},
      vaultorders[0],
      {
    ...
    if (
      (await setuptools.app.config.save(
        "Muledump/VaultBuilder created new layout"
      )) !== true
    )
      return false;
...
setuptools.app.muledump.vaultbuilder.tasks.saveAs = function (name, layout) {
  if (isNaN(+layout))
    layout = setuptools.app.muledump.vaultbuilder.findByName(name);
  layout = +layout;
...
setuptools.app.muledump.vaultbuilder.tasks.upgrade = function () {
  ...
  if (typeof setuptools.data.muledump.vaultbuilder.layouts !== "object")
    setuptools.data.muledump.vaultbuilder.layouts = {};
  ...
  Object.keys(setuptools.data.muledump.vaultbuilder.layouts).map(function (
    layout
  ) {
...
setuptools.app.muledump.vaultbuilder.tasks.save = function () {
  ...
  if (typeof state.layout !== "number")
    state.layout = setuptools.app.muledump.vaultbuilder.findNewId();
  if (state.layout < setuptools.config.vaultbuilderMinimumId)
    return setuptools.app.muledump.vaultbuilder.tasks.saveAs(
      setuptools.tmp.vaultbuilderState.data.layoutname + " (custom)"
    );

  setuptools.data.muledump.vaultbuilder.layouts[state.layout] = $.extend(
    true,
    {},
    vaultorders[0],
    {
      layoutname:
        setuptools.tmp.vaultbuilderState.data.layoutname +
        (rename ? " (custom)" : ""),
  ...
  setuptools.data.muledump.vaultbuilder.layouts[state.layout].vaultorder =
    setuptools.tmp.vaultbuilderState.data.vaultorder;
...
setuptools.app.muledump.vaultbuilder.tasks.load = function (layout) {
  ...
  if (isNaN(+layout)) return false;
  setuptools.data.muledump.vaultbuilder.config.layout = layout;
  setuptools.app.muledump.vaultbuilder.drawVaultLayout(
    layout,
    setuptools.data.muledump.vaultbuilder.config.guid
  );
  return true;
};
...
setuptools.app.muledump.vaultbuilder.tasks.delete = function (layout) {
  ...
  if (layout === setuptools.data.muledump.vaultbuilder.config.layout)
    layout = 0;
  setuptools.data.muledump.vaultbuilder.config.layout = layout;
  setuptools.app.config.save(
    "Muledump/VaultBuilder deleted a vault layout",
    true
  );
  return true;
};
...
setuptools.app.muledump.vaultbuilder.tasks.accountView = function (guid) {
  if (typeof guid !== "string" || !(mules[guid] instanceof Mule)) return;
  setuptools.data.muledump.vaultbuilder.config.guid = guid;
  setuptools.app.config.save(
    "Muledump/VaultBuilder changed account view",
    true
  );
  setuptools.app.muledump.vaultbuilder.ui(
    setuptools.data.muledump.vaultbuilder.config.layout ||
...
setuptools.app.muledump.vaultbuilder.menu.import = function (mode) {
  ...
  if (mode === "export") {
    ...
    Object.keys(setuptools.data.muledump.vaultbuilder.layouts).forEach(
      function (layout) {
        var data = setuptools.data.muledump.vaultbuilder.layouts[layout];
        html +=
          ' \
    ...
    html +=
      ...
      (setuptools.state.hosted === false && setuptools.browser === "chrome"
        ? '<strong class="mr5">Warning</strong> <div style="text-align: justify;">Your browser may flag zip downloads from local apps as being potentially malicious.</div>'
        : "") +
      "</div>\
        ";
  }
  ...
  options.push({
      ...
      ' \
            <div class="flex-container" style="justify-content: space-between;">\
                <div class="flex-container mr5 noFlexAutoWidth" style="flex-flow: column; flex-grow: 0;">\
                    <div class="setuptools link vaultbuilder importMode menuStyle buttonStyle bright mr0 mb5 ' +
      (mode === "import" ? "selected" : "") +
      '" style="width: 150px; height: 17px;">Import</div>\
                    <div class="setuptools link vaultbuilder exportMode menuStyle buttonStyle bright notice menuSmall mr0 ' +
      (mode === "export" ? "selected" : "") +
      '" style="width: 150px; height: 17px;">Export</div>\
                </div>\
                <div class="flex-container noFlexAutoWidth p5 ' +
      (mode === "export" ? "notice" : "") +
      ' bright buttonStyle mr0" style="flex-wrap: wrap; height: 100%; flex-grow: 3;">\
                    ' +
      html +
      "\
                </div>\
    ...
    binding: function () {
      //  cancel button
      $("div.setuptools.link.vaultbuilder.cancel").on(
        "click.createCancel",
        function () {
      ...
      //  import mode button
      $("div.setuptools.link.vaultbuilder.importMode").on(
        "click.vaultbuilder.importMode",
        function () {
      ...
      //  export mode button
      $("div.setuptools.link.vaultbuilder.exportMode").on(
        "click.vaultbuilder.exportMode",
        function () {
      ...
      $('div.setuptools.menu input[name="saveAsId"]').on(
      ...
      function ImportFileParseZips(files) {
        ...
        Object.keys(files.layouts).forEach(function (layout) {
          if (+layout >= setuptools.config.vaultbuilderMinimumId)
            data[layout] = files.layouts[layout];
        });

        var label = $('label[for="files"]');
        label.text(
          Object.keys(files.layouts).length +
            " file" +
            (Object.keys(files.layouts).length !== 1 ? "s" : "") +
            " found"
        );
        ...
        Object.keys(files.layouts).forEach(function (layout) {
          var dom = $(
            '<div class="flex-container mb5" data-layout="' +
              layout +
              '"> \
                        <input type="checkbox" name="selected" class="mr5 ml0" placeholder="Selected" ' +
              (layout < setuptools.config.vaultbuilderMinimumId
                ? ""
                : "checked") +
              '>\
                        <input value="' +
              files.layouts[layout].layoutname +
              '" type="text" name="name" placeholder="Layout Name" class="setuptools flex-container noFlexAutoWidth mr5" style="height: 17px; flex-grow: 1; width: 25%;">\
                        <input value="' +
              (+layout < setuptools.config.vaultbuilderMinimumId
                ? ""
                : layout) +
              '" type="text" name="layout" placeholder="' +
              (+layout < setuptools.config.vaultbuilderMinimumId
                ? layout
                : "Layout ID") +
              '" class="setuptools flex-container noFlexAutoWidth" style="height: 17px; flex-grow: 1; width: 25%;">\
                    </div>'
          )
            .css({ display: "none" })
            .appendTo(container)
          ...
          dom.find('input[name="layout"]').each(function (e) {
            ...
            $(this).on("change.inputvalidation", function () {
              if (+$(this).val() < setuptools.config.vaultbuilderMinimumId)
                $(this)
                  .attr(
                    "placeholder",
                    "Must be >= " + setuptools.config.vaultbuilderMinimumId
                  )
                  .val("");
            });
          });
          ...
          dom.find('input[type="checkbox"]').each(function () {
            $(this).on("change.vaultbuilder.import.selected", function (e) {
              ...
              if (
                typeof +$(this).siblings('input[name="layout"]').val() !==
                  "number" ||
                +$(this).siblings('input[name="layout"]').val() <
                  setuptools.config.vaultbuilderMinimumId
              ) {
                this.checked = false;
              }
        ...
        $("div.setuptools.link.vaultbuilder.import.save").on(
          "click.vaultbuilder.save",
          function () {
            var selected = [];
            ...
            selected.forEach(function (layout) {
              ...
              if (!layout || +layout < setuptools.config.vaultbuilderMinimumId)
                return;
              setuptools.data.muledump.vaultbuilder.layouts[layout] =
                data[layout];
            });
            ...
            setuptools.app.config.save(
              "Muledump/VaultBuilder imported configuration"
            );
            setuptools.lightbox.menu.context.close();
          }
        );
      ...
      $("div.setuptools.link.vaultbuilder.saveExport").on(
        "click.vaultbuilder.export",
        function () {
          //  initiate our zip file
          var zip = new JSZip();
          $('select[name="exportLayouts"] option:selected').each(function () {
            zip.file(
              "layouts/" + $(this).val() + ".json",
              JSON.stringify(
                setuptools.data.muledump.vaultbuilder.layouts[$(this).val()]
              )
            );
          });
  ...
  setuptools.lightbox.menu.context.create(
    "mulemenu-vaultbuilderMenuImport",
    false,
    position,
    options
  );
...
setuptools.app.muledump.vaultbuilder.menu.duplicate = function () {
  ...
  Object.keys(setuptools.data.muledump.vaultbuilder.layouts).filter(function (
    ...
    layoutOptions +=
      ' \
            <option style="background-color: #3f3f3f; outline: 0;" value="' +
      index +
      '" ' +
      (setuptools.data.muledump.vaultbuilder.config.layout === +index
        ? "selected"
        : "") +
      ">" +
      data.layoutname +
  ...
  options.push({
    ...
    binding: function () {
      ...
      $("div.setuptools.link.vaultbuilder.duplicateNew").on(
        "click.saveAsConfirm",
        function () {
          var input = $('input[name="duplicateNewName"]');
          setuptools.app.muledump.vaultbuilder.tasks.load(
            +$('select[name="duplicateFrom"]').val(),
            true
          );
          if (
            setuptools.app.muledump.vaultbuilder.tasks.saveAs(input.val()) ===
            true
          ) {
            setuptools.app.muledump.vaultbuilder.ui();
            return;
          ...
          input
            .addClass("conflict")
            .val("")
            .attr("placeholder", "Cannot use that name");
        }
      );
    },
  });
  ...
  setuptools.lightbox.menu.context.create(
    "mulemenu-vaultbuilderMenuDuplicate",
    false,
    position,
    options
  );
...
setuptools.app.muledump.vaultbuilder.menu.create = function () {
  if (
    setuptools.lightbox.menu.context.isOpen(
      "mulemenu-vaultbuilderMenuCreate"
    ) === true
  ) {
    setuptools.lightbox.menu.context.close();
    return;
  ...
  options.push({
    ...
    value:
      ' \
        ...
        ',
    binding: function () {
      $('div.setuptools.menu input[name="createNewWidth"]').on(
        "keydown.validateinput",
        setuptools.app.muledump.vaultbuilder.validateNumericInput
      );
      $('div.setuptools.menu input[name="createNewHeight"]').on(
        "keydown.validateinput",
        setuptools.app.muledump.vaultbuilder.validateNumericInput
      );

      $(
        "div.setuptools.menu div.setuptools.link.vaultbuilder.createNew.confirm"
      ).on("click.createNew", async function () {
        ...
        if (
          (await setuptools.app.muledump.vaultbuilder.tasks.create(
            name,
            width,
            height
          )) === true
        ...
        $('input[name="createNewName"]')
          .addClass("conflict")
          .val("")
          .attr("placeholder", "Choose another");
      });
...
setuptools.app.muledump.vaultbuilder.menu.saveAs = function () {
  if (
    setuptools.lightbox.menu.context.isOpen(
      "mulemenu-vaultbuilderMenuSaveAs"
    ) === true
  ) {
    setuptools.lightbox.menu.context.close();
    return;
  ...
  options.push({
    ...
    value:
      ' \
        ...
        ',
    binding: function () {
      $("div.setuptools.link.vaultbuilder.saveAs").on(
        "click.saveAsConfirm",
        function () {
...
setuptools.app.muledump.vaultbuilder.menu.rename = function () {
  if (
    setuptools.lightbox.menu.context.isOpen(
      "mulemenu-vaultbuilderMenuRename"
    ) === true
  ) {
    setuptools.lightbox.menu.context.close();
    return;
  ...
  options.push({
    ...
    value:
      ' \
        ...
        ',
    binding: function () {
      $("div.setuptools.link.vaultbuilder.renameAs").on(
        "click.renameAsConfirm",
        function () {
          setuptools.app.muledump.vaultbuilder.tasks.rename(
            setuptools.data.muledump.vaultbuilder.config.layout,
            $('input[name="renameAs"]').val()
          );
          setuptools.app.muledump.vaultbuilder.ui();
        }
      );
...
setuptools.app.muledump.vaultbuilder.menu.accountView = function () {
  ...
  setuptools.lightbox.menu.context.create(
    "mulemenu-vaultbuilderAccountView",
    false,
    position,
    options
  );
...
setuptools.app.muledump.vaultbuilder.menu.options = function () {
  ...
  Object.keys(mules).filter(function (guid) {
    if (
      Object.keys(accountView).length >
      setuptools.data.config.vaultbuilderAccountViewLimit + 1
    )
      return;
  ...
  if (
    ...
    options.push(
      ...
      },
      {
        ...
        name: "Delete",
        callback: function () {
          setuptools.app.muledump.vaultbuilder.tasks.delete(
            setuptools.data.muledump.vaultbuilder.config.layout
          );
          setuptools.app.muledump.vaultbuilder.ui();
        },
  ...
  setuptools.lightbox.menu.context.create(
    "mulemenu-vaultbuilderMenuOptions",
    false,
    position,
    options
  );
