...
setuptools.app.groups.manager = function (group, open) {
  ...
  //
  if (
    typeof setuptools.data.groups.format === "undefined" ||
    Object.keys(setuptools.data.groups.groupList).length === 0
  ) {
    ...
    for (i = 0; i < setuptools.data.groups.groupOrder.length; i++) {
      ...
      if (
        group.indexOf(open) === -1 &&
        group.indexOf(setuptools.data.groups.groupOrder[i]) > -1
      ) {
        selected = " selected";
      } else {
        for (x = 0; x < group.length; x++) {
          if (
            (!open || group.indexOf(open) > -1) &&
            group[x] === setuptools.data.groups.groupOrder[i]
          ) {
            selected = " selected";
            break;
          }
      ...
      setuptools.lightbox.build(
        "groups-manager",
        ' \
                <div class="setuptools groupMember cell' +
          selected +
          '" \
                data-groupName="' +
          setuptools.data.groups.groupOrder[i] +
          '">' +
          setuptools.data.groups.groupOrder[i] +
          "</div> \
            "
  ...
  setuptools.lightbox.drawhelp(
    "groups-manager",
    "docs/setuptools/help/groups-manager/manager",
    "Groups Manager Help"
  );
  setuptools.lightbox.display("groups-manager", {
    variant: "fl-GroupsManager",
  });

  $(".setuptools.link.groups.add").click(setuptools.app.groups.add);

  $(".setuptools.link.groups.toggle").click(async function () {
    var self = $(this);
    var label = self.text();
    ...
    for (var i in setuptools.data.groups.groupList)
      if (setuptools.data.groups.groupList.hasOwnProperty(i))
        setuptools.data.groups.groupList[i].enabled =
          self.hasClass("enableAll") === true;

    //  save and report changes
    if ((await setuptools.app.config.save("GroupsManager/Toggle")) === true) {
  ...
  function OpenEditor(groupName, self) {
    ...
    var ListAccountTiles = function (groupName, page) {
      var availableAccountsList =
        setuptools.lightbox.menu.paginate.state.availableAccounts.PageList;
      if (
        page >
        setuptools.lightbox.menu.paginate.state.availableAccounts.lastPage
      )
        page =
          setuptools.lightbox.menu.paginate.state.availableAccounts.lastPage;
      $('div.availableAccounts div.customPage input[name="customPage"]').val(
        page + 1
      );
      if (typeof groupName !== "string")
        setuptools.lightbox.error("Supplied groupName was not a string.", 28);
      ...
      if (maxIndex > availableAccountsList.length)
        maxIndex = availableAccountsList.length;
      if (
        availableAccountsList.length <= setuptools.data.config.accountsPerPage
      ) {
        minIndex = 0;
        maxIndex = availableAccountsList.length;
      }
      ...
      for (i = minIndex; i < maxIndex; i++)
        html +=
          '<div class="cell noselect">' + availableAccountsList[i] + "</div>";

      if (html.length === 0) {
        html +=
          '<div class="cell noselect nocontext">No available accounts</div>';
      }
    ...
    var ListGroupMembers = function (groupName, page) {
      var groupMembersList =
        setuptools.lightbox.menu.paginate.state.groupMembers.PageList;
      if (page > setuptools.lightbox.menu.paginate.state.groupMembers.lastPage)
        page--;
      $('div.groupMembers div.customPage input[name="customPage"]').val(
        page + 1
      );
      if (typeof groupName !== "string")
        setuptools.lightbox.error("Supplied groupName was not a string.", 28);
      ...
      var maxIndex =
        setuptools.data.config.accountsPerPage * page +
        setuptools.data.config.accountsPerPage;
      if (maxIndex > groupMembersList.length)
    ...
    function availableAccountsUpdate(direction, account) {
      if (direction === "add" && typeof account !== "string")
        setuptools.lightbox.error(
          "A valid account must be provided when adding",
          30
        );
      if (direction === "add")
      ...
      if (direction === "reset")
        setuptools.lightbox.menu.paginate.findPage(0, "availableAccounts");
      ...
      $("div.setuptools.groups.editor > div.availableAccounts.list").html(
        " \
          ...
          ' \
                <div class="dragbox">' +
          data.ActionCallback(data.ActionItem, data.currentPage) +
          "</div> \
            "
      );
    ...
    function groupMembersUpdate(direction, account) {
      ...
      if (direction === "add")
        setuptools.lightbox.menu.paginate.findPage(account, "groupMembers");
      if (direction === "reset")
        setuptools.lightbox.menu.paginate.findPage(0, "groupMembers");
      ...
      if (["remove", "reset"].indexOf(direction) > -1) {
        var newLastPage = Math.ceil(
          data.PageList.length / setuptools.data.config.accountsPerPage
        );
        if (newLastPage > 0) newLastPage--;
        if (data.currentPage > newLastPage)
          setuptools.lightbox.menu.paginate.findPage(
            data.PageList.length - 1,
            "groupMembers"
          );
      }
    ...
    function availableAccountsContext() {
      function addToBottom(account) {
        ...
        state.availableAccounts.PageList.splice(
          state.availableAccounts.PageList.indexOf(account),
          1
        );
      ...
      function addToCurrent(account) {
        var state = setuptools.lightbox.menu.paginate.state;
        var index =
          state.groupMembers.currentPage *
          setuptools.data.config.accountsPerPage;
      ...
      function AddAll() {
        var state = setuptools.lightbox.menu.paginate.state;
        setuptools.lightbox.menu.paginate.state.groupMembers.PageList =
          setuptools.lightbox.menu.paginate.state.groupMembers.PageList.concat(
        ...
        groupMembersUpdate(
          "add",
          setuptools.lightbox.menu.paginate.state.groupMembers.PageList[0]
        );
      ...
      var cell = $(
        "div.availableAccounts > div.dragbox > div.cell:not(.nocontext)"
      );
      ...
      cell.click(async function () {
        ...
        if (
          ...
          Object.keys(setuptools.tmp.contextMenuOpen).length > 0
        ) {
        ...
        if ($(this).hasClass("selected") === false) {
          ...
          var options = [
            ...
            },
            {
              ...
              value: groupMembersPaginate.html.search.replace(
                "Search by Name",
                "Add After Account..."
              ),
              binding: function () {
                ...
                function addToSpecified(account) {
                  ...
                  state.availableAccounts.PageList.splice(
                    state.availableAccounts.PageList.indexOf(callbackArg),
                    1
                  );
                ...
                $("div.input div.setuptools.div.pageControls div.search").click(
                  async function () {
    ...
    function groupMembersContext() {
      ...
      function RemoveAccount(account) {
        ...
        if (index === -1) return;
        state.PageList.splice(index, 1);
        setuptools.lightbox.menu.paginate.state.availableAccounts.PageList.push(
          account
        );
      ...
      function RemoveAll() {
        var state = setuptools.lightbox.menu.paginate.state;
        setuptools.lightbox.menu.paginate.state.availableAccounts.PageList =
          setuptools.lightbox.menu.paginate.state.availableAccounts.PageList.concat(
        ...
        availableAccountsUpdate(
          "add",
          setuptools.lightbox.menu.paginate.state.availableAccounts.PageList[0]
        );
      ...
      cell.contextmenu(function (e) {
        ...
        if ($(this).hasClass("selected") === false) {
          ...
          if (position.length === 0) {
            position = $(
              "div.groupMembers div.pageControls > div.editor.control.searchName"
            );
          }
            ...
            },
            {
              ...
              binding: function () {
                ...
                function addToSpecified(account) {
                  ...
                  //  find the index of the searched account
                  var OldIndex =
                    state.groupMembers.PageList.indexOf(callbackArg);
                  if (state.groupMembers.PageList.indexOf(account) === -1)
                    return;

                  //  move the account there
                  state.groupMembers.PageList.splice(OldIndex, 0, callbackArg);
                  state.groupMembers.PageList.splice(
                    state.groupMembers.PageList.indexOf(account) + 1,
                    0,
                    callbackArg
                  );
    ...
    for (var i in setuptools.data.accounts.accounts)
      if (setuptools.data.accounts.accounts.hasOwnProperty(i))
        if (
          setuptools.data.groups.groupList[groupName].members.indexOf(i) === -1
        ) {
    ...
    if (group.indexOf(open) > -1)
      $(
        '.setuptools.groupMember.cell[data-groupName!="' + groupName + '"]'
      ).removeClass("selected");
    $(".setuptools.div.groupEditor.container > h3").text(
      "Group Editor - " +
        groupName +
        " (" +
        (setuptools.data.groups.groupList[groupName].enabled === true
          ? "Enabled"
          : "Disabled") +
        ")"
    );
    ...
    $(".setuptools.revert.group").click(async function () {
      setuptools.lightbox.close("groups-manager");
      setuptools.app.groups.manager(groupName, true);
    });

    $(".setuptools.save.group").click(async function () {
      setuptools.data.groups.groupList[groupName].members =
        setuptools.lightbox.menu.paginate.state.groupMembers.PageList;
      if ((await setuptools.app.config.save("GroupsManager/Save")) === true) {
        $("div.save.result").html("<div>Saved!</div>");
        $("div.save.result > div").fadeOut(2500, function () {
          $(this).remove();
        });

        $(".controller.notifier.saveNotice")
          .removeClass("empty")
          .addClass("notice")
          .text("Reload Muledump to Apply")
          .css({ cursor: "pointer" })
  ...
  function CloseEditor() {
    ...
    $(".setuptools.div.groups.editor").text(
    ...
    for (var i in setuptools.data.groups.groupList)
      if (setuptools.data.groups.groupList.hasOwnProperty(i))
        if (setuptools.data.groups.groupList[i].enabled === true)
          $(
            '.setuptools.groupMember.cell[data-groupName="' + i + '"]'
          ).addClass("selected");
  }
  ...
  function EnableSelected() {
    $(".setuptools.groupMember.cell.selected").each(function (index, element) {
      ...
      if (setuptools.data.groups.groupList[groupName])
        setuptools.data.groups.groupList[groupName].enabled = true;
  ...
  function DisableSelected() {
    $(".setuptools.groupMember.cell.selected").each(function (index, element) {
      var groupName = $(element).text();
      if (setuptools.data.groups.groupList[groupName])
        setuptools.data.groups.groupList[groupName].enabled = false;
  ...
  function GroupMenu(e) {
    ...
    function MoveToStart() {
      ...
      for (i = 0; i < selectedClass.length; i++) {
        setuptools.data.groups.groupOrder.splice(
          setuptools.data.groups.groupOrder.indexOf($(selectedClass[i]).text()),
          1
        );
        setuptools.data.groups.groupOrder.splice(
          i,
          0,
          $(selectedClass[i]).text()
        );
    ...
    function MoveLeft() {
      ...
      var groupList = [];
      var index =
        setuptools.data.groups.groupOrder.indexOf($(selectedClass[0]).text()) -
        1;
      if (index < 0) index = 0;
      for (i = 0; i < selectedClass.length; i++) {
        setuptools.data.groups.groupOrder.splice(
    ...
    function MoveRight() {
      ...
      var groupList = [];
      var index =
        setuptools.data.groups.groupOrder.indexOf(
          $(selectedClass[$(selectedClass).length - 1]).text()
        ) + 1;
      if (index > setuptools.data.groups.groupOrder.length)
        index = setuptools.data.groups.groupOrder;
      for (i = 0; i < selectedClass.length; i++) {
        setuptools.data.groups.groupOrder.splice(
          setuptools.data.groups.groupOrder.indexOf($(selectedClass[i]).text()),
          1
    ...
    $(".setuptools.div.groupControls div:not(.groupMenu)").each(function (
      index,
      element
    ) {
    ...
    var selectedClass = $(
      ".setuptools.div.groupList div.dragbox div.cell.selected"
    );
    ...
    //  single group menu
      ...
      if (
        $(".setuptools.div.groupControls div.groupMenu").hasClass(
          "selected"
        ) === false
      ) {
        //  base info
        ...
        var position = $(
          ".setuptools.div.groupList div.dragbox div.cell:first-child"
        );
        var options = [
          {
            option: "hover",
        ...
        if (
          $(".setuptools.div.groupList div.dragbox div.cell:not(.selected)")
            .length > 0
        ) {
          options.push({
            class: "moveToStart",
  ...
  //  display group menu
  $(".setuptools.div.groupControls div.groupMenu")
    .unbind("click")
    .click(GroupMenu);
  ...
  $(".setuptools.div.groupControls div.groupSelect").click(function (e) {
    ...
    //  deselect other menu options
    $(".setuptools.div.groupControls div:not(.groupSelect)").each(function (
      index,
      element
    ) {
      $(element).removeClass("selected");
    ...
    //  display the menu
    if (
      $(".setuptools.div.groupControls div.groupSelect").hasClass(
        "selected"
      ) === false
    ) {
      //  base info
  ...
  if (
    typeof open === "string" &&
    setuptools.data.groups.groupOrder.indexOf(open) > -1
  )
    OpenEditor(open, $('.div[data-groupName="' + open + '"]'));
  if (
    open === true &&
    group.length === 1 &&
    setuptools.data.groups.groupOrder.indexOf(group[0]) > -1
  )
    OpenEditor(group[0], $('.div[data-groupName="' + group[0] + '"]'));
  ...
  $(".setuptools.groupMember.cell")
    ...
    .on("mouseup", function (e) {
      ...
      if (firstSelected !== lastSelected) {
        ...
        var firstIndex =
          setuptools.data.groups.groupOrder.indexOf(firstSelected);
...
setuptools.app.groups.delete = function (groupName) {
  ...
  if (typeof groupName === "object") {
    ...
    if (groupNames.length === 0 && typeof groupName !== "string")
      setuptools.lightbox.error(
        "Either groupName was not a string or was not provided",
        27
      );
  }
  ...
  if (groupNames.length > 1)
    setuptools.lightbox.build(
      "groups-delete",
      "Are you sure you wish to delete the following groups?"
    );
  ...
  setuptools.lightbox.drawhelp(
    "groups-delete",
    "docs/setuptools/help/groups-manager/delete",
    "Delete Group Help"
  );
  setuptools.lightbox.display("groups-delete");

  $(".setuptools.link.confirm").click(async function () {
    //  parse the provided group list
    groupList = groupList.split(", ");
    for (i = 0; i < groupList.length; i++) {
      delete setuptools.data.groups.groupList[groupList[i]];
      setuptools.data.groups.groupOrder.splice(
        setuptools.data.groups.groupOrder.indexOf(groupList[i]),
        1
      );
    }
...
setuptools.app.groups.copy = function (groupName, newGroupName) {
  if (typeof setuptools.data.groups.groupList[groupName] === "undefined") {
    setuptools.lightbox.build(
      "groups-copy",
      'The specified group "' + groupName + '" does not exist.'
    );
  } else {
    if (newGroupName)
      setuptools.lightbox.build(
  ...
  function Process() {
    ...
    setuptools.data.groups.groupList[newGroupName] = $.extend(
      true,
      {},
      setuptools.data.groups.groupList[groupName]
    );
...
setuptools.app.groups.rename = function (groupName, newGroupName) {
  if (typeof setuptools.data.groups.groupList[groupName] === "undefined") {
    ...
    setuptools.lightbox.build(
      "groups-rename",
      ' \
            Provide the name you wish to rename group "' +
        groupName +
        '" as. <br><br> \
            <div class="w400px">\
                <div class="setuptools div app groups">\
  ...
  setuptools.lightbox.display("groups-rename", {
  ...
  function Process() {
    ...
    if (
      typeof setuptools.data.groups.groupList[newGroupName] === "object" ||
      newGroupName.length < 1
    ) {
...
setuptools.app.groups.merge = function (selectedClass, newGroupName) {
  if (selectedClass.length === 0) {
    setuptools.lightbox.build(
      "groups-merge",
      "No selected groups were included to be merged."
    );
  } else {
    if (newGroupName)
      setuptools.lightbox.build(
  ...
  setuptools.lightbox.drawhelp(
    "groups-merge",
    "docs/setuptools/help/groups-manager/merge",
    "Merge Groups Help"
  );
  ...
  function Process() {
    ...
    for (i = selectedClass.length - 1; i >= 0; i--)
      if ($(selectedClass[i]).text() === newGroupName) nameAsExisting = true;

    if (
      typeof setuptools.data.groups.groupList[newGroupName] === "object" &&
      nameAsExisting === false
    ) {
    ...
    for (i = selectedClass.length - 1; i >= 0; i--) {
      ...
      newMemberList = newMemberList.concat(
        setuptools.data.groups.groupList[currentGroup].members
      );
...
setuptools.app.groups.add = function (groupName) {
  ...
  if (typeof setuptools.data.groups.groupList[groupName] === "object") {
    setuptools.lightbox.build(
      "groups-add",
      "The group name specified already exists.<br><br>"
    );
  }
  ...
  for (i = 1; i <= setuptools.data.groups.groupOrder.length; i++) {
    setuptools.lightbox.build(
      ...
      " \
                <option " +
        (i === setuptools.data.groups.groupOrder.length ? "selected" : "") +
        ">" +
        Number(i + 1) +
        "</option> \
            "
  ...
  setuptools.lightbox.drawhelp(
    "groups-add",
    "docs/setuptools/help/groups-manager/add",
    "New Group Help"
  );
  ...
  function GroupSave(doClose) {
    ...
    if (typeof setuptools.data.groups.groupList[groupName] !== "object") {
      ...
      setuptools.data.groups.groupList[groupName] = $.extend(
        true,
        {},
        setuptools.objects.group,
        { enabled: enabled }
      );
  ...
  $(".setuptools.div.app select, .setuptools.div.app input").keyup(function (
    e
  ) {
...
setuptools.app.groups.load = function (accounts) {
  ...
  for (i = 0; i < setuptools.data.groups.groupOrder.length; i++) {
    ...
    for (x = 0; x < groupData.members.length; x++) {
      if (
        typeof setuptools.data.accounts.accounts[groupData.members[x]] ===
        "undefined"
      ) {
        trigger = true;
      ...
      for (x = 0; x < groupData.members.length; x++) {
        ...
        if (
          ...
          setuptools.app.techlog(
            "Groups/Load Warning: Found unknown account " +
              groupData.members[x],
            "force"
          );
          continue;
        ...
        if (
          setuptools.data.accounts.accounts[groupData.members[x]].enabled ===
          true
        ) {
          if (setuptools.data.config.groupsMergeMode === 1) {
            if (MasterList.indexOf(groupData.members[x]) === -1) {
              if (typeof AccountList[groupName] === "undefined")
                AccountList[groupName] = [];
  ...
  if (compact.length > 0) {
    for (var j = 0; j < compact.length; j++)
      setuptools.data.groups.groupList[compact[j]].members
  ...
  if (setuptools.data.config.groupsMergeMode === 1) {
    ...
    for (var i in AccountList)
      if (AccountList.hasOwnProperty(i))
        if (AccountList[i].length > maxIndex) maxIndex = AccountList[i].length;
  ...
  for (x = 0; x < AccountList.length; x++) {
    ...
    if (typeof accounts[AccountName] === "string")
      groupAccounts[AccountName] = accounts[AccountName];
  }
